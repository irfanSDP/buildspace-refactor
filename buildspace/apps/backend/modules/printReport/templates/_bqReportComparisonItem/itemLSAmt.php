<?php if (! empty($itemRow[0])): ?>
    ...
    <?php if($participate) : ?>
        <td class="bqRateCell" style="text-align:center;"><?php echo (! $printNoPrice  && $rate > 0 && $itemId > 0 && $rationalizedRates && array_key_exists($itemId, $rationalizedRates) && $rationalizedRates[$itemId] != 0 )? number_format(($rationalizedRates[$itemId] - $rate) / $rate * 100, 2).'%' : null ?></td>
    <?php else:?>
