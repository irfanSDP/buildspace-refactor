<table cellpadding="0" cellspacing="0" class="mainTable">
    ...
    <tr>
        ...
        <td class="bqHeadCell" rowspan="2" style="min-width:300px;width:300px;">
            <?php echo $descHeader; ?>
        </td>
        <td class="bqHeadCell" colspan="3">
            <?php echo "Amount"; ?>
        </td>
    ...
    for($x=0; $x <= $maxRows; $x++):
        ...
        $isApprove  = $itemRow ? $itemRow[sfBuildspaceVOWithClaimsReportGenerator::ROW_APPROVED] : null;
        $omission   = $itemRow ? $itemRow[sfBuildspaceVOWithClaimsReportGenerator::ROW_OMISSION] : null;
        $addition   = $itemRow ? $itemRow[sfBuildspaceVOWithClaimsReportGenerator::ROW_ADDITION] : null;
        $net        = $itemRow ? $itemRow[sfBuildspaceVOWithClaimsReportGenerator::ROW_NET] : null;
        $percentage = $itemRow ? $itemRow[sfBuildspaceVOWithClaimsReportGenerator::ROW_CLAIM_WORKDONE]['up_to_date_percentage'] : null;
        $amount     = $itemRow ? $itemRow[sfBuildspaceVOWithClaimsReportGenerator::ROW_CLAIM_WORKDONE]['total_claim'] : null;
        ...
        <tr>
            ...
            <td class="bqRateCell" style="text-align:right;">
                <?php echo $percentage && $percentage != 0 ? number_format($percentage,2).'%' : null ?>
            </td>
            <td class="bqRateCell">
                <?php echo ! $printNoPrice && $amount && $amount != 0 ? number_format($amount, $priceFormatting[2], $priceFormatting[0], ($rateCommaRemove) ? '' : $priceFormatting[1]) : null ?>
            </td>
        </tr>
        ...
        <?php if($printGrandTotal) : ?>
            <tr>
                ...
                <td class="footerSumAmount">
                    <?php echo (array_key_exists('nett_omission_addition', $voTotals)) ? number_format($voTotals['nett_omission_addition'], $priceFormatting[2], $priceFormatting[0], ($rateCommaRemove) ? '' : $priceFormatting[1]) : null; ?>
                </td>
                <td class="footerSumAmount">
                    <?php echo (array_key_exists('nett_omission_addition', $voTotals) && $voTotals['nett_omission_addition'] > 0 && array_key_exists('total_claim', $voTotals)) ? number_format($voTotals['total_claim'] / $voTotals['nett_omission_addition'] * 100, 2).'%' : null; ?>
                </td>
                <td class="footerSumAmount">
                    <?php echo (array_key_exists('total_claim', $voTotals)) ? number_format($voTotals['total_claim'], $priceFormatting[2], $priceFormatting[0], ($rateCommaRemove) ? '' : $priceFormatting[1]) : null; ?>
                </td>
            </tr>
        <?php else: ?>
            <tr>
