<table cellpadding="0" cellspacing="0" class="mainTable">
    <tr>
        ...
        {
            if(array_key_exists($column['id'], $unitNames) && count($unitNames[$column['id']]) > 0)
            {
                $headerCount+= 3;
                $headerCount+= count($unitNames[$column['id']]);
            }
        }
        ?>
    ...
    <tr>
        ...
        <?php foreach($billColumnSettings as $column) :?>
            <?php if(array_key_exists($column['id'], $unitNames) && count($unitNames[$column['id']]) > 0) : ?>
                <?php
                $colspan = 3 + count($unitNames[$column['id']]);
                ?>
    ...
    <tr>
        <?php foreach($billColumnSettings as $column) :?>
            <?php if(array_key_exists($column['id'], $unitNames) && count($unitNames[$column['id']]) > 0) : ?>
                ...
                <?php foreach($unitNames[$column['id']] as $counter => $name) :?>
        ...
        <tr>
            ...
            <?php foreach($billColumnSettings as $column) :?>
                <?php if(array_key_exists($column['id'], $unitNames) && count($unitNames[$column['id']]) > 0) : ?>
                    ...
                    <?php if($itemId > 0 && $withUnit && array_key_exists($column['id'], $elementTypeTotals[$itemId]) && array_key_exists('unit_total', $elementTypeTotals[$itemId][$column['id']]) && count($elementTypeTotals[$itemId][$column['id']]['unit_total'])): ?>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <?php for($i = 1; $i <= count($unitNames[$column['id']]); $i++) :?>
                            <td class="bqRateCell">
                                &nbsp;
                            </td>
                        <?php endfor; ?>
