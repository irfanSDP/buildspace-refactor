<table cellpadding="0" cellspacing="0" class="mainTable">
    ...
    <tr>
        ...
        <td class="bqHeadCell"
            style="min-width:400px;width: <?php echo $descriptionWidth; ?> px;"><?php echo $descHeader; ?></td>
    ...
    for ( $x = 0; $x <= $maxRows; $x ++ ):
        ...
        if ( $itemId > 0 && count($tenderers) )
        {
            ...
            {
                if ( array_key_exists($itemId, $tenderersElementTotals) && array_key_exists($tenderer['id'], $tenderersElementTotals[$itemId]) )
                {
                    $listOfRates[$tenderer['id']] = $tenderersElementTotals[$itemId][$tenderer['id']];
                }
            }
        ...
        <tr>
            ...
            <?php if ( count($tenderers) ) : $counter = 1; ?>
                <?php foreach ( $tenderers as $k => $tenderer ) : ?>
                    ...
                    ?>">
                        <?php echo !$printNoPrice && $itemId > 0 && $tenderersElementTotals && array_key_exists($itemId, $tenderersElementTotals) && array_key_exists($tenderer['id'], $tenderersElementTotals[$itemId]) && $tenderersElementTotals[$itemId][$tenderer['id']] != 0 ? number_format($tenderersElementTotals[$itemId][$tenderer['id']], $priceFormatting[2], $priceFormatting[0], ( $rateCommaRemove ) ? '' : $priceFormatting[1]) : null ?>
                    </td>
                <?php endforeach; ?>
            <?php endif;?>
        </tr>
    ...
    <?php if ( $printGrandTotal ) : ?>
        <tr>
            ...
            <?php foreach ( $tenderers as $k => $tenderer ) : ?>
                <td class="footerSumAmount">
                    <?php echo ( $contractorBillGrandTotals && array_key_exists($tenderer['id'], $contractorBillGrandTotals) && $contractorBillGrandTotals[$tenderer['id']] != 0 ) ? number_format($contractorBillGrandTotals[$tenderer['id']], $priceFormatting[2], $priceFormatting[0], ( $rateCommaRemove ) ? '' : $priceFormatting[1]) : null; ?>
                </td>
            <?php endforeach; ?>
        </tr>
        <tr>
