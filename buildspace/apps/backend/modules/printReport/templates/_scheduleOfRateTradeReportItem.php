<table cellpadding="0" cellspacing="0" class="mainTable">
    ...
    <?php
        ...
        for($x=0; $x <= $maxRows; $x++):
            ...
            $multi = $itemRow ? $itemRow[sfBuildspaceScheduleOfRateItemGenerator::ROW_BILL_ITEM_MULTI_RATE] : false;

            $rate = $itemRow ? $itemRow[sfBuildspaceScheduleOfRateItemGenerator::ROW_BILL_ITEM_RATE] : null;
            
            $unit = $itemRow ? $itemRow[sfBuildspaceScheduleOfRateItemGenerator::ROW_BILL_ITEM_UNIT] : null;

            $markup = $itemRow ? $itemRow[sfBuildspaceScheduleOfRateItemGenerator::ROW_BILL_ITEM_MARKUP] : null;

            $totalQty = $itemRow ? $itemRow[sfBuildspaceScheduleOfRateItemGenerator::ROW_BILL_ITEM_TOTAL_QTY] : null;

            $totalCost = $itemRow ? $itemRow[sfBuildspaceScheduleOfRateItemGenerator::ROW_BILL_ITEM_TOTAL_COST] : null;
            ...
            elseif($itemRow and $itemRow[4] == sfBuildspaceScheduleOfRateItemGenerator::ROW_TYPE_ELEMENT)
            {
            ...
            if ( $indentItem AND $itemRow AND ($itemRow[4] != sfBuildspaceScheduleOfRateItemGenerator::ROW_TYPE_ELEMENT AND $itemRow[4] != ResourceItem::TYPE_HEADER) )
            {
                $itemPadding = 15;
            }
            else
        ...
        <tr>
            ...
            <?php endif; ?>
            <td class="bqRateCell">
                <?php echo ! $printNoPrice && $markup && $markup != 0 ? number_format($markup, 2).'%' : null ?>
            </td>
            <td class="bqQtyCell">
