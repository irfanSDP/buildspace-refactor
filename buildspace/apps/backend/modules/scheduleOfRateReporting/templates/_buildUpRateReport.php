<table cellpadding="0" cellspacing="0" class="mainTable">
    ...
    <tr>
        ...
        <td class="bqDescriptionCell" style="border-right: none;"><pre class="description"><?php echo $billItemInfo[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_DESCRIPTION]; ?></pre></td>

        <td class="bqRateCell" style="border-right: none;">&nbsp;</td>
        <td class="bqRateCell" style="border-right: none;">&nbsp;</td>
        <td class="bqRateCell" style="border-right: none;">&nbsp;</td>

        <?php if ( $billItemInfo[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_ID] > 0 ): ?>
    ...
    <?php
        ...
        for($x=0; $x <= $maxRows; $x++):
            ...
            $number           = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_NUMBER]['final_value'] : NULL;
            $constant         = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_CONSTANT]['final_value'] : NULL;
            $qty              = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_QTY]['final_value'] : NULL;
            $rate             = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_RATE]['final_value'] : NULL;
            $wastage          = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_WASTAGE]['final_value'] : NULL;
            $unit             = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_UNIT] : NULL;

            $total            = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_TOTAL] : NULL;
            $lineTotal        = $itemRow ? $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_LINE_TOTAL] : NULL;
            ...
            if ( $itemRow )
            {
                if ( $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_NUMBER]['has_formula'] )
                {
                    $numberColor = 'color: #F78181;';
                }

                if ( $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_CONSTANT]['has_formula'] )
                {
                    $constantColor = 'color: #F78181;';
                }

                if ( $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_QTY]['has_formula'] )
                {
                    $qtyColor = 'color: #F78181;';
                }

                if ( $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_RATE]['has_formula'] )
                {
                    $rateColor = 'color: #F78181;';
                }

                if ( $itemRow[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_WASTAGE]['has_formula'] )
                {
