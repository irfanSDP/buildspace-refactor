...
<table cellpadding="0" cellspacing="0" class="mainTable">
    <tr>
        ...
        <td colspan="<?php echo $headerCount; ?>">
            <?php include_partial('postContractRemeasurementReport/buildUpQtyHeaderReport', array(
    ...
    <tr>
        ...
        <td class="bqDescriptionCell" style="border-right: none;"><pre class="description"><?php echo $billItemInfo[sfBillItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_DESCRIPTION]; ?></pre></td>

        <?php if ( $billItemInfo[sfBillItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_ID] > 0 ): ?>
    ...
    <?php
        ...
        for($x=0; $x <= $maxRows; $x++):
            ...
            $factor           = $itemRow ? $itemRow[sfBillItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_FACTOR] : null;
            $total            = $itemRow ? $itemRow[sfBillItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_TOTAL] : null;
            $sign             = $itemRow ? $itemRow[sfBillItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_SIGN] : null;
            ...
            if ($itemRow and ($itemRow[4] == sfBillItemBuildUpQtyReportGenerator::ROW_SOQ_HEADER_TYPE))
            {
                $headerStyle = 'font-weight:bold;text-decoration:underline;';
            }

            if ($itemRow and ($itemRow[4] == sfBillItemBuildUpQtyReportGenerator::ROW_SOQ_ITEM_TYPE))
            {
                $headerStyle = 'font-weight:bold;';
            }

            if ($itemRow and ($itemRow[4] == sfBillItemBuildUpQtyReportGenerator::ROW_SOQ_MEASUREMENT_TYPE))
            {
        ...
        <tr>
            ...
            <?php foreach ( $dimensions as $dimension ): ?>
                ...
                if ( isset($itemRow[sfBillItemBuildUpQtyReportGenerator::ROW_FORMULATED_COLUMNS][$dimension['id'].'-dimension_column']) ) {
                    $formulatedColValue = $itemRow[sfBillItemBuildUpQtyReportGenerator::ROW_FORMULATED_COLUMNS][$dimension['id'].'-dimension_column']['final_value'];

                    if ( $itemRow[sfBillItemBuildUpQtyReportGenerator::ROW_FORMULATED_COLUMNS][$dimension['id'].'-dimension_column']['has_formula'] )
                    {
            ...
            <td class="bqUnitCell <?php echo $fontColorStyling; ?>" style="<?php echo $borderTopStyling; ?>">
                <?php echo ($itemRow[sfBillItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_ID]) ? $sign : "&nbsp;"; ?>
            </td>
        </tr>
