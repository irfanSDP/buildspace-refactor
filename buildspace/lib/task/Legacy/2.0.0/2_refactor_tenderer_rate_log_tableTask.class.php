...
class refactor_tenderer_rate_log_tableTask extends sfBaseTask {
    ...
    protected function configure()
    {
        ...
        $this->namespace           = '';
        $this->name                = '2_0_0_2_refactor_tenderer_rate_log_table';
        $this->briefDescription    = '';
        $this->detailedDescription = <<<EOF
The [2_0_0_2_refactor_tenderer_rate_log_table|INFO] task does things.
Call it with:

  [php symfony 2_0_0_2_refactor_tenderer_rate_log_table|INFO]
EOF;
    }
    ...
    protected function execute($arguments = array(), $options = array())
    {
        ...
        AND table_name = '".strtolower(TenderBillItemRateLogTable::getInstance()->getTableName())."');");
        ...
        if ($isTableExists)
        {
            $stmt = $con->prepare("DROP TABLE ".TenderBillItemRateLogTable::getInstance()->getTableName()." CASCADE");
        ...
        $queries = array(
            ...
            'CREATE UNIQUE INDEX tender_bill_item_rate_logs_unique_idx ON '.TenderBillItemRateLogTable::getInstance()->getTableName().' (tender_company_id, bill_item_id, changes_count, project_revision_id);',
            'CREATE INDEX tender_bill_item_rate_logs_id_idx ON '.TenderBillItemRateLogTable::getInstance()->getTableName().' (id);',
            'CREATE INDEX tender_bill_item_rate_logs_fk_idx ON '.TenderBillItemRateLogTable::getInstance()->getTableName().' (tender_company_id, bill_item_id, project_revision_id);',
            ...
            'ALTER TABLE '.TenderBillItemRateLogTable::getInstance()->getTableName().' ADD CONSTRAINT tender_bill_item_rate_logs_project_revision_id FOREIGN KEY (project_revision_id) REFERENCES '.ProjectRevisionTable::getInstance()->getTableName().'(id) ON DELETE CASCADE NOT DEFERRABLE INITIALLY IMMEDIATE;'
        );
        ...
        return $this->logSection('2_0_0_2_refactor_tenderer_rate_log_table', 'Successfully refactored table '.TenderBillItemRateLogTable::getInstance()->getTableName().'!');
    }
}