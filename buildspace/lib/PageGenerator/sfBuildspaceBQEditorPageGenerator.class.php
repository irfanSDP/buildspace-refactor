...
class sfBuildspaceBQEditorPageGenerator extends sfBuildspaceBQPageGenerator
{
    private $editorProjectInfo;
    private $printLatestBQ;

    public function __construct(EditorProjectInformation $editorProjectInfo, BillElement $element=null, ProjectStructure $bill=null, $printLatestBQ=false)
    {
        $this->editorProjectInfo = $editorProjectInfo;
        $this->printLatestBQ     = $printLatestBQ;
        $bill                    = $element ? $element->ProjectStructure : $bill;
    ...
    protected function queryBillStructure()
    {
        ...
        $nlTable = "LEFT JOIN ".EditorBillItemNotListedTable::getInstance()->getTableName()." inl ON inl.bill_item_id = c.id AND inl.company_id = ".$this->editorProjectInfo->company_id."
        ...
        $projectRevisionSql = $this->printLatestBQ ? " AND c.project_revision_deleted_at IS NULL " : " AND c.project_revision_id = ".$this->editorProjectInfo->printing_revision_id." ";
        ...
        foreach($elements as $element)
        {
            ...
            $stmt = $pdo->prepare("SELECT c.id, c.element_id, c.type, COALESCE(item_info.grand_total, 0) AS grand_total_after_markup, c.bill_ref_element_no, c.bill_ref_page_no, c.bill_ref_char, c.lft, c.rgt, c.root_id, c.level, ".$sqlFieldCond."
                FROM ".BillItemTable::getInstance()->getTableName()." c ".$nlTable."
                LEFT JOIN ".EditorBillItemInfoTable::getInstance()->getTableName()." item_info ON item_info.bill_item_id = c.id AND item_info.company_id = ".$this->editorProjectInfo->company_id."
                LEFT JOIN ".UnitOfMeasurementTable::getInstance()->getTableName()." uom ON c.uom_id = uom.id AND uom.deleted_at IS NULL
                WHERE c.element_id = ".$element['id']." AND c.deleted_at IS NULL ".$projectRevisionSql."
    ...
    protected function getLumpSumPercent()
    {
        $pdo                = $this->pdo;
        $bill               = $this->bill;
        $projectRevisionSql = $this->printLatestBQ ? " AND i.project_revision_deleted_at IS NULL " : " AND i.project_revision_id = ".$this->editorProjectInfo->printing_revision_id." ";

        $stmt = $pdo->prepare("SELECT i.id, c.percentage
            FROM ".EditorBillItemLumpSumPercentageTable::getInstance()->getTableName()." c
            JOIN ".EditorBillItemInfoTable::getInstance()->getTableName()." item_info ON item_info.id = c.bill_item_info_id
            JOIN ".BillItemTable::getInstance()->getTableName()." i ON (item_info.bill_item_id = i.id AND i.type = ".BillItem::TYPE_ITEM_LUMP_SUM_PERCENT.")
            JOIN ".BillElementTable::getInstance()->getTableName()." e ON i.element_id = e.id
            WHERE item_info.company_id = ".$this->editorProjectInfo->company_id." ".$projectRevisionSql."
            AND e.project_structure_id = ".$bill->id." AND i.project_revision_deleted_at IS NULL
    ...
    protected function getItemQuantities()
    {
            ...
            if ( ! empty($implodedItemIds) )
            {
                $sqlFieldCond = '(
                    CASE i.type WHEN '.BillItem::TYPE_ITEM_NOT_LISTED.' THEN
                        CASE COALESCE(SUM(inl_qty.quantity_per_unit * columnSetting.quantity), 0)
                        WHEN 0
                        THEN 0
                        ELSE COALESCE(SUM(inl_qty.quantity_per_unit * columnSetting.quantity), 0)
                        END
                    ELSE
                ...
                $nlTable = "LEFT JOIN ".EditorBillItemInfoTable::getInstance()->getTableName()." item_info ON item_info.bill_item_id = i.id AND item_info.company_id = ".$this->editorProjectInfo->company_id."
                    LEFT JOIN ".EditorBillItemTypeReferenceTable::getInstance()->getTableName()." inl_qty ON inl_qty.bill_item_info_id = item_info.id
                ...
                $stmt = $pdo->prepare("SELECT i.id, ".$sqlFieldCond." FROM ".BillItemTable::getInstance()->getTableName()." i ".$nlTable."
                    WHERE i.id IN (".$implodedItemIds.") ".$projectRevisionSql." AND i.deleted_at IS NULL GROUP BY i.id");
        ...
        else
        {
            $sqlFieldCond = '(
            CASE i.type WHEN '.BillItem::TYPE_ITEM_NOT_LISTED.' THEN
                CASE COALESCE(inl_qty.quantity_per_unit, 0)
                WHEN 0
                THEN 0
                ELSE COALESCE(inl_qty.quantity_per_unit, 0)
                END
            ELSE
            ...
            foreach($bill->BillColumnSettings as $column)
            {
                ...
                $nlTable = "LEFT JOIN ".EditorBillItemInfoTable::getInstance()->getTableName()." item_info ON item_info.bill_item_id = i.id AND item_info.company_id = ".$this->editorProjectInfo->company_id."
                LEFT JOIN ".EditorBillItemTypeReferenceTable::getInstance()->getTableName()." inl_qty ON inl_qty.bill_item_info_id = item_info.id AND inl_qty.bill_column_setting_id = ".$column->id;
                ...
                if ( ! empty($implodedItemIds) )
                {
                    ...
                    WHERE i.id IN (".$implodedItemIds.") ".$projectRevisionSql." AND fc.deleted_at IS NULL AND r.deleted_at IS NULL");
    ...
    protected function getItemIncludeStatus()
    {
        ...
        {
            $nlTable = "LEFT JOIN ".EditorBillItemInfoTable::getInstance()->getTableName()." item_info ON item_info.bill_item_id = r.id AND item_info.company_id = ".$this->editorProjectInfo->company_id."
                LEFT JOIN ".EditorBillItemTypeReferenceTable::getInstance()->getTableName()." inl_qty ON inl_qty.bill_item_info_id = item_info.id AND inl_qty.bill_column_setting_id = ".$column['id'];

            if ( ! empty($implodedItemIds) )
            {
                $stmt = $this->pdo->prepare("SELECT r.bill_item_id, r.include
                    FROM ".BillItemTypeReferenceTable::getInstance()->getTableName()." r ".$nlTable."
                    WHERE r.bill_item_id IN (".$implodedItemIds.") AND r.bill_column_setting_id = ".$column['id']."
                    AND r.deleted_at IS NULL");

                $stmt->execute();

                $includeStatus = $stmt->fetchAll(PDO::FETCH_COLUMN|PDO::FETCH_GROUP);
    ...
    protected function getRatesAfterMarkup()
    {
        ...
        $stmt = $pdo->prepare("SELECT item_info.bill_item_id, COALESCE(fc.final_value, 0) AS value
            FROM ".EditorBillItemFormulatedColumnTable::getInstance()->getTableName()." fc
            JOIN ".EditorBillItemInfoTable::getInstance()->getTableName()." item_info ON fc.relation_id = item_info.id
            JOIN ".BillItemTable::getInstance()->getTableName()." i ON item_info.bill_item_id = i.id
            JOIN ".BillElementTable::getInstance()->getTableName()." e ON i.element_id = e.id
            WHERE item_info.company_id = ".$this->editorProjectInfo->company_id." ".$projectRevisionSql."
            AND fc.column_name = '".BillItem::FORMULATED_COLUMN_RATE."' AND e.project_structure_id = ".$bill->id."
            AND i.deleted_at IS NULL AND e.deleted_at IS NULL ORDER BY i.id");
    ...
    {
        $primeCostRate = EditorBillItemPrimeCostRateTable::getByCompanyAndBillItemId($billItemId, $this->editorProjectInfo->company_id, Doctrine_Core::HYDRATE_ARRAY);
