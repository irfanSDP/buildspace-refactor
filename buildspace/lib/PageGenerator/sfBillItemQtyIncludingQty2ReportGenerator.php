...
class sfBillItemQtyIncludingQty2ReportGenerator extends sfBuildspaceBQMasterFunction {
			...
			*/
			...
			foreach ( $occupiedRows as $key => $occupiedRow )
			{
				...
				if ( $key + 1 == $occupiedRows->count() && $item['type'] != BillItem::TYPE_HEADER && $item['type'] != BillItem::TYPE_HEADER_N && $item['type'] != BillItem::TYPE_NOID )
				{
					...
					$row[self::DIFFERENCE]         = $item[$this->currentColumn->id . '-quantity_per_unit_difference'];
					$row[self::QTY_1]              = $item[$this->currentColumn->id . '-quantity_per_unit-final_value'];
					$row[self::QTY_2]              = $item[$this->currentColumn->id . '-quantity_per_unit_remeasurement-final_value'];
