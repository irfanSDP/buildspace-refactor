...
class sfBuildSpaceBQEstimationPrintAll {
    ...
    public function __construct(sfWebRequest $request, ProjectStructure $projectStructure, $elements = null, $withNotListedItem = false)
    {
        ...
        $this->orientation         = ( $this->numberOfBillColumns > 1 and !$this->printGrandTotalQty ) ? sfBuildspaceBQEstimationPageGenerator::ORIENTATION_LANDSCAPE : sfBuildspaceBQEstimationPageGenerator::ORIENTATION_PORTRAIT;
    ...
    {
        $this->generateSummaryPages($sendToBrowser);

        $this->generateBillItemAndCollectionPages($sendToBrowser);
    }
    ...
    public function generateProjectSummaryPages($project, $withNotListedItem = false, $sendToBrowser = true)
    {
        ...
        $stmt = $this->pdo->prepare("SELECT b.id, b.title AS description, ROUND(COALESCE(SUM(i.grand_total_after_markup) ,0),2) AS total
    ...
    {
        $bqPageGenerator   = new sfBuildspaceEstimationSummaryPageGenerator($this->projectStructure, null, $this->withNotListedItem);
    ...
    public function generateBillItemAndCollectionPages($sendToBrowser)
    {
        ...
        {
            $bqPageGenerator      = new sfBuildspaceBQEstimationPageGenerator($this->projectStructure, $element, $this->withNotListedItem);
            $pages                = $bqPageGenerator->generatePages();
