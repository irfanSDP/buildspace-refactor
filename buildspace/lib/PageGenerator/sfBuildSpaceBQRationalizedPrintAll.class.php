...
class sfBuildSpaceBQRationalizedPrintAll {

    public $pdfGenerator = null;
    public $withNotListedItem;

    public function __construct(sfWebRequest $request, ProjectStructure $projectStructure, $elements = null, $withNotListedItem = false)
    {
        ...
        $this->orientation         = ($this->numberOfBillColumns > 1 and !$projectStructure->BillLayoutSetting->print_grand_total_quantity) ? sfBuildspaceBQRationalizedPageGenerator::ORIENTATION_LANDSCAPE : sfBuildspaceBQRationalizedPageGenerator::ORIENTATION_PORTRAIT;
    ...
    public function generateProjectSummaryPages($project, $withNotListedItem = false, $sendToBrowser = true) 
    {
        ...
        if($this->request->hasParameter('tid'))
        {
            $tenderAlternative = TenderAlternativeTable::getInstance()->find($this->request->getParameter('tid'));
        ...
        $stmt = $this->pdo->prepare("SELECT b.id, b.title AS description, ROUND(COALESCE(SUM(rate.grand_total) ,0),2) AS total
            ...
            ".$tenderAlternativeJoinSql."
            WHERE e.deleted_at IS NULL AND i.deleted_at IS NULL AND i.project_revision_deleted_at IS NULL
            AND b.deleted_at IS NULL AND p.deleted_at IS NULL
            ".$tenderAlternativeWhereSql."
            AND p.id = " . $project['id'] . " ".$whereClause."
            GROUP BY b.id
    ...
    {
        $bqPageGenerator    = new sfBuildspaceRationalizedSummaryPageGenerator($this->projectStructure, null, $this->withNotListedItem);
    ...
    public function generateBillItemAndCollectionPages($sendToBrowser)
    {
        ...
        {
            $bqPageGenerator      = new sfBuildspaceBQRationalizedPageGenerator($this->projectStructure, $element, $this->withNotListedItem);
            $pages                = $bqPageGenerator->generatePages();
                ...
                foreach($pages as $key => $page)
                {
                    ...
                    for($i=1;$i<=$page['item_pages']->count(); $i++)
                    {
                        if($page['item_pages'] instanceof SplFixedArray and $page['item_pages']->offsetExists($i))
                        {
                            ...
                            $layout .= get_partial('printBQ/'.$billItemsLayout, $billItemsLayoutParams);
                    ...
                    foreach($page['collection_pages'] as $pageNo => $collectionPage)
                    {
                        ...
                        $layout .= get_partial('printBQ/'.$collectionPageLayout, $collectionPageParams);
