...
class sfBuildSpaceStockOutItemReportPageGenerator extends sfBuildspaceBQMasterFunction {
	...
	const ROW_BILL_ITEM_TOTAL_COST_WITH_TAX = 5;
	const ROW_BILL_ITEM_UNIT = 7;
	const ROW_BILL_ITEM_DO_QTY = 8;
	const ROW_BILL_ITEM_STOCK_OUT_QTY = 10;
	const ROW_BILL_ITEM_BALANCE_QTY = 13;
	const ROW_BILL_ITEM_TOTAL_COST_WITHOUT_TAX = 14;

	const TOTAL_BILL_ITEM_PROPERTY = 15;
	...
	public function generateBillPages(Array $buildUpItems, &$itemPages, $billTotals)
	{
		...
		foreach ( $buildUpItems as $x => $item )
		{
			...
			foreach ( $occupiedRows as $key => $occupiedRow )
			{
				...
				$row[self::ROW_BILL_ITEM_TOTAL_COST_WITH_TAX]    = null;
				$row[self::ROW_BILL_ITEM_TOTAL_COST_WITHOUT_TAX] = null;
				$row[self::ROW_BILL_ITEM_UNIT]                   = null;
				$row[self::ROW_BILL_ITEM_DO_QTY]                 = null;
				$row[self::ROW_BILL_ITEM_STOCK_OUT_QTY]          = null;

				if ( $key + 1 == $occupiedRows->count() )
				{
					$row[self::ROW_BILL_ITEM_ID]                     = $item['id'];
					$row[self::ROW_BILL_ITEM_TOTAL_COST_WITH_TAX]    = $item['total_cost'];
					$row[self::ROW_BILL_ITEM_TOTAL_COST_WITHOUT_TAX] = $item['total_cost_without_tax'];
					$row[self::ROW_BILL_ITEM_DO_QTY]                 = $item['do_quantity'];
					$row[self::ROW_BILL_ITEM_STOCK_OUT_QTY]          = $item['stock_out_quantity'];
					$row[self::ROW_BILL_ITEM_BALANCE_QTY]            = $item['balance_quantity'];
	...
	public function setBlankRow()
	{
		...
		$blankRow[self::ROW_BILL_ITEM_TOTAL_COST_WITH_TAX] = null;
		$blankRow[self::ROW_BILL_ITEM_UNIT]                = null;
		$blankRow[self::ROW_BILL_ITEM_DO_QTY]              = null;
		$blankRow[self::ROW_BILL_ITEM_STOCK_OUT_QTY]       = null;
