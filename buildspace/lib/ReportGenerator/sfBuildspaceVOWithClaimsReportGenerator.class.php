...
class sfBuildspaceVOWithClaimsReportGenerator extends sfBuildspaceBQMasterFunction
{
    public $pageTitle;
    public $fontSize;
    public $headSettings;
    ...
    public function generatePages()
    {
        ...
        ->andWhere('vo.project_structure_id = ?', $this->project->id)
        ...
        {
            $records[$key]['relation_id']            = $this->project->id;
            ...
            $voTotals['omission']+= ($records[$key]['is_approved'] && abs($records[$key]['total_claim']) > 0) ? $records[$key]['omission'] : 0;
            $voTotals['addition']+= ($records[$key]['is_approved'] && abs($records[$key]['total_claim']) > 0) ? $records[$key]['addition'] : 0;
            $voTotals['nett_omission_addition']+= ($records[$key]['is_approved'] && abs($records[$key]['total_claim']) > 0) ? $records[$key]['nett_omission_addition'] : 0;
            $voTotals['total_claim']+= ($records[$key]['is_approved'] && abs($records[$key]['total_claim']) > 0) ? $records[$key]['total_claim'] : 0;
        ...
        if(count($data))
        {
            $this->generateItemPages($data, 1, array(), $itemPages);
        }
        else
        {
            ...
            if($rowCount <= $maxRows)
            {
                foreach($occupiedRows as $key => $occupiedRow)
                {
                    ...
                    if($key+1 == $occupiedRows->count())
                    {
                        ...
                        $row[self::ROW_NET]          = (array_key_exists('nett_omission_addition', $voItem)) ? $voItem['nett_omission_addition'] : 0;

                        $uptodatePercentage = ( isset($voItem['total_claim']) AND $voItem['total_claim'] != 0 ) ? Utilities::percent($voItem['total_claim'], $row[self::ROW_NET]) : 0;

                        $row[self::ROW_CLAIM_WORKDONE] = array(
                            'up_to_date_percentage' => $uptodatePercentage,
                            'total_claim'           => (array_key_exists('total_claim', $voItem)) ? $voItem['total_claim'] : 0
                        );
                    }
