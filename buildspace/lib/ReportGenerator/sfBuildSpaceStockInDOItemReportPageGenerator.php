...
class sfBuildSpaceStockInDOItemReportPageGenerator extends sfBuildspaceBQMasterFunction {
	...
	const ROW_BILL_ITEM_UNIT = 3;
	const ROW_BILL_ITEM_TYPE = 4;
	const ROW_BILL_ITEM_INVOICE_QTY = 5;
	const ROW_BILL_ITEM_DO_QTY = 6;
	...
	public function generateBillPages(Array $items, &$itemPages, $billTotals)
	{
		...
		foreach ( $items as $x => $item )
		{
			...
			if ( !empty( $items[$x]['remarks'] ) )
			{
				$description = $description . ' - ' . $items[$x]['remarks'];
			}

			$occupiedRows = Utilities::justify(strip_tags($description), $this->MAX_CHARACTERS);
			...
			foreach ( $occupiedRows as $key => $occupiedRow )
			{
				...
				$row[self::ROW_BILL_ITEM_INVOICE_QTY] = null;

				if ( $key + 1 == $occupiedRows->count() )
				{
					$row[self::ROW_BILL_ITEM_ID]          = $item['id'];
					$row[self::ROW_BILL_ITEM_INVOICE_QTY] = $item['invoiceQuantity'];
	...
	public function setBlankRow()
	{
		...
		$blankRow[self::ROW_BILL_ITEM_INVOICE_QTY] = null;
