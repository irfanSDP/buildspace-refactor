...
class sfBuildspaceExportScheduleOfRateBillXML extends sfBuildspaceXMLGenerator
{
    public $xml;
    ...
    const TAG_SCHEDULE_OF_RATE_BILL = "SCHEDULE_OF_RATE_BILL";
    const TAG_ITEM = "item";
    ...
    {
        parent::create(self::TAG_SCHEDULE_OF_RATE_BILL,
            array(
    ...
    public function processBillLayoutSetting($billLayoutSetting)
    {
        ...
        $billPhrase = ( array_key_exists('ScheduleOfRateBillLayoutPhrase', $billLayoutSetting) ) ? true : false;

        if ($billPhrase)
        {
            $billPhrase = $billLayoutSetting['ScheduleOfRateBillLayoutPhrase'];

            unset( $billLayoutSetting['ScheduleOfRateBillLayoutPhrase'] );
        }
        else
        {
            $billPhrase = false;
        ...
        $headSetting = ( array_key_exists('ScheduleOfRateBillLayoutHeadSettings', $billLayoutSetting) ) ? true : false;

        if ($headSetting && count($headSetting))
        {
            $headSetting = $billLayoutSetting['ScheduleOfRateBillLayoutHeadSettings'];

            unset( $billLayoutSetting['ScheduleOfRateBillLayoutHeadSettings'] );
        }
        else
        {
            $headSetting = false;
