...
class sfBillItemBuildUpRateReportGenerator extends sfBuildspaceBQMasterFunction {

    use sfBuildspaceReportPageFormat;

    protected $bill                 = array();
    protected $resourceTrades       = array();
    protected $buildUpItems         = array();
    ...
    public function generatePages()
    {
        ...
        else
        {
            ...
            foreach ( $this->resourceTrades as $resourceTrade )
            {
                ...
                if ( ! isset($this->buildUpItems[$resourceTrade['id']]) )
                {
                    continue;
                }

                $this->generateBillPages($this->buildUpItems[$resourceTrade['id']], $ancestors, $itemPages, array());
            }
        }

        $pages = SplFixedArray::fromArray($itemPages);

        unset($itemPages, $this->resourceTrades, $this->buildUpItems);
    ...
    public function setBuildUpItems($buildUpItems)
    {
        $this->buildUpItems = $buildUpItems;
    }
