 ...
 */
class ScheduleOfRateForm extends BaseScheduleOfRateForm
{
    ...
    {
        $query = DoctrineQuery::create()->select('r.id, r.name')->from('ScheduleOfRate r');
        $query->where('LOWER(r.name) = ?', strtolower($values['name']));
        $query->andWhere('r.deleted_at IS NULL');

        if($query->count() > 0)
        {
            $sfError = new sfValidatorError($validator, 'There is already another schedule of rate with that name.');
            ...
            {
                $scheduleOfRate = $query->fetchOne();
                if($this->object->id != $scheduleOfRate->id)
                {
                    throw new sfValidatorErrorSchema($validator, array('name' => $sfError));
                }
            }
