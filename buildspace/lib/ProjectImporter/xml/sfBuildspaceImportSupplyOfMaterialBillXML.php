...
class sfBuildspaceImportSupplyOfMaterialBillXML extends sfBuildspaceXMLParser
{
    public $rootProject;
    ...
    public function extractData()
    {
        ...
        $this->supplyOfMaterial = ( $xmlData->{sfBuildspaceExportBillXML::TAG_BILLSETTING}->count() > 0 ) ? $xmlData->{sfBuildspaceExportBillXML::TAG_BILLSETTING}->children() : false;

        $this->layoutSetting = ( $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_LAYOUTSETTING}->count() > 0 ) ? $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_LAYOUTSETTING}->children() : false;

        $this->units = ( $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_UNITOFMEASUREMENT}->count() > 0 ) ? $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_UNITOFMEASUREMENT}->children() : false;

        $this->elements = ( $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_ELEMENTS}->count() > 0 ) ? $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_ELEMENTS}->children() : false;

        $this->items = ( $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_ITEMS}->count() > 0 ) ? $xmlData->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_ITEMS}->children() : false;
    ...
    {
        if ($this->supplyOfMaterial->id)
        {
            unset( $this->supplyOfMaterial->id );
        }

        $this->supplyOfMaterial->project_structure_id = $this->breakdownIds[(int) $this->supplyOfMaterial->project_structure_id];
        $this->supplyOfMaterial->description          = (string) $this->supplyOfMaterial->description;
        $this->supplyOfMaterial->created_at           = 'NOW()';
        $this->supplyOfMaterial->updated_at           = 'NOW()';
        $this->supplyOfMaterial->created_by           = $this->userId;
        $this->supplyOfMaterial->updated_by           = $this->userId;

        unset( $this->supplyOfMaterial->deleted_at );

        $dataAndStructure = parent::generateArrayOfSingleData($this->supplyOfMaterial, true);

        $stmt = new sfImportStatementGenerator();

        $stmt->createInsert(SupplyOfMaterialTable::getInstance()->getTableName(), $dataAndStructure['structure']);

        $stmt->addRecord($dataAndStructure['data']);

        $stmt->save();

        unset( $this->supplyOfMaterial, $stmt );
    }

    public function processLayoutSetting()
    {
        $billPhrase = ( array_key_exists(sfBuildspaceExportSupplyOfMaterialBillXML::TAG_PHRASE,
            $this->layoutSetting) ) ? true : false;

        if ($billPhrase)
        {
            $billPhrase = $this->layoutSetting->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_PHRASE}->children();
        }
        else
        {
            $billPhrase = false;
        ...
        $headSetting = ( array_key_exists(sfBuildspaceExportSupplyOfMaterialBillXML::TAG_HEADSETTING,
            $this->layoutSetting) ) ? true : false;

        if ($headSetting)
        {
            $headSetting = $this->layoutSetting->{sfBuildspaceExportSupplyOfMaterialBillXML::TAG_HEADSETTING}->children();
        }
        else
        {
            $headSetting = false;
        ...
        $stmt->createInsert(
            SupplyOfMaterialLayoutSettingTable::getInstance()->getTableName(),
        ...
        if ($billPhrase)
        {
            $billPhrase->som_layout_setting_id = $returningId;
            ...
            $stmt->createInsert(
                SupplyOfMaterialLayoutPhraseSettingTable::getInstance()->getTableName(),
                $dataAndStructure['structure']
            );
        ...
        if ($headSetting and is_object($headSetting))
        {
            foreach ($headSetting as $head)
            {
                $item = $head->children();

                $item->som_layout_setting_id = $returningId;
                $item->created_at            = 'NOW()';
                $item->updated_at            = 'NOW()';
                ...
                $stmt->createInsert(
                    SupplyOfMaterialLayoutHeadSettingTable::getInstance()->getTableName(),
                    $dataAndStructure['structure']
                );
    ...
    public function processItems()
    {
        ...
        foreach ($this->items as $item)
        {
            ...
            $item->supply_rate = (string) $item->supply_rate;
            ...
            {
                //Update Root Id
                $stmt->updateRecord(SupplyOfMaterialItemTable::getInstance()->getTableName(),
                    array(
                        'id' => $newItemId
                    ),
                    array(
