 ...
 */
class ScheduleOfRateBuildUpRateItem extends BaseScheduleOfRateBuildUpRateItem
{
    ...
    {
        return ScheduleOfRateBuildUpRateItemTable::getFormulatedColumnByRelationIdAndColumnName($this->id, $columnName, $hydrate);
    }

    public function calculateTotal()
    {
        return ScheduleOfRateBuildUpRateItemTable::calculateTotalById($this->id);
    }

    public function calculateLineTotal()
    {
        return ScheduleOfRateBuildUpRateItemTable::calculateLineTotalById($this->id);
    }
        ...
        try
        {
            ...
            foreach($this->FormulatedColumns as $formulatedColumn)
            {
                ...
                if(is_array($nodes))
                {
                    ...
                    {
                        $referencedNode = Doctrine_Core::getTable('ScheduleOfRateBuildUpRateFormulatedColumn')->find($node['node_from']);
        ...
        if($this->deleted_at == NULL)
        {
            ...
            if($this->resource_item_library_id > 0)
            {
                    ...
                    $this->uom_id != $this->ResourceItemLibrary->uom_id)
                {
                    ...
                    Doctrine_Manager::getInstance()->getCurrentConnection()
                        ->fetchAssoc("UPDATE ".Doctrine_Core::getTable('ScheduleOfRateBuildUpRateFormulatedColumn')->getTableName()." SET
                        linked = FALSE
                        WHERE relation_id = ".$this->id);
                }
            }

            ScheduleOfRateItemTable::updateTimestamp($this->ScheduleOfRateItem);
        }
    }
    ...
    public function copyFormulatedColumnsFromItem(ScheduleOfRateBuildUpRateItem $item)
    {
    ...
    private function updatePriority($priority, $excludeId)
    {
        $query = DoctrineQuery::create()->select('i.id')
            ...
            ->where('i.schedule_of_rate_item_id = ?', $this->schedule_of_rate_item_id)
