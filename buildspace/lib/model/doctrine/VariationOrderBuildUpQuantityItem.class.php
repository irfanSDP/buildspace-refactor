...
/**
 * VariationOrderBuildUpQuantityItem
 *
 ...
 */
class VariationOrderBuildUpQuantityItem extends BaseVariationOrderBuildUpQuantityItem {

    const TYPE_ADDITIONAL_QTY = 1;
    const TYPE_OMISSION_QTY = 2;

    const TYPE_ADDITIONAL_TEXT = 'Addition';
    const TYPE_OMISSION_TEXT = 'Omission';
    ...
    private static $buildUpQtyTypes = array(self::TYPE_ADDITIONAL_QTY, self::TYPE_OMISSION_QTY);

    public static function getBuildUpQtyTypes()
    {
        return self::$buildUpQtyTypes;
    }

    public function getSignText()
    {
        return VariationOrderBuildUpQuantityItemTable::getSignTextBySign($this->sign);
    }

    public function getFormulatedColumnByName($columnName, $hydrate = null)
    {
        return VariationOrderBuildUpQuantityItemTable::getFormulatedColumnByRelationIdAndColumnName($this->id, $columnName, $hydrate);
    }

    public function calculateTotal()
    {
        $total                 = 0;
        $formulatedColumnNames = $this->getTable()->getFormulatedColumnNames($this->VariationOrderItem->UnitOfMeasurement);
        ...
        try
        {
            ...
            foreach ( $this->FormulatedColumns as $formulatedColumn )
            {
                ...
                if ( is_array($nodes) )
                {
                    ...
                    {
                        $referencedNode = Doctrine_Core::getTable('VariationOrderBuildUpQuantityFormulatedColumn')->find($node['node_from']);
            ...
            if ( count($formulatedColumnIds) > 0 )
            {
                ...
                Doctrine_Query::create()
                    ->delete('VariationOrderBuildUpQuantityEdge e')
    ...
    public function copyFormulatedColumnsFromItem(VariationOrderBuildUpQuantityItem $item)
    {
        $formulatedColumnNames = VariationOrderBuildUpQuantityItemTable::getFormulatedColumnNames($this->VariationOrderItem->UnitOfMeasurement);
        $edgeTable             = Doctrine_Core::getTable('VariationOrderBuildUpQuantityEdge');
    ...
    private function updatePriority($priority, $excludeId)
    {
        $records = DoctrineQuery::create()->select('i.id')
            ...
            ->where('i.variation_order_item_id = ?', $this->variation_order_item_id)
