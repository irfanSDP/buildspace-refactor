 ...
 */
class ScheduleOfRateBillItem extends BaseScheduleOfRateBillItem
{
    const TYPE_HEADER = 1;
    ...
    {
        ScheduleOfRateBillItemTable::updateRootPriority($priority, $this->element_id, $excludeId);
    }
    ...
    {
        if($this->type == self::TYPE_HEADER or $this->type == self::TYPE_HEADER_N)
        {
            Doctrine_Query::create()
                ->delete('ScheduleOfRateBillItem i')
    ...
    public function moveTo(ScheduleOfRateBillItem $targetItem, $lastPosition=false)
    {
        ...
        try
        {
            ...
            {
                Doctrine_Query::create()->update('ScheduleOfRateBillItem i')
    ...
    public function copyTo(ScheduleOfRateBillItem $targetItem, $lastPosition=false)
    {
        $con = $this->getTable()->getConnection();
        ...
        try
        {
            ...
            if($this->node->hasChildren())
            {
                $children = $this->getTable()
                    ->createQuery('i')
                    ->select('i.id, i.description, i.estimation_rate, i.contractor_rate, i.difference, i.note, i.uom_id, i.element_id, i.type, i.level')
                    ->where('i.root_id = ?', $this->root_id)
            ...
            $cloneItem                  = new ScheduleOfRateBillItem();
            ...
            $cloneItem->estimation_rate = $this->estimation_rate;
            $cloneItem->contractor_rate = $this->contractor_rate;
    ...
    private function _loadNestedSetData(Array $nestedSetData, ScheduleOfRateBillItem $containerScope, ScheduleOfRateBillItem $parent)
    {
        ...
        foreach($nestedSetData as $nestedSet)
        {
            ...
            $obj = new ScheduleOfRateBillItem();
    ...
    public function indent()
    {
        ...
        if($this->node->isRoot() and $previousRoot and ($previousRoot->type == self::TYPE_HEADER or $previousRoot->type == self::TYPE_HEADER_N))
        {
        ...
        else if(!$this->node->isRoot() and $this->node->hasPrevSibling() and ($this->node->getPrevSibling()->type == self::TYPE_HEADER or $this->node->getPrevSibling()->type == self::TYPE_HEADER_N))
        {
    ...
    {
        if(!$this->node->isRoot() and (($this->type == self::TYPE_HEADER or $this->type == self::TYPE_HEADER_N) or ($this->type != self::TYPE_HEADER and $this->type != self::TYPE_HEADER_N and !$this->node->hasNextSibling())))
        {
