 ...
 */
class ScheduleOfRateBillElement extends BaseScheduleOfRateBillElement
{
    public function getTotalEstimationRates()
    {
        return ScheduleOfRateBillElementTable::getTotalEstimationRateByElementId($this->id);
    }

    public function copyTo(ScheduleOfRateBillElement $targetItem, $lastPosition=false)
    {
        ...
        try
        {
            ...
            $roots = DoctrineQuery::create()
                ->select('i.id, i.description, i.note, i.type, i.estimation_rate, i.uom_id, i.level, i.lft, i.rgt, i.priority')
            ...
            $rootSql = "INSERT INTO ".ScheduleOfRateBillItemTable::getInstance()->getTableName()." (element_id, description, note, type, estimation_rate, uom_id, level, lft, rgt, priority, created_at, updated_at, created_by, updated_by)
                    VALUES (".$cloneElement->id.", :description, :note, :type, :estimation_rate, :uom_id, :level, :lft, :rgt, :priority, NOW(), NOW(), ".$userId.", ".$userId.") RETURNING id";

            $itemSql = "INSERT INTO ".ScheduleOfRateBillItemTable::getInstance()->getTableName()." (element_id, description, note, type, estimation_rate, uom_id, root_id, level, lft, rgt, priority, created_at, updated_at, created_by, updated_by)
                    VALUES (".$cloneElement->id.", :description, :note, :type, :estimation_rate, :uom_id, :root_id, :level, :lft, :rgt, :priority, NOW(), NOW(), ".$userId.", ".$userId.") RETURNING id";
            ...
            foreach($roots as $root)
            {
                $rootStmt->execute(array(
                    ...
                    'estimation_rate' => $root['estimation_rate'],
                ));
                ...
                $items = DoctrineQuery::create()
                    ->select('i.id, i.description, i.note, i.type, i.estimation_rate, i.uom_id, i.element_id, i.level, i.lft, i.rgt, i.priority')
                    ->from('ScheduleOfRateBillItem i')
                ...
                foreach($items as $item)
                {
                    $itemStmt->execute(array(
                        ...
                        'estimation_rate' => $item['estimation_rate'],
                    ));
                }
            }
    ...
    public function delete(Doctrine_Connection $conn = null)
    {
        ...
        try
        {
            ...
            Doctrine_Manager::getInstance()->getCurrentConnection()
                ->fetchAssoc("UPDATE ".ScheduleOfRateBillItemTable::getInstance()->getTableName()." SET deleted_at = NOW() WHERE element_id = ".$this->id);

            Doctrine_Query::create()
                ->delete('ScheduleOfRateBillElement e')
                ->where('e.id = ?', $this->id)
                ->execute();
