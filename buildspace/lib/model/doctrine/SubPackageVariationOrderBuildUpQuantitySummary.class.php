...
/**
 * SubPackageVariationOrderBuildUpQuantitySummary
 * 
 ...
 */
class SubPackageVariationOrderBuildUpQuantitySummary extends BaseSubPackageVariationOrderBuildUpQuantitySummary
{
    ...
    public function calculateTotalQuantity()
    {
        $result = DoctrineQuery::create()->select('COALESCE(SUM(b.total), 0) as total')
            ->from('SubPackageVariationOrderBuildUpQuantityItem b')
            ->where('b.sub_package_variation_order_item_id = ?', $this->sub_package_variation_order_item_id)
    ...
    {
        $linkedTotalQuantity = $this->SubPackageVariationOrderItem->getScheduleOfQuantitiesTotalAmount($this->type);
    ...
    public function save(Doctrine_Connection $conn = null)
    {
        ...
        if($this->deleted_at == NULL and !$isNew)
        {
            ...
            if($this->type == SubPackageVariationOrderBuildUpQuantityItem::TYPE_ADDITIONAL_QTY)
            {
                $this->SubPackageVariationOrderItem->addition_quantity = $this->final_quantity;
                $this->SubPackageVariationOrderItem->has_addition_build_up_quantity = true;
            }
            else
            {
                $this->SubPackageVariationOrderItem->omission_quantity = $this->final_quantity;
                $this->SubPackageVariationOrderItem->has_omission_build_up_quantity = true;
            }

            $this->SubPackageVariationOrderItem->save($conn);
        }
    }
}
