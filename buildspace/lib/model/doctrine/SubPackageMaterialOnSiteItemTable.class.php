...
/**
 * SubPackageMaterialOnSiteItemTable
 *
 * This class has been auto-generated by the Doctrine ORM Framework
 */
class SubPackageMaterialOnSiteItemTable extends Doctrine_Table {
	 ...
	 *
	 * @return object SubPackageMaterialOnSiteItemTable
	 */
	public static function getInstance()
	{
		return Doctrine_Core::getTable('SubPackageMaterialOnSiteItem');
	}

	public static function createItemFromLastRow($previousItem, $materialOnSiteId, $fieldName = null, $fieldValue = null)
	{
		$item = new SubPackageMaterialOnSiteItem();

		$item->sp_material_on_site_id = $materialOnSiteId;
	...
	public static function createItem(SubPackageMaterialOnSiteItem $nextItem, $materialOnSiteId)
	{
		$item                         = new SubPackageMaterialOnSiteItem();
		$item->sp_material_on_site_id = $materialOnSiteId;
		$item->type                   = ResourceItem::TYPE_WORK_ITEM;
	...
	public static function importStockOutItems(Doctrine_Connection $conn, $targetItemId, SubPackageMaterialOnSite $mos, Array $resourceItemIds)
	{
		$resourceTradeId = - 1;
		...
		if ( count($resourceItemIds) > 0 && array_search("", $resourceItemIds) === false )
		{
			...
			$newDOQuantities = StockInDeliveryOrderItemQuantityTable::getOverAllItemQuantitiesFilterByResourceItemIdByProject($mos->SubPackage->ProjectStructure, $resourceItemIds);
			$newSOQuantities = StockOutUsedQuantityItemQuantityTable::getOverAllItemQuantitiesFilterByResourceItemIdByProject($mos->SubPackage->ProjectStructure, $resourceItemIds);
			...
			$invoiceItemRemarks = StockInInvoiceItemTable::getInvoiceRemarks($mos->SubPackage->ProjectStructure, $resourceTrade);
			...
			if ( $items and !empty( $items ) )
			{
				...
				foreach ( $items as $item )
				{
					...
					$item['sp_material_on_site_id']  = $mos->id;
				...
				foreach ( $trees as $key => $tree )
				{
					if ( !$targetItem )
					{
						SubPackageMaterialOnSiteItemTable::updateRootPriority($priority, $mos);
				...
				(description, type, uom_id, sp_material_on_site_id, import_resource_item_id, priority, lft, rgt,
				...
				foreach ( $returnIds as $key => $returnId )
				{
					...
					*/
					if ( $shiftExistingRLValues && $targetItem )
					{
						...
						AND root_id = " . $targetItem->root_id . " AND sp_material_on_site_id = " . $mos->id . "
						...
						AND sp_material_on_site_id = " . $mos->id . " AND deleted_at IS NULL");
				...
				END WHERE root_id IS NULL AND sp_material_on_site_id = " . $mos->id . "
				AND deleted_at IS NULL");
				...
				{
					$stmt = $pdo->prepare("INSERT INTO " . self::getInstance()->getTableName() . "
					(description, type, uom_id, sp_material_on_site_id, import_resource_item_id, root_id, priority,
	...
	public static function importMaterialOnSiteItems(Doctrine_Connection $conn, $targetItemId, SubPackageMaterialOnSite $mos, Array $mosItemIds)
	{
	...
	public static function updateRootPriority($priority, SubPackageMaterialOnSite $mos, $excludeId = null)
	{
		$pdo = self::getInstance()->getConnection()->getDbh();

		$sqlUpdateRoot = "UPDATE " . self::getInstance()->getTableName() . " SET priority = priority + 1
		WHERE sp_material_on_site_id =" . $mos->id . " AND priority >=" . $priority . " AND id = root_id";
		...
		WHERE i.root_id = r.id AND i.id <> r.id AND i.priority <> r.priority AND i.sp_material_on_site_id = " . $mos->id . "
		AND i.deleted_at IS NULL");

		$stmt->execute();
	}

	private static function getTargetedResourceItemPriority(SubPackageMaterialOnSite $mos, $targetItem)
	{
		if ( !$targetItem )
		{
			...
			$rootItem = DoctrineQuery::create()->select('i.id')
				...
				->where('i.sp_material_on_site_id = ?', array( $mos->id ))
	...
	public static function materialOnSiteLoadNestedSetData(Array $nestedSetData, Array $parent, $rootId, $parentRgtVal, $itemCount)
	{
		...
		foreach ( $nestedSetData as $nestedSet )
		{
			...
			self::getInstance()->importedMaterialOnSiteItemsChildren[] = "(
				...
				" . $nestedSet['type'] . ",
				" . $uomId . ",
				" . $nestedSet['sp_material_on_site_id'] . ",
	...
	public static function getItemsByIds(SubPackage $subPackage, $itemIds)
	{
		$items   = array();
		...
		$stmt = $pdo->prepare("SELECT DISTINCT p.id, p.description, p.type, p.delivered_qty,
			...
			uom.symbol AS uom_symbol, mos.id as sp_material_on_site_id, mos.description as material_on_site_name, mos.priority
			FROM " . SubPackageMaterialOnSiteItemTable::getInstance()->getTableName() . " i
			JOIN " . SubPackageMaterialOnSiteItemTable::getInstance()->getTableName() . " p
			ON (i.lft BETWEEN p.lft AND p.rgt AND p.deleted_at IS NULL)
			LEFT JOIN " . SubPackageMaterialOnSiteTable::getInstance()->getTableName() . " mos ON mos.id = p.sp_material_on_site_id
			LEFT JOIN " . UnitOfMeasurementTable::getInstance()->getTableName() . " uom ON p.uom_id = uom.id AND uom.deleted_at IS NULL
			WHERE mos.sub_package_id = " . $subPackage->id . " AND i.id IN (" . implode(',', $itemIds) . ")
