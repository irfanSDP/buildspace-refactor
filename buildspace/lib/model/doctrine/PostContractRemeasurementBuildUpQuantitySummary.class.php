...
/**
 * PostContractRemeasurementBuildUpQuantitySummary
 *
 ...
 */
class PostContractRemeasurementBuildUpQuantitySummary extends BasePostContractRemeasurementBuildUpQuantitySummary
{
    ...
    {
        $query = DoctrineQuery::create()->select('COALESCE(SUM(b.total), 0) as total')
            ->from('PostContractRemeasurementBuildUpQuantityItem b')
            ->where('b.post_contract_bill_item_rate_id = ?', $this->post_contract_bill_item_rate_id)
    ...
    public function save(Doctrine_Connection $conn = null)
    {
        ...
        if($this->deleted_at == NULL and !$isNew)
        {
            ...
            $billItemTypeRef = PostContractRemeasurementItemTypeReferenceTable::getByItemIdAndColumnId($this->post_contract_bill_item_rate_id, $this->bill_column_setting_id);

            if (!$billItemTypeRef or $billItemTypeRef->isNew())
            {
                $billItemTypeRef                                  = new PostContractRemeasurementItemTypeReference();
                $billItemTypeRef->post_contract_bill_item_rate_id = $this->post_contract_bill_item_rate_id;
            ...
            PostContractRemeasurementClaimTable::updateBuildUpQty($this, $conn);
        }
    }
}
