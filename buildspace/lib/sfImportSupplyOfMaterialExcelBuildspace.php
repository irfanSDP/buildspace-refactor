...
class sfImportSupplyOfMaterialExcelBuildspace
{
    ...
    public function process()
    {
        try
        {
            ...
            {
                $this->buildSpaceSheets[$idx] = new sfImportSupplyOfMaterialExcelBuildSpaceSheet($sheet);
            }
        } catch (Exception $e)
        {
            throw $e;
    ...
    public function saveAsNewBill(
        ...
        $somBill = $this->createSupplyOfMaterialBill($billInfo, $projectStructure, $conn);
        ...
        SupplyOfMaterialLayoutSettingTable::cloneExistingPrintingLayoutSettingsForBill($somBill, $defaultSetting,
            $billPhraseSetting, $headSettings);

        $this->saveIntoBill($somBill->ProjectStructure, $selectElementIds, $withRate, $conn);
    }

    private function createSupplyOfMaterialBill(
        Array $billInfo,
        ProjectStructure $projectStructure,
        Doctrine_Connection $conn = null
    ) {
        ...
        $stmt = $pdo->prepare("INSERT INTO " . SupplyOfMaterialTable::getInstance()->getTableName() . "
        ...
        $supplyOfMaterialReturnedId = $stmt->fetch(PDO::FETCH_COLUMN, 0);

        return Doctrine_Core::getTable('SupplyOfMaterial')->find($supplyOfMaterialReturnedId);
    }
    ...
    private function reassignRootIds(Array $roots, Array $importedItemToItemIds, Doctrine_Connection $conn = null)
    {
        ...
        if ($rootIds && $itemIds)
        {
            $stmt = $pdo->prepare("UPDATE " . SupplyOfMaterialItemTable::getInstance()->getTableName() . " SET root_id = cast(virtual_table.root_id AS int)
            FROM
            (SELECT UNNEST(ARRAY[" . implode(",", $itemIds) . "]) AS id,
                UNNEST(ARRAY['" . implode("','", $rootIds) . "']) AS root_id
            ) AS virtual_table WHERE " . SupplyOfMaterialItemTable::getInstance()->getTableName() . ".id = virtual_table.id
            AND " . SupplyOfMaterialItemTable::getInstance()->getTableName() . ".root_id IS NULL");
...
class sfImportSupplyOfMaterialExcelBuildSpaceSheet
{
