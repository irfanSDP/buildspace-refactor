...
class sfRemeasurementElementReportGenerator extends sfBuildspaceBQMasterFunction {
	...
	public function generatePages()
	{
		...
		$this->generateElementsPage($this->data, 1, array(), $itemPages);
	...
	public function generateElementsPage(Array $elements, $pageCount, $ancestors, &$itemPages)
	{
		...
		foreach ( $elements as $x => $item )
		{
			...
			if ( $rowCount >= $maxRows )
			{
				...
				$pageCount ++;
				$this->generateElementsPage($elements, $pageCount, $ancestors, $itemPages, true);
				break;
			}
