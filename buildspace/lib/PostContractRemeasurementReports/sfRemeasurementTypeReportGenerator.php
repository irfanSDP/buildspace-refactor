...
class sfRemeasurementTypeReportGenerator extends sfBuildspaceBQMasterFunction {
	...
	public function __construct(PostContract $postContract, ProjectStructure $bill, $data, $descriptionFormat = self::DESC_FORMAT_FULL_LINE)
	{
	...
	public function generatePages()
	{
		...
		$this->generateBillTypesPages($this->data, 1, array(), $itemPages);
	...
	public function generateBillTypesPages(Array $types, $pageCount, $ancestors, &$itemPages)
	{
		...
		foreach ( $types as $x => $item )
		{
			$occupiedRows = Utilities::justify($types[$x]['name'], $this->MAX_CHARACTERS);
			...
			if ( $rowCount >= $maxRows )
			{
				...
				$pageCount ++;
				$this->generateBillTypesPages($types, $pageCount, $ancestors, $itemPages, true);
				break;
			}
