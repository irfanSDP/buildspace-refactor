...
class sfMaterialOnSiteItemExcelGenerator extends sfBuildspaceExcelReportGenerator {
	...
	public function createHeader($new = false)
	{
		...
		$this->activeSheet->setCellValue($this->colDeliveredQty . $row, 'Delivered Qty');
		$this->activeSheet->setCellValue($this->colUsedQty . $row, 'Used Qty');
		$this->activeSheet->setCellValue($this->colBalance . $row, 'Balance');
		...
		$this->activeSheet->getColumnDimension($this->colDeliveredQty)->setWidth(12);
		$this->activeSheet->getColumnDimension($this->colUsedQty)->setWidth(12);
		$this->activeSheet->getColumnDimension($this->colBalance)->setWidth(12);
	...
	public function setResourceItem($item, $description, $char, $itemType)
	{
		...
		$this->activeSheet->setCellValue($this->colDeliveredQty . $row, $item[sfMaterialOnSiteNormalItemReportGenerator::ROW_BILL_ITEM_DELIVERED_QTY]);
		$this->activeSheet->getStyle($this->colDeliveredQty . $row)->applyFromArray($this->getRateStyling($item));
		$this->activeSheet->getStyle($this->colDeliveredQty . $row)->getNumberFormat()->applyFromArray($this->getNumberFormatStandard());

		$this->activeSheet->setCellValue($this->colUsedQty . $row, $item[sfMaterialOnSiteNormalItemReportGenerator::ROW_BILL_ITEM_USED_QTY]);
		$this->activeSheet->getStyle($this->colUsedQty . $row)->applyFromArray($this->getRateStyling($item));
		$this->activeSheet->getStyle($this->colUsedQty . $row)->getNumberFormat()->applyFromArray($this->getNumberFormatStandard());

		$this->activeSheet->setCellValue($this->colBalance . $row, $item[sfMaterialOnSiteNormalItemReportGenerator::ROW_BILL_ITEM_BALANCE_QTY]);
		$this->activeSheet->getStyle($this->colBalance . $row)->applyFromArray($this->getRateStyling($item));
		$this->activeSheet->getStyle($this->colBalance . $row)->getNumberFormat()->applyFromArray($this->getNumberFormatStandard());
	...
	public function setBillHeader($billHeader = null, $topLeftTitle = null, $subTitle = null)
	{
		...
		if ( !empty( $this->mosPrintSettings['site_belonging_address'] ) or !empty( $this->mosPrintSettings['original_finished_date'] ) )
		{
			...
			$this->activeSheet->setCellValue($this->colUsedQty . $this->currentRow, Utilities::truncateString($this->mosPrintSettings['original_finished_date'], 100));
			$this->activeSheet->mergeCells($this->colUsedQty . $this->currentRow . ':' . $this->colBalance . $this->currentRow);
			$this->activeSheet->getStyle($this->colUsedQty . $this->currentRow . ':' . $this->colBalance . $this->currentRow)->applyFromArray($this->getHeaderStyle());
		}
		...
		if ( !empty( $this->mosPrintSettings['contract_duration'] ) or !empty( $this->mosPrintSettings['contract_original_amount'] ) )
		{
			...
			$this->activeSheet->setCellValue($this->colUsedQty . $this->currentRow, Utilities::truncateString($this->mosPrintSettings['contract_original_amount'], 100));
			$this->activeSheet->mergeCells($this->colUsedQty . $this->currentRow . ':' . $this->colBalance . $this->currentRow);
			$this->activeSheet->getStyle($this->colUsedQty . $this->currentRow . ':' . $this->colBalance . $this->currentRow)->applyFromArray($this->getHeaderStyle());
		}
		...
		if ( !empty( $this->mosPrintSettings['payment_revision_no'] ) or !empty( $this->mosPrintSettings['evaluation_date'] ) )
		{
			...
			$this->activeSheet->setCellValue($this->colUsedQty . $this->currentRow, Utilities::truncateString($this->mosPrintSettings['evaluation_date'], 100));
			$this->activeSheet->mergeCells($this->colUsedQty . $this->currentRow . ':' . $this->colBalance . $this->currentRow);
			$this->activeSheet->getStyle($this->colUsedQty . $this->currentRow . ':' . $this->colBalance . $this->currentRow)->applyFromArray($this->getHeaderStyle());
		}
