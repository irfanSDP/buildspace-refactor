...
class sfBQLibraryItemUnitExcelExporterGenerator extends sfBuildspaceExcelReportGenerator {
	...
	private $withRate = false;

	public function __construct($printingPageTitle, $printSettings, $withRate = false)
	{
	...
	public function process($pages, $lock = false, $header, $subTitle, $topLeftTitle, $withoutCents, $totalPage)
	{
		...
		foreach ( $pages as $i => $item )
		{
			...
			$itemType = $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_TYPE];
			...
			switch ($itemType)
			{
				...
				case ScheduleOfRateItem::TYPE_HEADER:
					$description .= $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_DESCRIPTION] . "\n";
					$prevItemType  = $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_TYPE];
					$prevItemLevel = $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_LEVEL];

					break;

				default:
					$description .= $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_DESCRIPTION] . "\n";
					$char .= $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_ROW_IDX];

					if ( $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_ID] )
					{
						$this->newItem();

						$this->setBQItem($item, $description, $char);
	...
	public function setBQItem($item, $description, $char)
	{
		$row         = $this->currentRow;
		$factorValue = null;
		$totalValue  = null;
		...
		$this->activeSheet->setCellValue($this->colUnit . $row, $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_UNIT]);
		$this->activeSheet->getStyle($this->colUnit . $row)->applyFromArray($this->getUnitStyle());

		if ( $this->withRate )
		{
			$rate = empty($item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_RATE]) ? null : $item[sfBQLibraryItemReportGenerator::ROW_BILL_ITEM_RATE];
