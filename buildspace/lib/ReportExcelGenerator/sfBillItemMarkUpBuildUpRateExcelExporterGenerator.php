...
class sfBillItemMarkUpBuildUpRateExcelExporterGenerator extends sfBuildspaceExcelReportGenerator {
	...
	public function process($pages, $lock = false, $header, $subTitle, $topLeftTitle, $withoutCents, $totalPage)
	{
		...
		foreach ( $pages as $i => $page )
		{
			...
			foreach ( $page as $item )
			{
				...
				$itemType = $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_TYPE];

				switch ($itemType)
				{
					case ResourceItem::TYPE_HEADER:
						$description = $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_DESCRIPTION] . "\n";
					...
					default:
						$description .= $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_DESCRIPTION] . "\n";
						$char .= $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_ROW_IDX];
		...
		foreach ( $this->billItemInfo as $item )
		{
			...
			switch ($itemType)
			{
				...
				default:
					...
					if ( $item[0] )
					{
						$this->setBillItemDescriptionRow(strip_tags($description), $char);
	...
	public function setBillItemDescriptionRow($description, $char)
	{
		$row         = $this->currentRow;
		...
		$this->activeSheet->setCellValue($this->colTotal . $row, $rate);
	...
	public function setBuildUpItem($item, $description, $char, $itemType)
	{
		...
		$this->activeSheet->setCellValue($this->colNumber . $row, $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_NUMBER]);
		...
		$this->activeSheet->setCellValue($this->colConstant . $row, $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_CONSTANT]['final_value']);
		...
		$this->activeSheet->setCellValue($this->colQty . $row, $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_QTY]['final_value']);
		...
		$this->activeSheet->setCellValue($this->colUnit . $row, $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_UNIT]);
		$this->activeSheet->getStyle($this->colUnit . $row)->applyFromArray($this->getNoStyle());

		$this->activeSheet->setCellValue($this->colRate . $row, $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_RATE]['final_value']);
		...
		$this->activeSheet->setCellValue($this->colTotal . $row, $item[sfBillItemBuildUpRateWithMarkupReportGenerator::ROW_BILL_ITEM_TOTAL]);
