...
class sfBillRevisionsReportGenerator extends sfBuildspaceExcelReportGenerator {
    ...
    private $tendererBillGrandTotalRevisions;
    private $estimateBillGrandTotalRevisions;
    ...
    public function setParameters($tenderers, $contractorProjectGrandTotal, $contractorBillGrandTotals, $estimateBillGrandTotalRevisions, $tendererBillGrandTotalRevisions)
    {
        ...
        $this->estimateBillGrandTotalRevisions = $estimateBillGrandTotalRevisions;

        $this->tendererBillGrandTotalRevisions = $tendererBillGrandTotalRevisions;
    }
    ...
    public function printGrandTotalValue($style)
    {
        ...
        foreach($this->tenderers as $tenderer)
        {
            ...
            $this->setValue($startColumn, $grandTotal);
    ...
    private function processEstimateItems($item)
    {
        ...
        {
            $revisionRate = $this->estimateBillGrandTotalRevisions[ $revisionNumber ]['bills'][ $item[ self::ROW_BILL_ITEM_ID ] ];
    ...
    private function processTendererItems($item)
    {
        ...
        foreach($this->tenderers as $k => $tenderer)
        {
            ...
            {
                array_push($listOfRates, $this->contractorBillGrandTotals[ $itemId ][ $tenderer['id'] ]);
            }
        }
        ...
        foreach($this->tenderers as $tenderer)
        {
            ...
            foreach($this->projectRevisions as $revisionNumber => $revision)
            {
                $tendererRate = $this->tendererBillGrandTotalRevisions[ $tenderer['id'] ][ $revisionNumber ]['bills'][ $item[ self::ROW_BILL_ITEM_ID ] ];

                parent::setValue($this->currentColumn++, $tendererRate);
            }
            $this->currentColumn--;

            $value = ( array_key_exists($item[ self::ROW_BILL_ITEM_ID ], $this->contractorBillGrandTotals) && array_key_exists($tenderer['id'], $this->contractorBillGrandTotals[ $item[ self::ROW_BILL_ITEM_ID ] ]) ) ? $this->contractorBillGrandTotals[ $item[ self::ROW_BILL_ITEM_ID ] ][ $tenderer['id'] ] : 0;
