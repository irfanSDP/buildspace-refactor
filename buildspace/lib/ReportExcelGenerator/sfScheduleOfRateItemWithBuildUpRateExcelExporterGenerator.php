...
class sfScheduleOfRateItemWithBuildUpRateExcelExporterGenerator extends sfBuildspaceExcelReportGenerator {
	...
	public function process($pages, $lock = false, $header, $subTitle, $topLeftTitle, $withoutCents, $totalPage)
	{
		...
		foreach ( $pages as $i => $item )
		{
			...
			$itemType = $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_TYPE];
			...
			switch ($itemType)
			{
				...
				default:
					$description .= $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_DESCRIPTION] . "\n";
					$char .= $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_ROW_IDX];

					if ( $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_ID] )
					{
						$this->newItem();
	...
	public function setBuildUpItemRow($item, $description, $char)
	{
		...
		$this->activeSheet->setCellValue($this->colNumber . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_NUMBER]);
		...
		$this->activeSheet->setCellValue($this->colConstant . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_CONSTANT]['final_value']);
		...
		$this->activeSheet->setCellValue($this->colQty . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_QTY]['final_value']);
		...
		$this->activeSheet->setCellValue($this->colUnit . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_UNIT]);
		$this->activeSheet->getStyle($this->colUnit . $row)->applyFromArray($this->getUnitStyle());

		$this->activeSheet->setCellValue($this->colRate . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_RATE]['final_value']);
		...
		$this->activeSheet->setCellValue($this->colTotal . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_TOTAL]);
		...
		$this->activeSheet->setCellValue($this->colWastage . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_WASTAGE]['final_value']);
		...
		$this->activeSheet->setCellValue($this->colLineTotal . $row, $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_LINE_TOTAL]);
	...
	public function createResourceItemHeader()
	{
		...
		foreach ( $this->sorItemInfo as $item )
		{
			$itemType = $item[sfScheduleOfRateItemBuildUpQtyReportGenerator::ROW_BILL_ITEM_TYPE];
	...
	public function setScheduleOfRateItemInfo($sorItemInfo)
	{
		$this->sorItemInfo = $sorItemInfo;
	}

	public function setSORItem($sorItem)
	{
	...
	private function setScheduleOfRateItemRow($description)
	{
		...
		$this->activeSheet->setCellValue($this->colUnit . $row, $this->sorItem['uom_symbol']);
		$this->activeSheet->getStyle($this->colUnit . $row)->applyFromArray($this->getUnitStyle());

		$this->activeSheet->setCellValue($this->colLineTotal . $row, $this->sorItem['total_rate']);
		$this->activeSheet->getStyle($this->colLineTotal . $row)->applyFromArray($this->getRateStyling());
		$this->activeSheet->getStyle($this->colLineTotal . $row)->getNumberFormat()->applyFromArray($this->getNumberFormatStandard());
