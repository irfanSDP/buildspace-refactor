...
class sfScheduleOfRateItemExcelExporterGenerator extends sfBuildspaceExcelReportGenerator {
	...
	public function process($pages, $lock = false, $header, $subTitle, $topLeftTitle, $withoutCents, $totalPage)
	{
		...
		foreach ( $pages as $i => $item )
		{
			...
			$itemType = $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_TYPE];
			...
			switch ($itemType)
			{
				...
				case ScheduleOfRateItem::TYPE_HEADER:
					$description .= $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_DESCRIPTION] . "\n";
					$prevItemType  = $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_TYPE];
					$prevItemLevel = $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_LEVEL];

					break;

				default:
					$description .= $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_DESCRIPTION] . "\n";
					$char .= $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_ROW_IDX];

					if ( $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_ID] )
					{
						$this->newItem();

						$this->setScheduleOfRateItem($item, $description, $char);
	...
	public function setScheduleOfRateItem($item, $description, $char)
	{
		$row         = $this->currentRow;
		$factorValue = null;
		$totalValue  = null;
		...
		$this->activeSheet->setCellValue($this->colUnit . $row, $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_UNIT]);
		$this->activeSheet->getStyle($this->colUnit . $row)->applyFromArray($this->getUnitStyle());

		$this->activeSheet->setCellValue($this->colRate . $row, $item[sfScheduleOfRateItemReportGenerator::ROW_BILL_ITEM_RATE]);
