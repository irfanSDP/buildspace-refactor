<?php
class sfPostContractItemReportGeneratorWorkdoneOnlyQty extends sfPostContractItemReportGenerator
{
    ...
    public function printGrandTotalValue($style)
    {
        ...
        parent::setPercentageValue( $this->colWorkDoneQty, ((array_key_exists($this->currentElementId, $this->elementTotals) && array_key_exists('up_to_date_percentage',$this->elementTotals[$this->currentElementId])) ? $this->elementTotals[$this->currentElementId]['up_to_date_percentage'] : 0) / 100);
    ...
    public function setBillHeader( $billHeader = null, $topLeftTitle, $subTitle ) 
    {
        ...
        //Set Right Title
        $this->activeSheet->setCellValue( $this->colWorkDoneQty."3", $this->topRightTitle );
        $this->activeSheet->mergeCells( $this->colWorkDoneQty.'3:'.$this->colWorkDoneAmount.'3' );
        $this->activeSheet->getStyle( $this->colWorkDoneQty.'3:'.$this->colWorkDoneAmount.'3' )->applyFromArray( $this->getRightTitleStyle() );
    }
    ...
    public function createHeader( $new = false ) 
    {
        ...
        $this->activeSheet->setCellValue( $this->colWorkDoneQty.$row, self::COL_NAME_WORKDONE );
        $this->activeSheet->mergeCells( $this->colWorkDoneQty.$this->currentRow.':'.$this->colWorkDoneAmount.$this->currentRow );

        $this->currentRow++;

        $this->activeSheet->setCellValue( $this->colWorkDoneQty.$this->currentRow, self::COL_NAME_QTY );
    ...
    public function processItems($item)
    {
        ...
        $workDoneQty = (array_key_exists('up_to_date_qty', $item[sfBuildspacePostContractReportPageElementGenerator::ROW_CLAIM_WORKDONE])) ? $item[sfBuildspacePostContractReportPageElementGenerator::ROW_CLAIM_WORKDONE]['up_to_date_qty'] : 0;
        ...
        parent::setNormalQtyValue( $this->colWorkDoneQty, $workDoneQty);
