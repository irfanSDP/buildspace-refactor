...
class sfBuildspacePostContractSubPackageReportPageItemWithCurrentClaimGenerator extends sfBuildspacePostContractSubPackageReportPageItemGenerator
{
    ...
    public function generatePages($typeRef)
    {
	    ...
	    $elementGrandTotals = SubPackagePostContractStandardClaimTable::getTotalClaimRateGroupByElement($this->bill->id, $typeRef, $revision, $this->subPackage->id);
        ...
        if(count($this->affectedElements))
        {
            foreach($this->affectedElements as $affectedElement)
            {
	            ...
	            $billItems = SubPackageTable::getDataForPrintingPreviewItemsByColumn($element, $revision, $this->subPackage, $typeRef, 'current_amount');
