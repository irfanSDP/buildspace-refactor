 ...
 */
class sfBuildSpaceScheduleOfRateLibraryImporter
{
    ...
    public function __construct(ScheduleOfRate $scheduleOfRate, SimpleXMLElement $parser, Doctrine_Connection $con)
    {
        $this->scheduleOfRate = $scheduleOfRate;
        $this->parser         = $parser;
        $this->con            = $con;
        $this->pdo            = $con->getDbh();
        $this->sorTrades      = $parser->TRADES;
        $this->sorItems       = $parser->ITEMS;

        // get Last Priority for current Trades
        $lastPriority = ScheduleOfRateTradeTable::getMaxPriorityByLibraryId($scheduleOfRate->id);
    ...
    public function import()
    {
        ...
        $importedTradeToTradeIds = $this->saveTradeLevel($stmt, $userId);

        $xmlItems = $this->sorItems->children();
        ...
        list(
            ...
            ) = $this->parseScheduleOfRateItems($xmlItems, $importedTradeToTradeIds, $userId);
     ...
     * @return mixed
     */
    private function saveTradeLevel(sfImportExcelStatementGenerator $stmt, $userId)
    {
        $trades = array();
        ...
        foreach ($this->sorTrades->children() as $importedTrade)
        {
            ...
            $trades[] = array(
                ...
                'relation_id' => $this->scheduleOfRate->id,
                '_csrf_token' => '',
        ...
        {
            $trades[$k]['id'] = $importedTradeToTradeIds[$trade['id']];

            unset( $trade );
        }

        return $importedTradeToTradeIds;
    }
    ...
    /**
     * @param SimpleXMLElement $xmlItems
     * @param                  $importedTradeToTradeIds
     * @param                  $userId
     * @return array
     */
    private function parseScheduleOfRateItems(SimpleXMLElement $xmlItems, $importedTradeToTradeIds, $userId)
    {
        ...
        foreach ($xmlItems as $importedItem)
        {
            $asRoot         = null;
            $rate           = null;
            $tradeId        = $importedTradeToTradeIds[(string) $importedItem->trade_id];
            ...
            if ((string) $importedItem->type != ScheduleOfRateItem::TYPE_HEADER AND !is_null($rate))
            {
                $ratesToSave[] = array(
                    $itemId,
                    ScheduleOfRateItem::FORMULATED_COLUMN_RATE,
                    $rate,
                    $rate,
                    'NOW()',
                    'NOW()',
    ...
    private function saveImportedRates(array $ratesToSave)
    {
        ...
        $stmt = $this->pdo->prepare('INSERT INTO ' . ScheduleOfRateItemFormulatedColumnTable::getInstance()->getTableName() . ' (relation_id, column_name, value, final_value, created_at, updated_at, created_by, updated_by) VALUES ' . implode(', ',
                $ratesToSaveNewArray));
        $stmt->execute();
    ...
    private function saveImportedItemsByBatch(
        ...
        {
            $stmt = $this->pdo->prepare('INSERT INTO ' . ScheduleOfRateItemTable::getInstance()->getTableName() . ' (trade_id, description, type, uom_id, level, root_id, lft, rgt, priority, created_at, updated_at) VALUES ' . implode(', ',
