define('buildspace/apps/ScheduleOfRateLibrary/AddResourceCategoryDialog',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/connect",
    "dojo/when",
    ...
    var AddResourceCategoryGrid = declare('buildspace.apps.ScheduleOfRateLibrary.AddResourceCategoryGrid', dojox.grid.EnhancedGrid, {
        libraryId: 0,
        scheduleOfRateItem: null,
        currencyAbbr: null,
        baseContainer: null,
        region: 'center',
            ...
            this.on('RowClick', function(e){
                ...
                if(colField == 'resource_library_exists' && resourceItem.id > 0 ){
                    ...
                    if(!resourceItem.resource_library_exists[0]){
                        pb.show();
                        var xhrArgs = {
                            url: 'scheduleOfRate/resourceCategoryAdd',
                            content: { sorid: self.scheduleOfRateItem.id, rid: resourceItem.id, _csrf_token: resourceItem._csrf_token },
                            handleAs: 'json',
                            load: function(resp) {
                                if(resp.success){
                                    ...
                                    var buildUpRateSummaryWidget = dijit.byId('buildUpRateSummary-'+self.scheduleOfRateItem.id);
                                    if(!buildUpRateSummaryWidget){
                                        buildUpRateSummaryWidget = new BuildUpRateSummary({
                                            id: 'buildUpRateSummary-'+self.scheduleOfRateItem.id,
                                            itemId: self.scheduleOfRateItem.id,
                                    ...
                                    var accContainer = dijit.byId('accordian_'+self.libraryId+'_'+self.scheduleOfRateItem.id+'-container'),
                                        resource = resp.resource,
                                        uom = resp.uom,
                                        formatter = new GridFormatter(),
                                        buildUpStore = new dojo.data.ItemFileWriteStore({
                                            url:"scheduleOfRate/getBuildUpRateItemList/sor_item_id/"+self.scheduleOfRateItem.id+"/resource_id/"+resource.id,
                                            clearOnClose: true
                                        }),
                                        grid = BuildUpGrid({
                                            resource: resource,
                                            scheduleOfRateItemId: self.scheduleOfRateItem.id,
                                            gridOpts: {
                                                itemId: self.scheduleOfRateItem.id,
                                    ...
                                    var accContentPane = new dijit.layout.ContentPane({
                                        ...
                                        doLayout: false,
                                        id: 'accPane-'+resource.id+'-'+self.scheduleOfRateItem.id,
                                        content: grid
                                    });
                                    ...
                                    var defaultEmptyPane = dijit.byId('accPane-empty_resource-'+self.scheduleOfRateItem.id);
                    ...
                    {
                        var xhrArgs = {
                            url: 'scheduleOfRate/resourceCategoryDelete',
                            content: { sorid: self.scheduleOfRateItem.id, rid: resourceItem.id, _csrf_token: resourceItem._csrf_token },
                            handleAs: 'json',
                            load: function(resp) {
                                if(resp.success){
                                    ...
                                    var accPane = dijit.byId('accPane-'+resp.rid+'-'+self.scheduleOfRateItem.id),
                                        accContainer = dijit.byId('accordian_'+self.libraryId+'_'+self.scheduleOfRateItem.id+'-container'),
                                        buildUpRateSummaryWidget = dijit.byId('buildUpRateSummary-'+self.scheduleOfRateItem.id);
                                    ...
                                    if(buildUpRateSummaryWidget){
                                        if(resp.is_last_resource){
                                            ...
                                            accContainer.addChild(new dijit.layout.ContentPane({
                                                ...
                                                style: "padding:0px;border:0px;",
                                                doLayout: false,
                                                id: 'accPane-empty_resource-'+self.scheduleOfRateItem.id,
                                            ...
                                            var xhrUpdateBuildUpSum = {
                                                ...
                                                content: { id: self.scheduleOfRateItem.id },
                                                handleAs: 'json',
                                                load: function(resp) {
                                                },
                                                error: function(error) {
    ...
    var Formatter = declare("buildspace.apps.ScheduleOfRateLibrary.AddResourceCategoryGrid.CellFormatter", null, {
    ...
    var Dialog = declare('buildspace.apps.ScheduleOfRateLibrary.AddResourceCategoryDialog', dijit.Dialog, {
        ...
        createContent: function(){
            ...
            var formatter = new Formatter(),
                store = dojo.data.ItemFileWriteStore({
                    url: "scheduleOfRate/getResourceList/item_id/"+self.scheduleOfRateItem.id
                }),
                content = AddResourceCategoryGrid({
                    libraryId: self.libraryId,
                    scheduleOfRateItem: self.scheduleOfRateItem,
