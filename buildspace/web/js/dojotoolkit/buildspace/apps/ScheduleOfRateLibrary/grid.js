define('buildspace/apps/ScheduleOfRateLibrary/grid', [
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/array",
    "dojo/dom-attr",
    ...
    './importSORItemDialog',
    './ImportBuildspaceSORFile',
    './ScheduleOfRateExportBuildspaceZipFile',
    'dojo/i18n!buildspace/nls/ScheduleOfRateLibrary'
], function (declare, lang, array, domAttr, connect, TooltipDialog, popup, Menu, number, Selector, Rearrange, FormulatedColumn, Filter, evt, keys, focusUtil, html, xhr, PopupMenuItem, Textarea, FormulaTextBox, FileImportDialog, DropDownButton, DropDownMenu, MenuItem, ImportResourceItemDialog, ImportSORItemDialog, ImportBuildspaceSORFile, ScheduleOfRateExportBuildspaceZipFile, nls) {

    var ScheduleOfRateGrid = declare('buildspace.apps.ScheduleOfRateLibrary.Grid', dojox.grid.EnhancedGrid, {
        type: null,
        libraryId: 0,
        tradeId: 0,
        ...
        postCreate: function () {
            ...
            this.on('CellClick', function (e) {
                ...
                if (colField === 'recalculate_resource_status' && item && item.id > 0 && item.recalculate_resources_library_status[0]) {
                    self.recalculateItemRate(item);
                }
            });
        ...
        doApplyCellEdit: function (val, rowIdx, inAttrName) {
            ...
            if (val !== item[inAttrName][0]) {
                ...
                if (item[attrNameParsed + '-has_build_up'] != undefined && item[attrNameParsed + '-has_build_up'][0]) {
                    buildspace.dialog.confirm(nls.confirmation, '<div>' + nls.detachAllBuildUpAndLink + '</div>', 60, 280, function () {
        ...
        contextMenuItems: function (e) {
            ...
            if (item.id > 0) {
                ...
                if (self.type == 'tree') {
                    if (cell.field == 'rate-value') {
                        var finalValue = number.parse(item['rate-final_value']);
            ...
            if (self.type == 'tree' && self.selectedItem) {
                if (self.copyCellType == 'rate' && item.id > 0 && item.type != buildspace.widget.grid.constants.HIERARCHY_TYPE_HEADER
            ...
            self.rowCtxMenu.addChild(new dijit.PopupMenuItem({
                ...
                onClick: dojo.hitch(self, pasteFunc, e.rowIndex),
        ...
        addRow: function (rowIndex) {
            ...
            if (itemBefore.id > 0) {
                ...
                var content = {
                    id: itemBefore.id,
                    prev_item_id: prevItemId,
                    relation_id: itemBefore.relation_id,
        ...
        deleteRow: function (rowIndex) {
            ...
            if (this.type != 'tree') {
                // for element level
                title = nls.deleteTradeDialogBoxTitle;
                msg = nls.deleteTradeDialogBoxMsg;
            } else {
                // for head/item level
                ...
                load: function (data) {
                    if (data.success) {
                        ...
                        dojo.forEach(items, function (node) {
                            store.fetchItemByIdentity({
                                'identity': node.id, onItem: function (item) {
                                    for (var property in node) {
                                        if (item.hasOwnProperty(property) && property != store._getIdentifierAttribute()) {
        ...
        recalculateItemRate: function (item) {
            var self = this,
                pb = buildspace.dialog.indeterminateProgressBar({
            ...
            dojo.xhrPost({
                ...
                load: function (data) {
                    if (data.success) {
                        ...
                        store.fetchItemByIdentity({
                            'identity': items.id, onItem: function (storeItem) {
                                for (var property in items) {
                                    if (storeItem.hasOwnProperty(property) && property != store._getIdentifierAttribute()) {
                                        store.setValue(storeItem, property, items[property]);
                                        store.save();
                                    }
                                }
    ...
    return declare('buildspace.apps.ScheduleOfRateLibrary.grid', dijit.layout.BorderContainer, {
        pageId: 'page-00',
        ...
        postCreate: function () {
            ...
            if (self.type != 'tree') {
                ...
                importDropDownMenu.addChild(new MenuItem({
                    label: nls.importFromBuildSpaceSOR,
                    onClick: function (e) {
                        var fileImportDialog = new ImportBuildspaceSORFile({
                            scheduleOfRateId: self.libraryId,
                            uploadUrl: "scheduleOfRate/importBuildSpaceSORFile/scheduleOfRateId/" + self.libraryId,
                            title: nls.importFromBuildSpaceSOR,
                            libraryGrid: grid
                        });
                        fileImportDialog.show();
                    }
                ...
                importDropDownMenu.addChild(new MenuItem({
                    ...
                    onClick: function (e) {
                        var fileImportDialog = new FileImportDialog({
                            ...
                            uploadUrl: "scheduleOfRate/previewImportedFile",
                ...
                importDropDownMenu.addChild(new MenuItem({
                    ...
                    onClick: function (e) {
                        var fileImportDialog = new FileImportDialog({
                            ...
                            uploadUrl: "scheduleOfRate/importPricelist",
                ...
                exportDropDownMenu.addChild(new MenuItem({
                    label: nls.exportToBuildSpaceSOR,
                    onClick: function (e) {
                        var fileExportDialog = new ScheduleOfRateExportBuildspaceZipFile({
                            scheduleOfRateLibraryId: self.libraryId
                        });
            ...
            var container = dijit.byId('scheduleOfRate_' + self.libraryId + '-stackContainer');
            if (container) {
