...
(function() {
  define('buildspace/apps/StockIn/InvoiceDeliveryOrderTab/StockInItemRemarkDialog', ["dojo/_base/declare", 'dojo/_base/lang', 'dojo/on', "dojo/keys", "dojo/parser", "dojo/_base/connect", "dijit/_WidgetBase", "dijit/_OnDijitClickMixin", "dijit/_TemplatedMixin", "dijit/_WidgetsInTemplateMixin", 'dijit/Toolbar', 'dijit/form/Button', "dijit/layout/BorderContainer", "dijit/layout/ContentPane", "dijit/form/ValidationTextBox", "dijit/form/SimpleTextarea", "dijit/registry", "dojo/dom-style", "dojo/dom-form", "dojo/request", "dojox/validate/web", "dojo/store/Memory", "dojox/form/Manager", "dojo/text!./templates/rfqItemRemarkForm.html", 'buildspace/widget/grid/cells/Formatter', 'dojo/i18n!../../../nls/StockIn', 'buildspace/widget/grid/Filter'], function(declare, lang, bind, keys, parser, connect, _WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _WidgetsInTemplateMixin, Toolbar, Button, BorderContainer, ContentPane, ValidationTextBox, SimpleTextarea, registry, domStyle, domForm, request, web, Memory, Manager, template, Formatter, nls, Filter) {
    var POItemRemarkForm, POItemRemarkFormContainer, stockInItemDescriptionGrid;
    stockInItemDescriptionGrid = declare("buildspace.apps.PurchaseOrder.ItemList.POItemRemarkGrid", dojox.grid.EnhancedGrid, {
      style: "border-top:none;",
      keepSelection: true,
      rowSelector: "0px",
      newStockInItemRemarkId: -1,
      rfqItemRemarkId: -1,
      stockInItemId: -1,
      poItem: null,
      resourceItemId: -1,
      stockInItemGridStore: null,
      ...
      updatePOItemDescription: function(item) {
        ...
        if (self.newStockInItemRemarkId > 0) {
          lang.mixin(formValues, {
            stockInItemRemarkId: self.newStockInItemRemarkId,
            stockInItemId: self.stockInItemId
          });
        } else {
          lang.mixin(formValues, {
            stockInItemRemarkId: self.rfqItemRemarkId,
            stockInItemId: self.stockInItemId
          });
        }
        ...
        pb.show();
        return request.post("stockIn/updateSelectedPreviousRFQItemRemark", {
          data: formValues,
          handleAs: 'json'
          ...
          if (response.success && response.id > 0) {
            self.newStockInItemRemarkId = response.id;
            self.refreshGrid();
            data = response.data;
            for (property in data) {
              if (self.poItem.hasOwnProperty(property) && property !== self.stockInItemGridStore._getIdentifierAttribute()) {
                self.stockInItemGridStore.setValue(self.poItem, property, data[property]);
              }
            }
    ...
    POItemRemarkForm = declare("buildspace.apps.PurchaseOrder.ItemList.POItemRemarkForm", [_WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      postCreate: function() {
        ...
        this.inherited(arguments);
        self = this;
        return bind(this.stockInItemRemarkFormSaveButton, 'click', function(e) {
          return self.save();
        });
      },
      startup: function() {
        this.inherited(arguments);
        return this['stockInItemRemarkForm'].setFormValues(this.formInfo);
      },
      ...
      save: function() {
        ...
        form = this['stockInItemRemarkForm'];
        if (!form.validate()) {
          return false;
        }
        formValues = dojo.formToObject(this['stockInItemRemarkForm'].id);
        pb = buildspace.dialog.indeterminateProgressBar({
          title: nls.pleaseWait + "..."
        });
        if (self.newStockInItemRemarkId > 0) {
          lang.mixin(formValues, {
            rfqItemRemarkId: self.newStockInItemRemarkId,
            stockInItemId: self.stockInItemId,
        ...
        pb.show();
        return request.post("stockIn/updateRFQItemRemark", {
          data: formValues,
          handleAs: 'json'
        }).then(function(response) {
          ...
          if (response.success && response.id > 0) {
            self.newStockInItemRemarkId = response.id;
            self['stockInItemRemarkForm'].reset();
    ...
    POItemRemarkFormContainer = declare("buildspace.apps.PurchaseOrder.ItemList.POItemRemarkFormContainer", BorderContainer, {
      gutters: false,
      ...
      _csrf_token: null,
      postCreate: function() {
        var dialogToolbar, form, grid, gridToolbar, itemRemarkId, resourceItemId, self, stockInItemId, store;
        this.inherited(arguments);
        self = this;
        stockInItemId = self.item.stockInItemId[0];
        resourceItemId = self.item.id[0];
        itemRemarkId = self.item.stockInItemRemarkId;
        store = new dojo.data.ItemFileWriteStore({
          url: "stockIn/getPreviousRFQItemRemarks/stockInItemId/" + stockInItemId + "/resourceItemId/" + resourceItemId,
          clearOnClose: true,
          urlPreventCache: true
        });
        grid = new stockInItemDescriptionGrid({
          ...
          stockInItemGridStore: self.stockInItemGridStore,
