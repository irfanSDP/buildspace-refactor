define('buildspace/apps/PostContractSubPackage/StandardBillManager',[
    ...
    function(declare, domStyle, when, number, currency, ContentPane, EnhancedGrid, GridFormatter, ScheduleOfQuantityGrid, BuildUpQuantityGrid, BuildUpQuantitySummary, aspect, nls, StandardBillGrid, LumpSumPercentDialog) {

        var Grid = declare('buildspace.apps.PostContractSubPackage.TypeGrid', EnhancedGrid, {
            rootProject: null,
            style: "border:none;",
            region: 'center',
            pageId: null,
        ...
        return declare('buildspace.apps.PostContractSubPackage.StandardBillManager', dijit.layout.BorderContainer, {
            ...
            postCreate: function(){
                ...
                dojo.subscribe('subPackagePostContractStandardBill' + self.billId + '-stackContainer-selectChild', "", function(page) {
                    var widget = dijit.byId('subPackagePostContractStandardBill' + self.billId + '-stackContainer');
                    if(widget) {
            ...
            createTypeGrid: function (){
                ...
                var stackContainer = dijit.byId('subPackagePostContractStandardBill' + this.billId + '-stackContainer');

                if(stackContainer) {
                    dijit.byId('subPackagePostContractStandardBill' + this.billId + '-stackContainer').destroyRecursive();
                }

                stackContainer = this.stackContainer = new dijit.layout.StackContainer({
                    style: 'border:0px;width:100%;height:100%;',
                    region: "center",
                    id: 'subPackagePostContractStandardBill' + this.billId + '-stackContainer'
                });
                ...
                billInfoQuery.then(function(billInfo){
                    try {
                        ...
                        var controller = new dijit.layout.StackController({
                            region: "top",
                            containerId: 'subPackagePostContractStandardBill' + me.billId + '-stackContainer'
                        });
            ...
            createElementGrid: function (typeItem, billInfo, typeItemGridStore){
                ...
                typeItemQuery.then(function(typeItem){
                    ...
                    var grid = new StandardBillGrid({
                        ...
                        gridOpts: {
                            store: store,
                            updateUrl: 'subPackagePostContractStandardBillClaim/standardBillClaimElementUpdate',
                ...
                try{
                    var grid = new StandardBillGrid({
                        ...
                        gridOpts: {
                            ...
                            claimRevision: billInfo.claim_project_revision_status,
                            selectedClaimRevision: billInfo.current_selected_claim_project_revision_status,
                            updateUrl: 'subPackagePostContractStandardBillClaim/standardBillClaimUpdate',
                ...
                scheduleOfQtyQuery.then(function(linkInfo){
                    ...
                    if(linkInfo.has_linked_qty){
                        ...
                        scheduleOfQtyGrid = ScheduleOfQuantityGrid({
                            ...
                            billColumnSettingId: billColumnSettingId,
                            disableEditingMode: true,
                            stackContainerId: 'subPackagePostContractStandardBill' + self.billId + '-stackContainer',
                            gridOpts: {
                                qtyType: type,
                    ...
                    var container = dijit.byId('subPackagePostContractStandardBill' + self.billId + '-stackContainer');
                    if(container){
