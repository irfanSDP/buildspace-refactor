define('buildspace/apps/PostContractSubPackage/Preliminaries/PreliminariesBillManager',[
	...
	return declare('buildspace.apps.PostContractSubPackage.PrelimBillManager', dijit.layout.BorderContainer, {
							...
							onRowDblClick: function(e) {
								...
								if (item.id[0] > 0) {
									...
									if (item.type == buildspace.widget.grid.constants.HIERARCHY_TYPE_HEADER || item.type == buildspace.widget.grid.constants.HIERARCHY_TYPE_HEADER_N || item.type == buildspace.widget.grid.constants.HIERARCHY_TYPE_NOID) {
										return;
									} else if ( separatedFieldName[0] != buildspace.apps.PostContractSubPackage.ProjectStructureConstants.PRELIM_COLUMN_TIMEBASED && separatedFieldName[0] != buildspace.apps.PostContractSubPackage.ProjectStructureConstants.PRELIM_COLUMN_WORKBASED ) {
										return;
									}

									if ( separatedFieldName[0] == buildspace.apps.PostContractSubPackage.ProjectStructureConstants.PRELIM_COLUMN_TIMEBASED ) {
										self.timeBasedClaimDialog(item, self.currentClaimRevision, self.currentSelectedClaimRevision, billGridStore);
									} else if ( separatedFieldName[0] == buildspace.apps.PostContractSubPackage.ProjectStructureConstants.PRELIM_COLUMN_WORKBASED ) {
            ...
            scheduleOfQtyQuery.then(function(linkInfo){
                ...
                if(linkInfo.has_linked_qty){
                    ...
                    scheduleOfQtyGrid = ScheduleOfQuantityGrid({
                        ...
                        stackContainerId: 'subPackageBillGrid' + self.billId + '-stackContainer',
                        gridOpts: {
                            qtyType: type,
