define('buildspace/apps/BQLibrary/importResourceDialog',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/connect",
    "dojo/when",
...
], function(declare, lang, connect, when, html, dom, keys, domStyle, GridFormatter, IndirectSelection, nls){

    var ImportResourceGrid = declare('buildspace.apps.BQLibrary.ImportResourceGrid', dojox.grid.EnhancedGrid, {
        type: null,
        itemId: 0,
        dialogWidget: null,
        _csrf_token: null,
        ...
        import: function(){
            ...
            if( itemIds.length > 0){
                ...
                pb.show();
                var xhrArgs = {
                    url: 'bqLibrary/importResourceItems',
    ...
    var ImportGridContainer = declare('buildspace.apps.BQLibrary.ImportResourceGridContainer', dijit.layout.BorderContainer, {
        ...
        postCreate: function(){
            ...
            var container = dijit.byId('bqLibraryBuildUpRate-resource_import_'+self.itemId+'-'+this.resource.id+'-stackContainer');
            if(container){
                var node = document.createElement("div");
                var child = new dojox.layout.ContentPane( {title: buildspace.truncateString(self.title, 60), id: self.pageId, executeScripts: true },node );
    ...
    return declare('buildspace.apps.BQLibrary.ImportResourceDialog', dijit.Dialog, {
        ...
        makeGridContainer: function(content){
            ...
            var stackContainer = dijit.byId('bqLibraryBuildUpRate-resource_import_'+id+'-stackContainer');
            if(stackContainer){
                dijit.byId('bqLibraryBuildUpRate-resource_import_'+id+'-stackContainer').destroyRecursive();
            }

            stackContainer = new dijit.layout.StackContainer({
                style:'width:100%;height:100%;border:0px;',
                region: "center",
                id: 'bqLibraryBuildUpRate-resource_import_'+id+'-stackContainer'
            });
            ...
            var controller = new dijit.layout.StackController({
                region: "top",
                containerId: 'bqLibraryBuildUpRate-resource_import_'+id+'-stackContainer'
            });
            ...
            dojo.subscribe('bqLibraryBuildUpRate-resource_import_'+id+'-stackContainer-selectChild',"",function(page){
                var widget = dijit.byId('bqLibraryBuildUpRate-resource_import_'+id+'-stackContainer');
                if(widget){
