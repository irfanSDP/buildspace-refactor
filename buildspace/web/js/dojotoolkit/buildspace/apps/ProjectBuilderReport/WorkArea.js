define('buildspace/apps/ProjectBuilderReport/WorkArea',[
    'dojo/_base/declare',
    './ProjectProperties',
    './ProjectBreakdown',
    'dojo/i18n!buildspace/nls/ProjectBuilder'], function(declare, ProjectProperties, ProjectBreakdown, nls){

    return declare('buildspace.apps.ProjectBuilderReport.WorkArea', dijit.layout.TabContainer, {
        region: "center",
        ...
        postCreate: function(){
            ...
            pb.show().then(function(){
                dojo.xhrGet({
                    ...
                    dojo.xhrGet({
                        url: "projectBuilderReport/mainInfoForm",
        ...
        removeBillTab: function (){
            ...
            for(var i in tac){
                ...
                if(tac[i].pane_info.type == buildspace.apps.ProjectBuilderReport.ProjectStructureConstants.TYPE_BILL){
        ...
        makeTab: function(item, options, closable){
            ...
            switch(parseInt(item.type)){
                case buildspace.apps.ProjectBuilderReport.ProjectStructureConstants.TYPE_BILL:
                    billType = buildspace.apps.ProjectBuilderReport.getBillTypeText(item.bill_type);
                    widget = new buildspace.apps.ProjectBuilderReport.BillManager(options);
                    id = item.id+'-form_'+item.type;
                    title = buildspace.truncateString(item.title, 35)+' :: '+billType;
                    break;
                case 9999999://sub packages
                    widget = new buildspace.apps.ProjectBuilderReport.SubPackages(options);
                    id = item.id+'-form_'+item.type;
