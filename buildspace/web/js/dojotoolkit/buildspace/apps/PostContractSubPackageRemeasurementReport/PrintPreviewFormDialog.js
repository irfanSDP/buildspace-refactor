...
(function() {
  define("buildspace/apps/PostContractSubPackageRemeasurementReport/PrintPreviewFormDialog", ["dojo/_base/declare", 'dojo/_base/lang', "dojo/keys", "dojo/parser", "dijit/_WidgetBase", "dijit/_OnDijitClickMixin", "dijit/_TemplatedMixin", "dijit/_WidgetsInTemplateMixin", 'dijit/Toolbar', 'dijit/form/Button', "dijit/layout/BorderContainer", "dijit/form/ValidationTextBox", "dijit/form/RadioButton", "dijit/form/SimpleTextarea", "dijit/registry", "dojo/dom-style", "dojo/dom-form", "dojo/request", "dojo/json", "dojox/validate/web", "dojox/form/Manager", "dojo/text!./templates/printPreviewForm.html", 'dojo/i18n!buildspace/nls/TenderingReport'], function(declare, lang, keys, parser, _WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _WidgetsInTemplateMixin, Toolbar, Button, BorderContainer, ValidationTextBox, RadioButton, SimpleTextarea, registry, domStyle, domForm, request, JSON, web, Manager, template, nls) {
    ...
    printPreviewForm = declare("buildspace.apps.PostContractSubPackageRemeasurementReport.PrintPreviewForm", [_WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _WidgetsInTemplateMixin], {
      ...
      print: function() {
        var bill_id_element, bill_type_id, csrfToken, descriptionFormat, form, formValues, opt, priceFormat, printNoCents, printingPageTitle, selectedRows, subPackageId;
        form = this.postContractRemeasurementPrintPreviewForm;
        if (!form.validate()) {
          return false;
        }
        ...
        if (this.subPackageId > 0) {
          subPackageId = document.createElement("input");
          subPackageId.setAttribute("name", "subPackageId");
          subPackageId.setAttribute("value", this.subPackageId);
          subPackageId.setAttribute('type', 'hidden');
          form.appendChild(subPackageId);
        }
        if (this.billId > 0) {
          bill_id_element = document.createElement("input");
          bill_id_element.setAttribute("name", "billId");
          bill_id_element.setAttribute("value", this.billId);
          bill_id_element.setAttribute('type', 'hidden');
          form.appendChild(bill_id_element);
        }
        if (this.bill_type_id && this.bill_type_id > 0) {
          bill_type_id = document.createElement("input");
          bill_type_id.setAttribute("name", "bill_type_id");
          bill_type_id.setAttribute("value", this.bill_type_id);
          bill_type_id.setAttribute('type', 'hidden');
          form.appendChild(bill_type_id);
        }
        ...
        opt.setAttribute("value", this.opt);
    ...
    return declare('buildspace.apps.PostContractSubPackageRemeasurementReport.PrePrintFormDialog', dijit.Dialog, {
