define('buildspace/apps/PostContractSubPackageRemeasurementReport/RemeasurementContainer',[
	'dojo/_base/declare',
	'dojo/aspect',
	'dojo/request',
	'dojo/store/Memory',
	...
	function(declare, aspect, request, Memory, RemeasurementGrid, BuildUpQuantitySummary, BuildUpQuantityGrid, GridFormatter, ScheduleOfQuantityGrid, nls) {

	return declare('buildspace.apps.PostContractSubPackageRemeasurementReport.RemeasurementContainer', dijit.layout.BorderContainer, {
		...
		postCreate: function() {
			...
			var formatter = new GridFormatter(),
				store = dojo.data.ItemFileWriteStore({
					url: "postContractSubPackageRemeasurement/getAllBills/subPackageId/" + self.subPackage.id + "/opt/" + self.opt,
					clearOnClose: true
				}),
				grid = new RemeasurementGrid({
					id: 'post-contract-subpackage-remeasurement-report-grid-'+self.subPackage.id,
					stackContainerTitle: nls.bill,
					pageId: 'post-contract-subpackage-remeasurement-report-grid-'+self.subPackage.id,
		...
		createBillTypesGrid: function(bill) {
			var self = this,
				...
				store = dojo.data.ItemFileWriteStore({
					url: "postContractSubPackageRemeasurement/getBillTypes/subPackageId/" + self.subPackage.id +"/bid/" + bill.id + "/opt/" + self.opt,
					clearOnClose: true
				});
			...
			var grid = new RemeasurementGrid({
				id: 'post-contract-subpackage-remeasurement-report-billtype'+ self.subPackage.id,
				stackContainerTitle: bill.title,
				pageId: 'post-contract-subpackage-remeasurement-report-billtype'+ self.subPackage.id + '_' + bill.id,
				project: self.project,
				subPackage: self.subPackage,
				gridOpts: {
					bill: bill,
					...
					onRowDblClick: function(e){
						...
						if(_item.id > 0 && _item.name[0] !== null){
							self.createElementGrid(bill, _item);
						}
					},
					...
					getAffectedElementsAndItemsByTypes: function(item, type) {
						...
						request.post('postContractSubPackageRemeasurementReport/getElementsAndItemsByTypes', {
							handleAs: 'json',
							data: {
		...
		createElementGrid: function(bill, billType) {
			var self = this,
				formatter = new GridFormatter(),
				store = dojo.data.ItemFileWriteStore({
					url: 'postContractSubPackageRemeasurement/getElementList/subPackageId/' + self.subPackage.id + '/btId/' + billType.id + "/opt/" + self.opt,
					clearOnClose: true
				});
			...
			var grid = new RemeasurementGrid({
				id: 'post-contract-subpackage-remeasurement-report-element-'+ self.subPackage.id,
				stackContainerTitle: billType.name,
				pageId: 'post-contract-subpackage-remeasurement-report-element-'+ self.subPackage.id + '_' + billType.id,
				project: self.project,
				subPackage: self.subPackage,
				gridOpts: {
					gridContainer: self,
					...
					getAffectedTypesAndItemsByTypes: function(item, type) {
						...
						request.post('postContractSubPackageRemeasurementReport/getTypesAndItemsByElement', {
							handleAs: 'json',
							data: {
								subPackageId: self.subPackage.id,
							...
							var typeGrid = dijit.byId('post-contract-subpackage-remeasurement-report-billtype'+ self.subPackage.id);
		...
		createItemGrid: function(bill, billType, element){
			...
			var store = new dojo.data.ItemFileWriteStore({
				url: "postContractSubPackageRemeasurement/getItemList/subPackageId/" + self.subPackage.id +"/btId/" + billType.id + "/elementId/" + element.id + "/opt/" + self.opt,
				clearOnClose: true
			});

			var grid = new RemeasurementGrid({
				id: 'post-contract-subpackage-remeasurement-report-item-'+self.subPackage.id,
				stackContainerTitle: element.description,
				pageId: 'post-contract-subpackage-remeasurement-report-item-'+self.subPackage.id+'_'+element.id,
				project: self.project,
				subPackage: self.subPackage,
				gridOpts: {
					gridContainer: self,
					...
					getAffectedTypesAndElementsByItems: function(item, type) {
						...
						request.post('postContractSubPackageRemeasurementReport/getTypesAndElementsByItem', {
							handleAs: 'json',
							data: {
								subPackageId: self.subPackage.id,
								bill_type_id: self.billTypeId,
							...
							var typeGrid    = dijit.byId('post-contract-subpackage-remeasurement-report-billtype'+ self.subPackage.id);
							var elementGrid = dijit.byId('post-contract-subpackage-remeasurement-report-element-'+ self.subPackage.id);
		...
		createBuildUpQuantityContainer: function(colField, editable, item, dimensionColumns, billColumnSettingId, billGridStore) {
			...
			if ( colField === 'addition-qty_per_unit' ) {
				moduleName = 'postContractSubPackageRemeasurementBillBuildUpQuantity';
				itemId     = item.sub_package_post_contract_bill_item_rate_id;
			} else {
			...
			scheduleOfQtyQuery.then(function(linkInfo){
				...
				if(linkInfo.has_linked_qty){
					...
					scheduleOfQtyGrid = new ScheduleOfQuantityGrid({
						...
						stackContainerId: 'postContractSubPackageRemeasurementReport-grid_'+self.subPackage.id+'-stackContainer',
						gridOpts: {
							qtyType: type,
		...
		makeGridContainer: function(content, title){
			...
			var stackContainer = dijit.byId('postContractSubPackageRemeasurementReport-grid_'+id+'-stackContainer');
			if(stackContainer){
				dijit.byId('postContractSubPackageRemeasurementReport-grid_'+id+'-stackContainer').destroyRecursive();
			}

			stackContainer = new dijit.layout.StackContainer({
				style:'width:100%;height:100%;border:0px;',
				region: "center",
				id: 'postContractSubPackageRemeasurementReport-grid_'+id+'-stackContainer'
			});
			...
			var controller = new dijit.layout.StackController({
				region: "top",
				containerId: 'postContractSubPackageRemeasurementReport-grid_'+id+'-stackContainer'
			});
			...
			dojo.subscribe('postContractSubPackageRemeasurementReport-grid_'+id+'-stackContainer-selectChild',"",function(page){
				var widget = dijit.byId('postContractSubPackageRemeasurementReport-grid_'+id+'-stackContainer');
				if(widget){
