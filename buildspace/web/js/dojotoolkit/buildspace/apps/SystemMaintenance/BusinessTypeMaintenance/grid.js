define('buildspace/apps/SystemMaintenance/BusinessTypeMaintenance/grid',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/array",
    "dojo/dom-attr",
    ...
    var BusinessTypeMaintenanceGrid = declare('buildspace.apps.SystemMaintenance.BusinessTypeMaintenance.Grid', dojox.grid.EnhancedGrid, {
        type: null,
        ...
        contextMenuItems: function(e){
            ...
            if(item && !isNaN(parseInt(item.id[0]))){
                this.rowCtxMenu.addChild(new dijit.MenuItem({
                    label: nls.addBusinessType,
                    iconClass:"icon-16-container icon-16-add",
                    onClick: dojo.hitch(this.formContainer, "addBusinessType")
                }));
                this.rowCtxMenu.addChild(new dijit.PopupMenuItem({
                    label: nls.edit,
                    iconClass:"icon-16-container icon-16-edit",
                    onClick: dojo.hitch(this.formContainer, "editBusinessType")
                }));
                this.rowCtxMenu.addChild(new dijit.PopupMenuItem({
                    label: nls.deleteBusinessType,
                    iconClass:"icon-16-container icon-16-delete",
                    onClick: dojo.hitch(this.formContainer, "deleteBusinessType")
                }));
            }
        },
        disableToolbarButtons: function(isDisable, buttonsToEnable){
            var deleteRowBtn = dijit.byId('deleteBusinessTypeRow-button');
            var editRowBtn = dijit.byId('editBusinessTypeRow-button');
            ...
            if(isDisable && buttonsToEnable instanceof Array ){
                dojo.forEach(buttonsToEnable, function(label){
                    var btn = dijit.byId(label+'BusinessTypeRow-button');
    ...
    return declare('buildspace.apps.SystemMaintenance.BusinessTypeMaintenance.grid', dijit.layout.BorderContainer, {
        pageId: 'page-00',
        ...
        postCreate: function(){
            ...
            var grid = this.grid = new BusinessTypeMaintenanceGrid(this.gridOpts);
            var toolbar = new dijit.Toolbar({region: "top", style:"padding:2px;border-bottom:none;width:100%;"});
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'addBusinessTypeRow-button',
                    label: nls.addBusinessType,
                    iconClass: "icon-16-container icon-16-add",
                    onClick: dojo.hitch(grid.formContainer, "addBusinessType")
                })
            );
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'editBusinessTypeRow-button',
                    label: nls.edit,
                    iconClass: "icon-16-container icon-16-edit",
                    disabled: grid.selection.selectedIndex > -1 ? false : true,
                    onClick: dojo.hitch(grid.formContainer, "editBusinessType")
                })
            );
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'deleteBusinessTypeRow-button',
                    label: nls.delete,
                    iconClass: "icon-16-container icon-16-delete",
                    disabled: grid.selection.selectedIndex > -1 ? false : true,
                    onClick: dojo.hitch(grid.formContainer, "deleteBusinessType")
                })
            );
