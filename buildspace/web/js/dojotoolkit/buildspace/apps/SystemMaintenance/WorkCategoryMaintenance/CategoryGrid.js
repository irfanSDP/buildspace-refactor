define("buildspace/apps/SystemMaintenance/WorkCategoryMaintenance/CategoryGrid",[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/array",
    "dojo/dom-attr",
...
], function(declare, lang, array, domAttr, Menu, Selector, Rearrange, evt, keys, focusUtil, html, xhr, PopupMenuItem, Select, Textarea, FilterToolbar, nls){

    var CategoryMaintenanceGrid = declare('buildspace.apps.SystemMaintenance.WorkCategoryMaintenance.CategoryMaintenanceGrid', dojox.grid.EnhancedGrid, {
        type: null,
        style: "border-top:none;",
        selectedItem: null,
        keepSelection: true,
        rowSelector: '0px',
        CategoryFormContainer: null,
        ...
        contextMenuItems: function(e){
            ...
            if(item && !isNaN(parseInt(item.id[0]))){
                this.rowCtxMenu.addChild(new dijit.MenuItem({
                    label: nls.addWorkCategory,
                    iconClass:"icon-16-container icon-16-add",
                    onClick: dojo.hitch(this.CategoryFormContainer, "addCategory")
                }));
                this.rowCtxMenu.addChild(new dijit.PopupMenuItem({
                    label: nls.edit,
                    iconClass:"icon-16-container icon-16-edit",
                    onClick: dojo.hitch(this.CategoryFormContainer, "editCategory")
                }));
                this.rowCtxMenu.addChild(new dijit.PopupMenuItem({
                    label: nls.delete,
                    iconClass:"icon-16-container icon-16-delete",
                    onClick: dojo.hitch(this.CategoryFormContainer, "deleteCategory")
                }));
            }
        },
        disableToolbarButtons: function(isDisable, buttonsToEnable){
            var deleteRowBtn = dijit.byId('deleteCategoryRow-button');
            var editRowBtn = dijit.byId('editCategoryRow-button');
            ...
            if(isDisable && buttonsToEnable instanceof Array ){
                dojo.forEach(buttonsToEnable, function(label){
                    var btn = dijit.byId(label+'CategoryRow-button');
    ...
    return declare('buildspace.apps.SystemMaintenance.CategoryMaintenance.grid', dijit.layout.BorderContainer, {
        pageId: 'page-00',
        ...
        postCreate: function(){
            ...
            var grid = this.grid = new CategoryMaintenanceGrid(this.gridOpts);
            var toolbar = new dijit.Toolbar({region: "top", style:"padding:2px;border-bottom:none;width:100%;"});
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'addCategoryRow-button',
                    label: nls.addWorkCategory,
                    iconClass: "icon-16-container icon-16-add",
                    onClick: dojo.hitch(grid.CategoryFormContainer, "addCategory")
                })
            );
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'editCategoryRow-button',
                    label: nls.edit,
                    iconClass: "icon-16-container icon-16-edit",
                    disabled: grid.selection.selectedIndex > -1 ? false : true,
                    onClick: dojo.hitch(grid.CategoryFormContainer, "editCategory")
                })
            );
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'deleteCategoryRow-button',
                    label: nls.delete,
                    iconClass: "icon-16-container icon-16-delete",
                    disabled: grid.selection.selectedIndex > -1 ? false : true,
                    onClick: dojo.hitch(grid.CategoryFormContainer, "deleteCategory")
                })
            );
            var filter  = new FilterToolbar({
                grid: grid,
                region:"top",
                filterFields: [{name : nls.name, description: nls.description}]
            });
