define("buildspace/apps/SystemMaintenance/SubPackageWorkMaintenance/Grid",[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/array",
    "dojo/dom-attr",
    ...
    var MaintenanceGrid = declare('buildspace.apps.SystemMaintenance.SubPackageWorkMaintenance.Grid', dojox.grid.EnhancedGrid, {
        type: null,
        ...
        contextMenuItems: function(e){
            ...
            if(item && !isNaN(parseInt(item.id[0]))){
                this.rowCtxMenu.addChild(new dijit.MenuItem({
                    label: nls.addSubPackageWork,
                    iconClass:"icon-16-container icon-16-add",
                    onClick: dojo.hitch(this.FormContainer, "addRecord")
                }));
                this.rowCtxMenu.addChild(new dijit.PopupMenuItem({
                    label: nls.edit,
                    iconClass:"icon-16-container icon-16-edit",
                    onClick: dojo.hitch(this.FormContainer, "editRecord")
                }));
                this.rowCtxMenu.addChild(new dijit.PopupMenuItem({
                    label: nls.delete,
                    iconClass:"icon-16-container icon-16-delete",
                    onClick: dojo.hitch(this.FormContainer, "deleteRecord")
                }));
            }
        },
        disableToolbarButtons: function(isDisable, buttonsToEnable){
            var self = this;
            var deleteRowBtn = dijit.byId('deleteRecordRow_'+self.gridOpts.param+'-button');
            var editRowBtn = dijit.byId('editRecordRow_'+self.gridOpts.param+'-button');
            ...
            if(isDisable && buttonsToEnable instanceof Array ){
                dojo.forEach(buttonsToEnable, function(label){
                    var btn = dijit.byId(label+'RecordRow_'+self.gridOpts.param+'-button');
    ...
    return declare('buildspace.apps.SystemMaintenance.SubPackageWorkMaintenance.grid', dijit.layout.BorderContainer, {
        pageId: 'page-00',
        ...
        postCreate: function(){
            ...
            var grid = this.grid = new MaintenanceGrid(this.gridOpts);
            var toolbar = new dijit.Toolbar({region: "top", style:"padding:2px;border-bottom:none;width:100%;"});
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'addRecordRow_'+self.gridOpts.param+'-button',
                    label: nls.addSubPackageWork,
                    iconClass: "icon-16-container icon-16-add",
                    onClick: dojo.hitch(grid.FormContainer, "addRecord")
                })
            );
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'editRecordRow_'+self.gridOpts.param+'-button',
                    label: nls.edit,
                    iconClass: "icon-16-container icon-16-edit",
                    disabled: true,
                    onClick: dojo.hitch(grid.FormContainer, "editRecord")
                })
            );
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'deleteRecordRow_'+self.gridOpts.param+'-button',
                    label: nls.delete,
                    iconClass: "icon-16-container icon-16-delete",
                    disabled: true,
                    onClick: dojo.hitch(grid.FormContainer, "deleteRecord")
                })
            );
