...
function(declare, lang, Rearrange, evt, keys, focusUtil, Textarea, FilterToolbar, GridFormatter, nls) {
    var AccountCodeMaintenanceGrid = declare('buildspace.apps.AccountCodeMaintenance.AccountCodeMaintenenceGrid', dojox.grid.EnhancedGrid, {
        region: "center",
        ...
        doApplyCellEdit: function(val, rowIdx, inAttrName) {
            ...
            if(val !== item[inAttrName][0]) {
                ...
                var url = "systemMaintenance/accountCodeUpdate";
                ...
                if(item && isNaN(parseInt(item.id[0]))){
                    ...
                    url = "systemMaintenance/accountCodeAdd";
                }
        ...
        deleteRow: function() {
            ...
            if(this.selection.selectedIndex > -1 && item && !isNaN(parseInt(item.id[0]))) {
                ...
                buildspace.dialog.confirm(nls.deleteAccountCodeConfirmation, '<div>'+nls.doYouReallywantToDeleteAccountCode+' ?</div>', 90, 320, function() {
                    pb.show().then(function() {
                        dojo.xhrPost({
                            url: "systemMaintenance/accountCodeDelete",
                            ...
                            load: function(data) {
                                if(data.success){
                                    ...
                                    buildspace.dialog.alert(nls.cannotDeleteAccountCodeTitle,nls.cannotDeleteAccountCodeMsg,90,320);
                                }
        ...
        disableToolbarButtons: function(isDisable, buttonsToEnable){
            var addRowBtn = dijit.byId('AddAccountCodeRow-button');
            var deleteRowBtn = dijit.byId('DeleteAccountCodeRow-button');
            ...
            if(isDisable && buttonsToEnable instanceof Array ){
                dojo.forEach(buttonsToEnable, function(label){
                    var btn = dijit.byId(label+'AccountCodeRow-button');
                    if(btn)
    ...
    return declare('buildspace.apps.AccountGroupMaintenance.AccountGroupMaintenanceFormContainer', dijit.layout.BorderContainer, {
        ...
        postCreate: function() {
            ...
            var accountConstants = buildspace.apps.SystemMaintenance.AccountConstants;

            var grid = this.AccountCodeMaintenanceGrid = new AccountCodeMaintenanceGrid({
                store: new dojo.data.ItemFileWriteStore({
                    clearOnClose: true,
                    url: "systemMaintenance/getAccountCodes/accountGroupId/" + this.accountGroupId,
                }),
                structure: [
                    {name: 'No.', field: 'id', width:'30px', styles:'text-align:center;', formatter: formatter.rowCountCellFormatter },
                    {name: nls.code, field: 'code', width:'150px', editable: true, cellType: 'buildspace.widget.grid.cells.Textarea', noresize: true},
                    ...
                    {name: nls.taxCode, field: 'tax_code', width:'150px', editable: true, cellType: 'buildspace.widget.grid.cells.Textarea', noresize: true},
                    {name: nls.type, field: 'type', width:'70px', styles:'text-align:center;', editable:true, cellType: 'dojox.grid.cells.Select', options: Object.values(accountConstants.AccountCodeValues), values: Object.values(accountConstants.AccountCodes) },
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'DeleteAccountCodeRow-button',
                    label: nls.deleteRow,
                    iconClass: "icon-16-container icon-16-delete",
                    disabled: true,
