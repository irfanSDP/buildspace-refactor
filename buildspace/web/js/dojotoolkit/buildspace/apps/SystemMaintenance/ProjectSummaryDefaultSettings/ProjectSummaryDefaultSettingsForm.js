define('buildspace/apps/SystemMaintenance/ProjectSummaryDefaultSettings/ProjectSummaryDefaultSettingsForm',[
    ...
    var SettingsForm = declare('buildspace.apps.SystemMaintenance.ProjectSummaryDefaultSettings.SettingsForm',
        [Form,
        _WidgetBase,
        _TemplatedMixin,
        _WidgetsInTemplateMixin,
    ...
    return declare('buildspace.apps.ProjectSummaryDefaultSettings.ProjectSummaryDefaultSettingsFormContainer', dijit.layout.BorderContainer, {
        project: null,
        region: 'center',
        ...
        postCreate: function(){
            ...
            this.formValues = dojo.xhrGet({
                url: 'systemMaintenance/getProjectSummaryDefaultSettings',
                handleAs: 'json',
                load: function(results){
                    var settingsForm = this.settingsForm = new SettingsForm({
                        id: 'project_summary_default_setting-settings_form',
                        region: 'top',
        ...
        submit: function(){
            ...
            if(dijit.byId('project_summary_default_setting-settings_form')){
                formValues = dojo.formToObject(dijit.byId('project_summary_default_setting-settings_form').id);
            }

            formValues['project_summary_default_setting[left_text]'] = this.leftEditor.get('value');
            formValues['project_summary_default_setting[right_text]'] = this.rightEditor.get('value');

            pb.show();

            dojo.xhrPost({
                url: 'systemMaintenance/updateProjectSummaryDefaultSettings',
                content: formValues,
                handleAs: 'json',
                load: function(resp) {
                    dojo.query('[id^="error-project_summary_default_setting_"]').forEach(function(node){
                        node.innerHTML = '';
                    });

                    if(!resp.success){
                        for(var key in resp.errors){
                            html.set(dom.byId("error-project_summary_default_setting_"+key), resp.errors[key]);
                        }
                    }
                    pb.hide();
                },
