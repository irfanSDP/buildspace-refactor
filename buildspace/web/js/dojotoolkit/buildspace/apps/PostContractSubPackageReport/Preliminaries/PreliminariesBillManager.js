define('buildspace/apps/PostContractSubPackageReport/Preliminaries/PreliminariesBillManager',[
	...
	function(declare, domStyle, when, currency, Memory, ContentPane, EnhancedGrid, GridFormatter, aspect, request, PreliminariesBillGrid, BuildUpQuantitySummary, BuildUpQuantityGrid, TimeBasedFormDialog, WorkBasedFormDialog, LumpSumPercentDialog, nls) {

	var PrelimBillElementContainer = declare('buildspace.apps.PostContractSubPackageReport.PrelimBillElementContainer', dijit.layout.BorderContainer, {
		...
		postCreate: function() {
			...
			dojo.subscribe('subPackageBillGrid' + self.billId + '-stackContainer-selectChild', "", function(page) {
				var widget = dijit.byId('subPackageBillGrid' + self.billId + '-stackContainer');
		...
		createElementGrid: function() {
			...
			var stackContainer = dijit.byId('subPackageBillGrid' + this.billId + '-stackContainer');

			if(stackContainer) {
				dijit.byId('subPackageBillGrid' + this.billId + '-stackContainer').destroyRecursive();
			}

			stackContainer = this.stackContainer = new dijit.layout.StackContainer({
				style: 'border:0px;width:100%;height:100%;',
				region: "center",
				id: 'subPackageBillGrid' + this.billId + '-stackContainer'
			});

			var billInfoQuery = dojo.xhrGet({
				url: "postContractSubPackage/getBillInfo",
				handleAs: "json",
				content: {
					id: this.billId,
					sub_package_id: this.subPackage.id
				}
			}),
			me = this;
			...
			billInfoQuery.then(function(billInfo) {
				try {
					var url = 'subPackagePostContractPreliminaries/getElementList';
					...
					var store = dojo.data.ItemFileWriteStore({
						url: url + '/id/' + self.billId + '/sub_package_id/' + self.subPackage.id,
						clearOnClose: true,
					...
					var grid = new PreliminariesBillGrid({
						...
						pageId: 'sub_package_element-page-' + me.billId,
						id: 'sub_package_element-page-container-' + me.billId,
						gridOpts: {
							gridContainer: self,
							store: store,
					...
					var controller = new dijit.layout.StackController({
						region: "top",
						containerId: 'subPackageBillGrid' + me.billId + '-stackContainer'
					});
					...
					var controllerPane = new dijit.layout.ContentPane({
						...
						region: 'top',
						id: 'subPackageBillGrid'+me.billId+'-controllerPane',
						content: controller
					});
		...
		createItemGrid: function(element, billInfo, elementGridStore) {
			var url = 'subPackagePostContractPreliminaries/getItemList', self = this,
				hierarchyTypes = {
					options: [
			...
			var store = new dojo.data.ItemFileWriteStore({
				url: url + '/id/' + element.id + "/bill_id/" + self.billId + '/sub_package_id/' + self.subPackage.id,
				clearOnClose: true
			});
			...
			return when(unitQuery, function(uom){
				...
				try{
					var grid = new PreliminariesBillGrid({
						...
						id: 'sub_package_item-page-container-' + self.billId,
						elementId: element.id,
						pageId: 'sub_package_item-page-' + self.billId,
						type: 'tree',
						gridOpts: {
							gridContainer: self,
							store: store,
							...
							onRowDblClick: function(e) {
								...
								if (item.id[0] > 0) {
									...
									if (item.type == buildspace.widget.grid.constants.HIERARCHY_TYPE_HEADER || item.type == buildspace.widget.grid.constants.HIERARCHY_TYPE_HEADER_N || item.type == buildspace.widget.grid.constants.HIERARCHY_TYPE_NOID) {
										return;
									} else if ( separatedFieldName[0] != buildspace.apps.PostContractSubPackageReport.ProjectStructureConstants.PRELIM_COLUMN_TIMEBASED && separatedFieldName[0] != buildspace.apps.PostContractSubPackageReport.ProjectStructureConstants.PRELIM_COLUMN_WORKBASED ) {
										return;
									}

									if ( separatedFieldName[0] == buildspace.apps.PostContractSubPackageReport.ProjectStructureConstants.PRELIM_COLUMN_TIMEBASED ) {
										self.timeBasedClaimDialog(item, self.currentClaimRevision, self.currentSelectedClaimRevision, billGridStore);
									} else if ( separatedFieldName[0] == buildspace.apps.PostContractSubPackageReport.ProjectStructureConstants.PRELIM_COLUMN_WORKBASED ) {
		...
		createBuildUpQuantityContainer: function(item, dimensionColumns, billColumnSettingId, billGridStore) {
			...
			var container = dijit.byId('subPackageBillGrid' + self.billId + '-stackContainer');
		...
		timeBasedClaimDialog: function(item, currentClaimRevision, currentSelectedClaimRevision, billGridStore) {
			...
			request.get('subPackagePostContractPreliminaries/getTimeBasedInformation', {
				query: {
					id: item.sub_package_post_contract_bill_item_rate_id,
		...
		workBasedClaimDialog: function(item, currentClaimRevision, currentSelectedClaimRevision, billGridStore) {
			...
			request.get('subPackagePostContractPreliminaries/getWorkBasedInformation', {
				query: {
					id: item.sub_package_post_contract_bill_item_rate_id,
					sub_package_id: self.subPackage.id,
					bill_id: self.billId
		...
		reconstructBillContainer: function() {
			var controllerPane = dijit.byId('subPackageBillGrid'+this.billId+'-controllerPane'),
				stackContainer = dijit.byId('subPackageBillGrid'+this.billId+'-stackContainer');
	...
	return declare('buildspace.apps.PostContractSubPackageReport.PrelimBillManager', dijit.layout.TabContainer, {
		region: "center",
		rootProject: null,
		subPackage: null,
		...
		postCreate: function() {
			...
			var prelimBillElementContainer = new PrelimBillElementContainer({
				id: 'sub_package_bill_element_container_'+this.subPackage.id+'-bill-'+this.billId,
