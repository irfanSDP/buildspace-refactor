define('buildspace/apps/PostContractSubPackageRemeasurement/RemeasurementContainer',[
	'dojo/_base/declare',
	"dojo/aspect",
	'./RemeasurementGrid',
	...
	return declare('buildspace.apps.PostContractSubPackageRemeasurement.RemeasurementContainer', dijit.layout.BorderContainer, {
		...
		postCreate: function() {
			...
			var formatter = new GridFormatter(),
				...
				grid = new RemeasurementGrid({
					id: 'post-contract-subpackage-remeasurement-grid-'+self.subPackage.id,
					stackContainerTitle: nls.bill,
					pageId: 'post-contract-subpackage-remeasurement-grid-'+self.subPackage.id,
		...
		createBillTypesGrid: function(bill) {
			...
			var grid = new RemeasurementGrid({
				id: 'post-contract-subpackage-remeasurement-billtype'+ self.subPackage.id,
				stackContainerTitle: bill.title,
				pageId: 'post-contract-subpackage-remeasurement-billtype'+ self.subPackage.id + '_' + bill.id,
				project: self.project,
				subPackage: self.subPackage,
				gridOpts: {
					store: store,
		...
		createElementGrid: function(billType) {
			...
			var grid = new RemeasurementGrid({
				id: 'post-contract-subpackage-remeasurement-element-'+ self.subPackage.id,
				stackContainerTitle: billType.name,
				pageId: 'post-contract-subpackage-remeasurement-element-'+ self.subPackage.id + '_' + billType.id,
				project: self.project,
				subPackage: self.subPackage,
				gridOpts: {
					store: store,
		...
		createItemGrid: function(billType, element){
			...
			var grid = new RemeasurementGrid({
				id: 'post-contract-subpackage-remeasurement-item-'+self.subPackage.id,
				stackContainerTitle: element.description,
				pageId: 'post-contract-subpackage-remeasurement-item-'+self.subPackage.id+'_'+element.id,
				project: self.project,
				subPackage: self.subPackage,
				gridOpts: {
					store: store,
			...
			scheduleOfQtyQuery.then(function(linkInfo){
				...
				if(linkInfo.has_linked_qty){
					...
					scheduleOfQtyGrid = new ScheduleOfQuantityGrid({
						...
						stackContainerId: 'postContractSubPackageRemeasurement-grid_'+self.subPackage.id+'-stackContainer',
						gridOpts: {
							qtyType: type,
				...
				var container = dijit.byId('postContractSubPackageRemeasurement-grid_' + self.subPackage.id + '-stackContainer');
		...
		makeGridContainer: function(content, title){
			...
			var stackContainer = dijit.byId('postContractSubPackageRemeasurement-grid_'+id+'-stackContainer');
			if(stackContainer){
				dijit.byId('postContractSubPackageRemeasurement-grid_'+id+'-stackContainer').destroyRecursive();
			}

			stackContainer = new dijit.layout.StackContainer({
				style:'width:100%;height:100%;border:0px;',
				region: "center",
				id: 'postContractSubPackageRemeasurement-grid_'+id+'-stackContainer'
			});
			...
			var controller = new dijit.layout.StackController({
				region: "top",
				containerId: 'postContractSubPackageRemeasurement-grid_'+id+'-stackContainer'
			});
			...
			dojo.subscribe('postContractSubPackageRemeasurement-grid_'+id+'-stackContainer-selectChild',"",function(page){
				var widget = dijit.byId('postContractSubPackageRemeasurement-grid_'+id+'-stackContainer');
