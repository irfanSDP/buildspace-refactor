define('buildspace/apps/PostContract/PostContractClaim/PostContractClaimClaimGrid',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    'dojo/_base/html',
    "dijit/focus",
    ...
    var PostContractClaimClaimGrid = declare('buildspace.apps.PostContract.PostContractClaim.PostContractClaimClaimEnhancedGrid', EnhancedGrid, {
        ...
        doApplyCellEdit: function(val, rowIdx, inAttrName){
            ...
            var postFunc = function(){
                ...
                pb.show().then(function(){
                    dojo.xhrPost({
                        url: "postContractClaim/claimUpdate",
                        content: {
                            id: item.id,
                        ...
                        load: function(resp) {
                            if(resp.success){
                                ...
                                if(item.status == LOCKED){
                                    dijit.byId('postContractClaimClaim-'+self.postContractClaim.id+'AddRow-button')._setDisabledAttr(false);
                                }else if(item.status == IN_PROGRESS){
                                    dijit.byId('postContractClaimClaim-'+self.postContractClaim.id+'AddRow-button')._setDisabledAttr(true);
                                }
                                ...
                                var claimQuery = dojo.xhrGet({
                                    ...
                                    content: {id: self.postContractClaim.id},
                                    handleAs: "json"
                                });
                                claimQuery.then(function(status){
                                    ...
                                    unitQuery.then(function(uom){
                                        self.postContractClaimItemContainer.createPostContractClaimItemGrid(uom, status, false);
                                    });
                                });
                            }
                            pb.hide();
        ...
        addNewClaim: function(_csrf_token){
            ...
            dojo.xhrGet({
                ...
                var msg = status.count == 0 ? nls.confirmAddFirstPostContractNewClaimTitleMsg :  nls.confirmAddPostContractNewClaimTitleMsg;

                new buildspace.dialog.confirm(nls.addNewClaim, msg, 90, 380, function() {
                    pb.show().then(function(){
                        dojo.xhrPost({
                            url: "postContractClaim/claimAdd",
                            content: { id: self.postContractClaim.id, _csrf_token:_csrf_token },
                            handleAs: 'json',
                            load: function(resp) {
                                if(resp.success){
                                    store.close();
        ...
        deleteRow: function(){
            ...
            if(this.selection.selectedIndex > -1 && item && !isNaN(parseInt(item.id[0])) && item.can_be_deleted[0]) {
                ...
                var xhrArgs = {
                    url: "postContractClaim/claimDelete",
                ...
                new buildspace.dialog.confirm(nls.deletePostContractClaimClaimDialogBoxTitle, nls.deletePostContractClaimClaimDialogBoxMsg, 90, 380, function() {
                    pb.show().then(function(){
        ...
        changeCurrentViewingRevision: function(item){
            ...
            pb.show().then(function(){
                dojo.xhrPost({
                    url: "postContractClaim/viewClaimRevision",
                    content: { id: item.id, _csrf_token: item._csrf_token },
                    handleAs: 'json',
                    load: function(resp) {
                        if(resp.success){
        ...
        disableToolbarButtons: function(isDisable){
            var deleteButton = dijit.byId('postContractClaimClaim-'+this.postContractClaim.id+'DeleteRow-button');
            if(deleteButton) deleteButton._setDisabledAttr(isDisable);
        }
    });

    return declare('buildspace.apps.PostContract.PostContractClaim.PostContractClaimClaim', dijit.layout.BorderContainer, {
        ...
        postCreate: function(){
            ...
            var url = "postContractClaim/getClaimList/id/"+this.postContractClaim.id;
            ...
            var _csrf_token = this.claimStatus._csrf_token,
                grid = new PostContractClaimClaimGrid({
            ...
            if(!this.locked)
            {
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'postContractClaimClaim-'+this.postContractClaim.id+'AddRow-button',
                        label: nls.addNewClaim,
                        iconClass: "icon-16-container icon-16-add",
                        disabled: (this.postContractClaim.status[0] == buildspace.apps.PostContract.ProjectStructureConstants.STATUS_APPROVED && this.claimStatus.can_add_new_claim) ? false : true,
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'postContractClaimClaim-'+this.postContractClaim.id+'DeleteRow-button',
                        label: nls.deleteRow,
