define('buildspace/apps/PostContract/VariationOrder/buildUpQuantitySummary',[
    'dojo/_base/declare',
    "dojo/dom-style",
    "dijit/_WidgetBase",
    ...
    return declare("buildspace.apps.PostContract.VariationOrder.buildUpQuantitySummary", [_WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _WidgetsInTemplateMixin], {
            ...
            if(checked != this.applyConversionFactor){
                ...
                pb.show().then(function(){
                    dojo.xhrPost({
                        ...
                        content: { id: self.itemId, value: checked, _csrf_token: self._csrf_token, type: self.type },
                        handleAs: 'json',
                        load: function(resp) {
                            self.conversionFactorAmount = resp.conversion_factor_amount;
                            self.conversionFactorAmountNode.set('value', self.conversionFactorAmount);
            ...
            if(amount != this.conversionFactorAmount){
                ...
                pb.show().then(function(){
                    dojo.xhrPost({
                        ...
                        content: { id: self.itemId, val: amount, token: 'amount', _csrf_token: self._csrf_token, type: self.type },
                        handleAs: 'json',
                        load: function(resp) {
                            self.conversionFactorAmount = resp.conversion_factor_amount;
                            self.conversionFactorAmountNode.set('value', self.conversionFactorAmount);
        ...
        submitConversionFactorOperator: function(){
            ...
            if(operator != this.conversionFactorOperator){
                ...
                pb.show().then(function(){
                    dojo.xhrPost({
                        url: 'variationOrder/buildUpSummaryConversionFactorUpdate',
                        content: { id: self.itemId, val: operator, token: 'operator', _csrf_token: self._csrf_token, type: self.type },
                        handleAs: 'json',
                        load: function(resp) {
                            self.conversionFactorOperator = resp.conversion_factor_operator;
                            self.conversionFactorOperatorNode.set('value', self.conversionFactorOperator);
            ...
            if(roundingType != this.roundingType){
                ...
                pb.show().then(function(){
                    dojo.xhrPost({
                        ...
                        content: { id: self.itemId, rounding_type: roundingType, _csrf_token: self._csrf_token, type: self.type },
                        handleAs: 'json',
                        load: function(resp) {
