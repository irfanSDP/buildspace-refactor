define('buildspace/apps/PostContract/VariationOrder/BillDialog',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/connect",
    "dojo/aspect",
    ...
    var BillGrid = declare('buildspace.apps.PostContract.VariationOrder.BillGrid', EnhancedGrid, {
        type: null,
        ...
        canEdit: function(inCell, inRowIndex){
            if(this._canEdit && inCell != undefined){
                var self = this,
                    item = this.getItem(inRowIndex),
                    field = inCell.field;
                if (((item.hasOwnProperty('level') && parseInt(String(item.level))) < 1 || String(item.id) == buildspace.constants.GRID_LAST_ROW) && !this.locked ){
        ...
        omitBillItems: function(typeUnit, isLastItem){
            var self = this,
                ...
                xhrArgs = {
                    url: 'variationOrder/omitBillItems',
                    content: {
            ...
            if(self.selectedItemIds.length == 0){
                ...
                buildspace.dialog.confirm(nls.confirmation,content,90,380, onYes);
            }else{
                pb.show().then(function(){
                    dojo.xhrPost(xhrArgs);
                });
        ...
        saveTotalUnit: function(){
            var self = this,
                ...
                xhrArgs = {
                    url: 'variationOrder/totalUnitUpdate',
                    content: {
                        vid: self.variationOrderItem.id,
    ...
    var BillGridContainer = declare('buildspace.apps.PostContract.VariationOrder.BillGridContainer', dijit.layout.BorderContainer, {
        stackContainerTitle: '',
        ...
        postCreate: function(){
            ...
            var container = dijit.byId('variationOrderBillGrid-'+this.variationOrderItem.id+'-stackContainer');
            if(container){
    ...
    return declare('buildspace.apps.PostContract.VariationOrder.BillDialog', dijit.Dialog, {
        ...
        buildRendering: function(){
            ...
            var self = this, updateUrl, type,  store, structure, title, formatter = new GridFormatter();
                ...
                gridOpts: {
                    ...
                    onRowDblClick: function(e){
                        if(self.type == "omit_from_bill"){
                            ...
                            if(parseInt(String(typeUnit.level)) > 0){
