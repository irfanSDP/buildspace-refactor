define('buildspace/apps/PostContract/VariationOrder/buildUpQuantityGrid',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/array",
    "dojo/dom-attr",
    ...
    'dojo/i18n!buildspace/nls/PostContract'], function(declare, lang, array, domAttr, connect, TooltipDialog, popup, Menu, Selector, Rearrange, FormulatedColumn, evt, keys, currency, focusUtil, html, xhr, PopupMenuItem, Select, Textarea, FormulaTextBox, nls ) {

    var BuildUpQuantityGrid = declare('buildspace.apps.PostContract.VariationOrder.BuildUpQuantity.grid', dojox.grid.EnhancedGrid, {
        ...
        addRow: function(rowIndex){
            ...
            if(!isNaN(parseInt(String(itemBefore.id)))){
                content = { before_id: itemBefore.id, type: self.type, _csrf_token:itemBefore._csrf_token, relation_id: itemBefore.relation_id };
            }else{
                var prevItemId = (rowIndex > 0) ? self.getItem(rowIndex-1).id : 0;
                content = { id: itemBefore.id, type: self.type, prev_item_id: prevItemId, relation_id: itemBefore.relation_id, _csrf_token:itemBefore._csrf_token }
            }
            pb.show();
        ...
        pasteItem: function(rowIndex){
            ...
            var prevItemId = (String(targetItem.id) == buildspace.constants.GRID_LAST_ROW && rowIndex > 0) ? self.getItem(rowIndex-1).id : 0;
        ...
        disableToolbarButtons: function(isDisable, buttonsToEnable){
            var addRowBtn = dijit.byId('variationOrderBuildUpGrid-'+this.VariationOrderItem.id+'AddRow-button');
            var deleteRowBtn = dijit.byId('variationOrderBuildUpGrid-'+this.VariationOrderItem.id+'DeleteRow-button');
            ...
            if(isDisable && buttonsToEnable instanceof Array ){
                ...
                dojo.forEach(buttonsToEnable, function(label){
                    var btn = dijit.byId('variationOrderBuildUpGrid-'+_this.VariationOrderItem.id+label+'Row-button');
    ...
    return declare('buildspace.apps.PostContract.VariationOrder.BuildUpQuantityGrid', dijit.layout.BorderContainer, {
            ...
            if(!disableButton){
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'variationOrderBuildUpGrid-'+this.VariationOrderItem.id+'AddRow-button',
                        label: nls.addRow,
                        iconClass: "icon-16-container icon-16-add",
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'variationOrderBuildUpGrid-'+this.VariationOrderItem.id+'DeleteRow-button',
                        label: nls.deleteRow,
                        iconClass: "icon-16-container icon-16-delete",
                        disabled: disableButton ? disableButton : grid.selection.selectedIndex > -1 ? false : true,
                        onClick: function(){
