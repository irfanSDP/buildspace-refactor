define(["dojo/_base/declare", './Builder', "buildspace/widget/grid/cells/Formatter", 'dojo/i18n!buildspace/nls/Tendering'], function(declare, Builder, GridFormatter, nls){
    return declare('buildspace.apps.TenderingReport', buildspace.apps._App, {
        win: null,
        init: function(args){
            this.win = new buildspace.widget.Window({
                title: nls.tenderingReport + ' > ' + nls.projectList,
                onClose: dojo.hitch(this, "kill")
            });

            var formatter = GridFormatter();

            this.projectListing = new buildspace.apps.TenderingReport.ProjectListing({
                region: 'center',
                gridOpts: {
                    structure: [
