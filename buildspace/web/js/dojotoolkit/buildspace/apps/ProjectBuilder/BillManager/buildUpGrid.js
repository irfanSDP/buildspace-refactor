define('buildspace/apps/ProjectBuilder/BillManager/buildUpGrid',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/array",
    "dojo/dom-attr",
...
], function(declare, lang, array, domAttr, Menu, Selector, Rearrange, FormulatedColumn, evt, keys, currency, focusUtil, html, xhr, PopupMenuItem, Select, Textarea, FormulaTextBox, popup, TooltipDialog, ImportResourceDialog, nls){

    var BuildUpGrid = declare('buildspace.apps.ProjectBuilder.BillManager.BuildUp.grid', dojox.grid.EnhancedGrid, {
        itemId: -1,
        resource: null,
        BQItem: null,
        ...
        postCreate: function(){
            ...
            this.on("RowContextMenu", function(e){
                ...
                self.contextMenu(e);
                if(item && ! self.disableEditingMode){
                    if (item.id > 0) {
                        self.disableToolbarButtons(false);
                    } else {
                        self.disableToolbarButtons(true, ['Import']);
                    }
        ...
        addRow: function(rowIndex){
            ...
            if(itemBefore.id > 0){
                ...
                content = { id: itemBefore.id, prev_item_id: prevItemId, relation_id: itemBefore.relation_id, resource_id: self.resource.id, _csrf_token:itemBefore._csrf_token }
            }
            pb.show();
            dojo.xhrPost({
                url: this.addUrl,
        ...
        disableToolbarButtons: function(isDisable, buttonsToEnable){
            var addRowBtn = dijit.byId('builUpGrid-'+this.resource.id+'_'+this.BQItem.id+'AddRow-button');
            var deleteRowBtn = dijit.byId('builUpGrid-'+this.resource.id+'_'+this.BQItem.id+'DeleteRow-button');
            var importBtn = dijit.byId('builUpGrid-'+this.resource.id+'_'+this.BQItem.id+'ImportRow-button');
            ...
            if(isDisable && buttonsToEnable instanceof Array ){
                ...
                dojo.forEach(buttonsToEnable, function(label){
                    var btn = dijit.byId('builUpGrid-'+_this.resource.id+'_'+_this.BQItem.id+label+'Row-button');
                    btn._setDisabledAttr(false);
                })
            }
        },
        updateTotalBuildUp: function(totalBuildUp){
            var accContainer = dijit.byId('accPane-'+this.resource.id+'-'+this.BQItem.id);
    ...
    return declare('buildspace.apps.ProjectBuilder.BillManager.BuildUpGrid', dijit.layout.BorderContainer, {
        ...
        postCreate: function(){
            ...
            var grid = this.grid = new BuildUpGrid(this.gridOpts);
            var toolbar = new dijit.Toolbar({region: "top", style:"padding:2px;border-bottom:none;width:100%;"});

            toolbar.addChild(
                new dijit.form.Button({
                    id: 'builUpGrid-'+this.resource.id+'_'+this.BQItem.id+'AddRow-button',
                    label: nls.addRow,
                    iconClass: "icon-16-container icon-16-add",
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'builUpGrid-'+this.resource.id+'_'+this.BQItem.id+'DeleteRow-button',
                    label: nls.deleteRow,
                    iconClass: "icon-16-container icon-16-delete",
                    disabled: grid.selection.selectedIndex > -1 ? false : true,
                    onClick: function(){
            ...
            toolbar.addChild(
                new dijit.form.Button({
                    id: 'builUpGrid-'+this.resource.id+'_'+this.BQItem.id+'ImportRow-button',
