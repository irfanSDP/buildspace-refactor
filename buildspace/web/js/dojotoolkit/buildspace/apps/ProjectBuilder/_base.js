define(["dojo/_base/declare", './Builder', "buildspace/widget/grid/cells/Formatter", 'dojo/i18n!buildspace/nls/ProjectBuilder'], function(declare, Builder, GridFormatter, nls){
    return declare('buildspace.apps.ProjectBuilder', buildspace.apps._App, {
        win: null,
        init: function(args){
            this.win = new buildspace.widget.Window({
                title: nls.ProjectBuilder + ' > ' + nls.projectList,
                onClose: dojo.hitch(this, "kill")
            });

            var formatter = GridFormatter();

            this.projectListing = new buildspace.apps.ProjectBuilder.ProjectListing({
                region: 'center',
                gridOpts: {
                    structure: [
