define('buildspace/apps/ProjectBuilder/exportItemDialog',[
    'dojo/_base/declare',
    'dojo/_base/lang',
    "dojo/_base/connect",
    "dojo/when",
...
], function(declare, lang, connect, when, html, dom, keys, domStyle, Form, _WidgetBase, _OnDijitClickMixin, _TemplatedMixin, _WidgetsInTemplateMixin, ValidationTextBox, GridFormatter, IndirectSelection, nls, ToggleButton, RadioButton){
    ...
    var ExportBillForm = declare('buildspace.apps.ProjectBuilder.ExportBillForm', [Form,
        ...
        submit: function(){
            if(this.validate()){
                ...
                buildspace.windowOpen('POST', 'billManagerExportFile/exportExcelByElement', {
                    filename:filename,
                    id: this.bill.id[0],
                    eids: values.eids,
                    wr: this.withRate,
                    wq: this.withQuantity,
                    wbr: this.withBillRef,
    ...
    var ExportItemGrid = declare('buildspace.apps.ProjectBuilder.ExportItemGrid', dojox.grid.EnhancedGrid, {
        ...
        exportBill: function(withRate, withQuantity, withBillRef){
    ...
    var ExportGridContainer = declare('buildspace.apps.ProjectBuilder.ExportItemGridContainer', dijit.layout.BorderContainer, {
        ...
        postCreate: function(){
            ...
            if(this.type != 'tree'){
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        ...
                        onClick: function(){
                            grid.exportBill(self.withRate, self.withQuantity, self.withBillRef);
                        }
                    })
                );
            ...
            var container = dijit.byId('ProjectBuilder-item_export_'+this.bill.id+'-stackContainer');
            if(container){
    ...
    return declare('buildspace.apps.ProjectBuilder.ExportItemDialog', dijit.Dialog, {
        ...
        createContent: function(){
            ...
            var formatter = new GridFormatter(),
                store = dojo.data.ItemFileWriteStore({
                    url: "billManagerExportFile/getBillElementList/id/"+self.bill.id
                }),
        ...
        createItemGrid: function(element){
            ...
            var store = new dojo.data.ItemFileWriteStore({
                url:"billManagerExportFile/getBillItemList/id/"+element.id
            });

            var grid = ExportGridContainer({
        ...
        makeGridContainer: function(content, title){
            var id = this.bill.id;
            var stackContainer = dijit.byId('ProjectBuilder-item_export_'+id+'-stackContainer');
            if(stackContainer){
                dijit.byId('ProjectBuilder-item_export_'+id+'-stackContainer').destroyRecursive();
            }

            stackContainer = new dijit.layout.StackContainer({
                style:'width:100%;height:100%;border:0px;',
                region: "center",
                id: 'ProjectBuilder-item_export_'+id+'-stackContainer'
            });
            ...
            var controller = new dijit.layout.StackController({
                region: "top",
                containerId: 'ProjectBuilder-item_export_'+id+'-stackContainer'
            });
            ...
            dojo.subscribe('ProjectBuilder-item_export_'+id+'-stackContainer-selectChild',"",function(page){
                var widget = dijit.byId('ProjectBuilder-item_export_'+id+'-stackContainer');
                if(widget){
