...
], function(declare, lang, array, domAttr, Menu, Selector, Rearrange, FormulatedColumn, evt, keys, currency, number, focusUtil, html, xhr, PopupMenuItem, Select, Textarea, FormulaTextBox, nls){

    var Grid = declare('buildspace.apps.scheduleOfQuantity.BuildUp.Grid', dojox.grid.EnhancedGrid, {
        type: null,
        scheduleOfQuantityItem: null,
        ...
        postCreate: function(){
            ...
            if(this.type == 'manual' && this.editable){
        ...
        disableToolbarButtons: function(isDisable, buttonsToEnable){
            var addRowBtn = dijit.byId('builUpGrid-'+this.type+'_'+this.scheduleOfQuantityItem.id+'AddRow-button');
            var deleteRowBtn = dijit.byId('builUpGrid-'+this.type+'_'+this.scheduleOfQuantityItem.id+'DeleteRow-button');
            ...
            if(isDisable && buttonsToEnable instanceof Array ){
                ...
                dojo.forEach(buttonsToEnable, function(label){
                    var btn = dijit.byId('builUpGrid-'+_this.type+'_'+_this.scheduleOfQuantityItem.id+label+'Row-button');
                    btn._setDisabledAttr(false);
                })
            }
        },
        updateTotalBuildUp: function(totalBuildUp){
            var accContainer = dijit.byId('accPane-'+this.type+'_'+this.scheduleOfQuantityItem.id);
            var title = this.type == 'manual' ? nls.manualItems : nls.importedItems;
            accContainer.set('title', title+'<span style="color:blue;float:right;">'+number.format(totalBuildUp, {places:2})+'&nbsp;'+this.scheduleOfQuantityItem.uom_symbol+'</span>');
        }
    });

    return declare('buildspace.apps.ScheduleOfQuantity.BuildUpQuantityGrid', dijit.layout.BorderContainer, {
        ...
        postCreate: function(){
            ...
            lang.mixin(this.gridOpts, {
                type: this.type,
                scheduleOfQuantityItem: this.scheduleOfQuantityItem,
                region:"center"
            });

            var grid = this.grid = new Grid(this.gridOpts);

            if(this.type == 'manual' && this.gridOpts.editable){
                var toolbar = new dijit.Toolbar({region: "top", style:"padding:2px;border-bottom:none;width:100%;"});

                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'builUpGrid-'+this.type+'_'+this.scheduleOfQuantityItem.id+'AddRow-button',
                        label: nls.addRow,
                        iconClass: "icon-16-container icon-16-add",
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'builUpGrid-'+this.type+'_'+this.scheduleOfQuantityItem.id+'DeleteRow-button',
                        label: nls.deleteRow,
                        iconClass: "icon-16-container icon-16-delete",
                        disabled: grid.selection.selectedIndex > -1 ? false : true,
                        onClick: function(){
