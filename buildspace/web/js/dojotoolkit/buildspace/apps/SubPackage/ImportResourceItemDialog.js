define('buildspace/apps/SubPackage/ImportResourceItemDialog',[
    'dojo/_base/declare',
    'dojo/aspect',
    'dojo/_base/lang',
    "dojo/_base/connect",
    ...
    var ImportResourceGrid = declare('buildspace.apps.SubPackage.ImportResourceItemGrid', dojox.grid.EnhancedGrid, {
        type: null,
        itemId: 0,
        dialogWidget: null,
        _csrf_token: null,
        ...
        constructor: function(args){
            ...
            this.connects = [];
            if(args.type == 'tree'){
                this.urlGetDescendantIds = 'subPackage/getResourceDescendants';
        ...
        disableToolbarButtons: function(isDisable){
            var importBtn = dijit.byId('SubPackages-ImportResourceItemsGrid-'+this.subPackage.id+'Import-button');
        ...
        import: function(trade){
            var self = this,
                title = nls.removeAllResourceItemsFrom+' '+buildspace.truncateString(self.subPackage.name, 25),
                msg = nls.areYouSureToRemoveAllResourceItemsUnderTrade+' <b>'+buildspace.truncateString(trade.description, 40)+'</b>?',
            ...
            pb.show();
            var xhrArgs = {
                url: 'subPackage/importResourceItems',
    ...
    var ImportGridContainer = declare('buildspace.apps.SubPackage.ImportResourceGridContainer', dijit.layout.BorderContainer, {
        ...
        postCreate: function(){
            ...
            if(self.type == 'tree'){
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'SubPackages-ImportResourceItemsGrid-'+self.subPackage.id+'Import-button',
            ...
            var container = dijit.byId('SubPackages-resource_items_import_'+self.subPackage.id+'-stackContainer');
            if(container){
    ...
    return declare('buildspace.apps.SubPackage.ImportResourceItemDialog', dijit.Dialog, {
        style:"padding:0px;margin:0px;",
        title: nls.importResourceItems,
        subPackage: null,
        subPackageGridStore: null,
        buildRendering: function(){
            ...
            this.title = nls.importResourceItems+' :: '+buildspace.truncateString(this.subPackage.name, 45);
            this.inherited(arguments);
        },
        postCreate: function(){
        ...
        createContent: function(){
            ...
            var formatter = new GridFormatter(),
                store = dojo.data.ItemFileWriteStore({
                    url: "subPackage/getResourceList/id/"+this.subPackage.id,
                    clearOnClose: true
                });

            var gridContainer = this.makeGridContainer(ImportGridContainer({
                stackContainerTitle: nls.resources,
                pageId: 'sub_packages_import-page_resource-'+this.subPackage.id,
        ...
        createTradeGrid: function(resource){
            var self = this, formatter = GridFormatter(),
                store = new dojo.data.ItemFileWriteStore({
                    url:"subPackage/getTradeList/id/"+self.subPackage.id+"/rid/"+resource.id,
                    clearOnClose: true
                });

            var grid = ImportGridContainer({
                stackContainerTitle: resource.name,
                pageId: 'sub_packages_import-page_resource_trade-'+this.subPackage.id+'_'+resource.id,
        ...
        createItemGrid: function(trade, resource){
            var self = this, formatter = GridFormatter(),
                store = new dojo.data.ItemFileWriteStore({
                    url:"subPackage/getResourceItemList/id/"+self.subPackage.id+"/rid/"+resource.id+"/tid/"+trade.id,
                    clearOnClose: true
                });

            var grid = ImportGridContainer({
                stackContainerTitle: trade.description,
                pageId: 'sub_packages_import-page_resource_item-'+this.subPackage.id+'_'+trade.id,
                subPackage: self.subPackage,
                type: 'tree',
                trade: trade,
                gridOpts: {
        ...
        makeGridContainer: function(content){
            var id = this.subPackage.id,
                stackContainer = dijit.byId('SubPackages-resource_items_import_'+id+'-stackContainer');
            if(stackContainer){
                dijit.byId('SubPackages-resource_items_import_'+id+'-stackContainer').destroyRecursive();
            }

            stackContainer = new dijit.layout.StackContainer({
                style:'width:100%;height:100%;border:0px;',
                region: "center",
                id: 'SubPackages-resource_items_import_'+id+'-stackContainer'
            });
            ...
            var controller = new dijit.layout.StackController({
                region: "top",
                containerId: 'SubPackages-resource_items_import_'+id+'-stackContainer'
            });
            ...
            dojo.subscribe('SubPackages-resource_items_import_'+id+'-stackContainer-selectChild',"",function(page){
                var widget = dijit.byId('SubPackages-resource_items_import_'+id+'-stackContainer');
                if(widget){
