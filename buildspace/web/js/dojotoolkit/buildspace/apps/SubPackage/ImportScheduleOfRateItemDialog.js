define('buildspace/apps/SubPackage/ImportScheduleOfRateItemDialog',[
    'dojo/_base/declare',
    'dojo/aspect',
    'dojo/_base/lang',
    "dojo/_base/connect",
    ...
    var ImportScheduleOfRateGrid = declare('buildspace.apps.SubPackage.ImportScheduleOfRateItemGrid', dojox.grid.EnhancedGrid, {
        type: null,
        itemId: 0,
        dialogWidget: null,
        _csrf_token: null,
        ...
        constructor: function(args){
            ...
            this.connects = [];
            if(args.type == 'tree'){
                this.urlGetDescendantIds = 'subPackage/getScheduleOfRateDescendants';
        ...
        disableToolbarButtons: function(isDisable){
            var importBtn = dijit.byId('SubPackages-ImportScheduleOfRateItemsGrid-'+this.subPackage.id+'Import-button');
        ...
        import: function(trade){
            var self = this,
                title = nls.removeAllScheduleOfRateItemsFrom+' '+buildspace.truncateString(self.subPackage.name, 25),
                msg = nls.areYouSureToRemoveAllScheduleOfRateItemsUnderTrade+' <b>'+buildspace.truncateString(trade.description, 40)+'</b>?',
            ...
            pb.show();
            var xhrArgs = {
                url: 'subPackage/importScheduleOfRateItems',
                content: { id: self.subPackage.id, ids: [itemIds], tid: trade.id, _csrf_token: self._csrf_token },
                handleAs: 'json',
                load: function(data) {
                    if(data.success){
    ...
    var ImportGridContainer = declare('buildspace.apps.SubPackage.ImportScheduleOfRateGridContainer', dijit.layout.BorderContainer, {
        ...
        postCreate: function(){
            ...
            if(self.type == 'tree'){
                ...
                toolbar.addChild(
                    new dijit.form.Button({
                        id: 'SubPackages-ImportScheduleOfRateItemsGrid-'+self.subPackage.id+'Import-button',
                        label: nls.importToSubPackage,
                        iconClass: "icon-16-container icon-16-import",
                        onClick: function(){
                            grid.import(self.trade);
                        }
                    })
                );
            ...
            var container = dijit.byId('SubPackages-scheduleOfRate_items_import_'+self.subPackage.id+'-stackContainer');
            if(container){
    ...
    return declare('buildspace.apps.SubPackage.ImportScheduleOfRateItemDialog', dijit.Dialog, {
        ...
        buildRendering: function(){
            ...
            this.title = nls.importScheduleOfRateItems+' :: '+buildspace.truncateString(this.subPackage.name, 45);
            this.inherited(arguments);
        },
        postCreate: function(){
        ...
        createContent: function(){
            ...
            var formatter = new GridFormatter(),
                store = dojo.data.ItemFileWriteStore({
                    url: "subPackage/getScheduleOfRateList/id/"+this.subPackage.id,
                    clearOnClose: true
                });

            var gridContainer = this.makeGridContainer(ImportGridContainer({
                stackContainerTitle: nls.scheduleOfRates,
                pageId: 'sub_packages_import-page_scheduleOfRate-'+this.subPackage.id,
        ...
        createTradeGrid: function(scheduleOfRate){
            var self = this, formatter = GridFormatter(),
                store = new dojo.data.ItemFileWriteStore({
                    url:"subPackage/getScheduleOfRateTradeList/id/"+self.subPackage.id+"/sorid/"+scheduleOfRate.id,
                    clearOnClose: true
                });

            var grid = ImportGridContainer({
                stackContainerTitle: scheduleOfRate.name,
                pageId: 'sub_packages_import-page_scheduleOfRate_trade-'+this.subPackage.id+'_'+scheduleOfRate.id,
        ...
        createItemGrid: function(trade, scheduleOfRate){
            var self = this, formatter = GridFormatter(),
                store = new dojo.data.ItemFileWriteStore({
                    url:"subPackage/getScheduleOfRateItemList/id/"+self.subPackage.id+"/sorid/"+scheduleOfRate.id+"/tid/"+trade.id,
                    clearOnClose: true
                });

            var grid = ImportGridContainer({
                stackContainerTitle: trade.description,
                pageId: 'sub_packages_import-page_scheduleOfRate_item-'+this.subPackage.id+'_'+trade.id,
                subPackage: self.subPackage,
                type: 'tree',
                trade: trade,
                gridOpts: {
        ...
        makeGridContainer: function(content){
            var id = this.subPackage.id,
                stackContainer = dijit.byId('SubPackages-scheduleOfRate_items_import_'+id+'-stackContainer');
            if(stackContainer){
                dijit.byId('SubPackages-scheduleOfRate_items_import_'+id+'-stackContainer').destroyRecursive();
            }

            stackContainer = new dijit.layout.StackContainer({
                style:'width:100%;height:100%;border:0px;',
                region: "center",
                id: 'SubPackages-scheduleOfRate_items_import_'+id+'-stackContainer'
            });

            stackContainer.addChild(new dijit.layout.ContentPane({
                title: nls.scheduleOfRates,
                content: content,
                grid: content.grid
            }));
            ...
            var controller = new dijit.layout.StackController({
                region: "top",
                containerId: 'SubPackages-scheduleOfRate_items_import_'+id+'-stackContainer'
            });
            ...
            dojo.subscribe('SubPackages-scheduleOfRate_items_import_'+id+'-stackContainer-selectChild',"",function(page){
                var widget = dijit.byId('SubPackages-scheduleOfRate_items_import_'+id+'-stackContainer');
                if(widget){
