define('buildspace/apps/CostData/LinkBillItems/LinkBillItemsGrid',[
    'dojo/_base/declare',
    "dojo/aspect",
    'dojo/number',
    'dojo/currency',
    ...
    var Grid = declare('buildspace.apps.CostData.LinkBillItems.LinkBillItemsGrid', EnhancedGrid, {
        style: "border:none;",
        region: 'center',
        keepSelection: true,
        rowSelector: '0px',
    ...
    return declare('buildspace.apps.CostData.LinkBillItems.LinkBillItemsContainer', dijit.layout.BorderContainer, {
        region: "center",
        title: '',
        ...
        addToolbar: function(){
            ...
            toolbar.addChild( self.conversionFactorInput = new TextBox({
                style: 'width:50px;float:right;padding:3px;'
            }));

            self.conversionFactorInput.set('value', number.parse(self.moduleContainer.conversionFactor));

            toolbar.addChild(new dijit.layout.ContentPane({
                style: 'padding-left: 10px; padding-right: 10px; padding-top: 3px; float: right;',
                content: nls.conversionFactor
            }));
        ...
        addBreakdownGrid: function(){
            ...
            var grid = self.grid = Grid({
                ...
                selectedItemIds: self.moduleContainer.selectedItemIds,
                store: new dojo.data.ItemFileWriteStore({
                    clearOnClose:true,
                    url:"costData/getBillItemList/costData/"+self.costData.id+'/id/'+self.item.id
                })
            });
        ...
        save: function() {
            ...
            pb.show().then( function() {
                dojo.xhrPost( {
                    url: self.moduleContainer.updateUrl,
                    handleAs: "json",
                    content: {
                        cost_data_id: self.costData.id,
                        conversion_factor: dojo.trim(self.conversionFactorInput.get('value')),
                        "selectedIds": self.grid.selectedItemIds.join(),
                        "deselectedIds": self.grid.deselectedItemIds.join(),
                        "_csrf_token": self._csrf_token
                    },
                    load: function(data) {
                        self.moduleContainer.conversionFactor = data.conversion_factor;
                        self.moduleContainer.selectedItemIds = data.bill_item_ids;
                        self.moduleContainer.deselectedItemIds = [];

                        self.conversionFactorInput.set('value', number.parse(self.moduleContainer.conversionFactor));

                        self.grid.refreshGrid();
                        self.grid.selectedItemIds = self.moduleContainer.selectedItemIds;
                        self.grid.deselectedItemIds = self.moduleContainer.deselectedItemIds;
