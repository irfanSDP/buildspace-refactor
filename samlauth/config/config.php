...
// -- safe env helper + some derived values
$env = function ($key, $default = null) {
    $v = getenv($key);
    return ($v !== false && $v !== '') ? $v : $default;
};

$rootDomain = $env('SAMLAUTH_ROOT_DOMAIN', 'buildspace.local');           // e.g. buildspace.local
$cookieName = $env('SAMLAUTH_COOKIE_NAME', 'BuildSpaceEProject-dev');        // non-empty fallback

// Platform-safe temp dir
$tempDir = (DIRECTORY_SEPARATOR === '\\')
    ? 'C:/laragon/tmp/simplesaml'
    : '/tmp/simplesaml';

// Logging handler: syslog on *nix, file on Windows
$loggingHandler = (stripos(PHP_OS, 'WIN') === 0) ? 'file' : 'syslog';


$envDir = realpath(__DIR__.'/..'); // path to samlauth root
...
if (class_exists(Dotenv::class)) {
    if (method_exists(Dotenv::class, 'createImmutable')) {
        ...
        // v3.x can be called in two ways depending on sub-packages present
        if (class_exists('Dotenv\Repository\RepositoryBuilder')) {
            // v3 with RepositoryBuilder available
            $repo = Dotenv\Repository\RepositoryBuilder::createWithDefaultAdapters()
                ->immutable()
                ->make();
            Dotenv::create($repo, $envDir)->safeLoad();
        } else {
            // plain v3
            Dotenv::create($envDir)->safeLoad();
        }
    }
}

// $rootCandidates = [
//     // for etender.buildspace.local
//     'C:/laragon/www/etender',
//     // for buildspace main app
//     'C:/laragon/www/buildspace',
//     // fallback to samlauth root
//     dirname(__DIR__),
// ];
...
$config = array(
    'version'              => getenv('SAMLAUTH_VERSION'),
    'application_title'    => trim(getenv('SAMLAUTH_APPLICATION_TITLE'), '"'),
    'login_form_title'     => trim(getenv('SAMLAUTH_LOGIN_FORM_TITLE'), '"'),
    'forgot_password_link' => 'https://' . getenv('EPROJECT_HOST') . '/forgot_password',
    'path_eproject'        => 'https://' . getenv('EPROJECT_HOST'),
    'sign_up_page'         => true,
    'view_tenders'         => false,
     ...
     * reverse proxy).
     */
    // 'baseurlpath' => 'https://' . getenv('SAMLAUTH_HOST') . '/',
    'baseurlpath' => 'https://auth.buildspace.local',
     ...
     */
    'technicalcontact_name' => getenv('TECHNICAL_CONTACT_NAME'),
    'technicalcontact_email' => getenv('TECHNICAL_CONTACT_EMAIL'),
     ...
     */
    'secretsalt' => getenv('SAMLAUTH_SECRET_SALT'),
     ...
     */
    'auth.adminpassword' => trim(getenv('SAMLAUTH_ADMIN_PASSWORD'), '"'),
     ...
     */
    // 'trusted.url.domains' => array(getenv('SAMLAUTH_HOST'), getenv('EPROJECT_HOST'), getenv('BUILDSPACE_HOST'), getenv('EDITOR_HOST'), getenv('COST_DATA_HOST')),
    ...
    //     'trusted.url.domains' => array_values(array_filter([
    //     $env('SAMLAUTH_HOST'),
    //     $env('EPROJECT_HOST'),
    //     $env('BUILDSPACE_HOST'),
    //     $env('EDITOR_HOST'),
    ...
    'trusted.url.domains' => ['auth.buildspace.local','bq.buildspace.local','etender.buildspace.local'],
     ...
     */
    'database.dsn' => 'pgsql:host=' . getenv('SAMLAUTH_DB_HOST') . ';dbname=' . getenv('SAMLAUTH_DB_DATABASE'),

    /*
     * SQL database credentials
     */
    'database.username' => getenv('SAMLAUTH_DB_USERNAME'),
    'database.password' => getenv('SAMLAUTH_DB_PASSWORD'),
    'database.options' => array(),

    /*
     * (Optional) Table prefix
     */
    'database.prefix' => 'saml',
     ...
     */
    'session.cookie.name' => getenv('SAMLAUTH_COOKIE_NAME') ?: 'SimpleSAMLSessionID',
     ...
     */
    'session.cookie.domain' => '.' . getenv('SAMLAUTH_ROOT_DOMAIN') ?: 'buildspace.local',
    ...
    'session.cookie.samesite' => 'Lax',    // or 'None' if you ever embed in iframes


    /*
     * Options to override the default settings for php sessions.
     */
    'session.phpsession.cookiename' => getenv('SAMLAUTH_COOKIE_NAME') ?: "BuildSpaceEProject-dev",
    'session.phpsession.savepath' => 'C:/laragon/tmp',
    'session.phpsession.httponly' => true,

    /*
     * Option to override the default settings for the auth token cookie
     */
    'session.authtoken.cookiename' => 'BuildSpaceEProjectAuthToken',
     ...
     * syntaxes.
     */
    // 'store.sql.dsn'                 => 'pgsql:host=' . getenv('SAMLAUTH_DB_HOST') . ';dbname=' . getenv('SAMLAUTH_DB_DATABASE'),
    'store.sql.dsn' => 'pgsql:host='
    . (getenv('SAMLAUTH_DB_HOST') ?: '127.0.0.1')
    . ';port=' . (getenv('SAMLAUTH_DB_PORT') ?: 5432)
    . ';dbname=' . (getenv('SAMLAUTH_DB_DATABASE') ?: 'saml'),

    /*
     * The username and password to use when connecting to the database.
     */
    'store.sql.username' => getenv('SAMLAUTH_DB_USERNAME') ?: 'postgres',
    'store.sql.password' => getenv('SAMLAUTH_DB_PASSWORD') ?: 'dbPassword',
     ...
     */
    'store.redis.host' => getenv('REDIS_HOST'),
    'store.redis.port' => 6379,

    /*
     * The prefix we should use on our Redis datastore.
     */
    'store.redis.prefix' => 'saml',
);
