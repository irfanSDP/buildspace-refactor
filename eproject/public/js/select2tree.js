//custom select2 tree plugin extension
(function($) {
...
function templateResultCustom(data, container) {
    if (data.element) {
        ...
        if (isSearching) {
            $switchSpn.css({"padding": "0 12px 0 12px"});
        } else if (hasChilds) {
            $switchSpn.addClass("switch-tree glyphicon");
            if ($switchSpn.hasClass("glyphicon-chevron-right"))
                $switchSpn.removeClass("glyphicon-chevron-right").addClass("glyphicon-chevron-down");
            else
                $switchSpn.removeClass("glyphicon-chevron-down").addClass("glyphicon-chevron-right");
            
            $switchSpn.css({"padding": "0 12px 0 12px","cursor": "pointer"});
        }
...
function switchAction($select, id, open, opts) {
    var children = $(".select2-results__options li[data-parent='" + id + "']");
    if (opts.hasOwnProperty('expandChildren') && opts.expandChildren) {
        //expand childs.
        children.each(function() {
            switchAction($select, $(this).attr("val"), open, opts);
        });
    }
    var parent = $(".select2-results__options li[val=" + id + "] span[class]:eq(0)");
    if (open) {
        parent.removeClass("glyphicon-chevron-right").addClass("glyphicon-chevron-down");
        children.slideDown();
    } else {
        parent.removeClass("glyphicon-chevron-down").addClass("glyphicon-chevron-right");
        children.slideUp();
    }
}
...
function open($select, opts) {
    setTimeout(function() {
        ...
        $(".switch-tree").mousedown(function() {
            switchAction($select, $(this).parent().attr("val"), $(this).hasClass("glyphicon-chevron-right"), opts);
...
function templateSelectionCustom(item) {
    ...
    if (parentsText != '') parentsText += ' <i class="glyphicon glyphicon-chevron-right"></i> ';
...
function getParentText($select, $element) {
    ...
    if (parent) {
        ...
        if (text != '') text += ' <i class="glyphicon glyphicon-chevron-right"></i> ';
        text += parent.text();
    }
    return text;
}
