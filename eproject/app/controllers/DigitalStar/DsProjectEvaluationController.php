...
class DsProjectEvaluationController extends \BaseController
{
    ...
    public function index()
    {
        ...
        return View::make('digital_star.evaluations.project.index', compact('statuses'));
    }
    ...
    public function list()
    {
        ...
        $records = $model->orderBy('ds_evaluations.start_date', 'asc')
            ->orderBy('projects.reference', 'asc')
            ->skip($limit * ($page - 1))
            ->take($limit)
            ->get();
        ...
        foreach ($records->all() as $key => $record) {
            ...
            $data[] = [
                ...
                'route:evaluation_form' => route('digital-star.evaluation.project.edit', array($record->evaluation_id, $record->form_id))
            ];
        }
    ...
    public function edit($evaluationId, $evaluationFormId)
    {
        ...
        \PCK\Helpers\Hierarchy\AdjacencyListNode::traverse($data[0], function($node) use ($evaluationForm, $readOnly) {
            if(isset($node['hasScores']) && $node['hasScores'])
            {
                ...
                {
                    $node['route:getUploads'] = route('digital-star.evaluation.project.node.uploads', array($evaluationForm->ds_evaluation_id, $evaluationForm->id, $node['nodeId']));
                    $node['route:doUpload']   = route('digital-star.evaluation.project.node.doUpload', array($evaluationForm->ds_evaluation_id, $evaluationForm->id, $node['nodeId']));
                }
            }
            return $node;
        }, '_children');
        ...
        return View::make('digital_star.evaluations.project.edit', compact('evaluation', 'evaluatedCompany', 'evaluationForm', 'project', 'data', 'readOnly', 'uploadedFiles', 'nodeIdsBySelectedScoreIds', 'excludedIds', 'score', 'remarks'));
    }
    ...
    public function update($evaluationId, $evaluationFormId)
    {
        ...
        if($evaluationForm->isSubmitted())
        {
            ...
            //return \Redirect::route('vendorPerformanceEvaluation.companyForms.approval.edit', array($evaluationForm->id));

            return \Redirect::route('digital-star.evaluation.project.edit', array($evaluationId, $evaluationForm->id));
        }
        ...
        if ($formSubmitType == 'submit')
        {
            ...
            DsEvaluationLog::logAction($evaluationForm->id, 'submitted', $role->id, $user);
