...
trait RequestVerifierProcessTrait {
    ...
    {
        return route('requestForInspection.show', array( $this->project->id, $this->id ));
    }
    ...
    public function getOnApprovedFunction()
    {
        return function()
        {
            ...
            Mailer::queueMultiple($emailRecipients, null, 'notifications.email.requestForInspection.requestPosted', trans('email.eProjectNotification'), array( 'project' => $project, 'route' => $this->getRoute(), ));

            SystemNotifier::send($systemRecipients, $this->getRoute(), 'notifications.system.requestForInspection.requestPosted', $this->createdBy);
        };
    }
