...
class NumberTextBox extends Element implements Elementable
{
    protected $validationRules = ['required', 'numeric', 'min', 'max'];
    ...
    public function getValidationErrorMessage()
    {
        ...
        $messages[$name . '.numeric']  = trans('formBuilder.mustBeValidNumber', ['label' => $this->label]);
        $messages[$name . '.min']      = trans('formBuilder.mustBeMinimum', ['label' => $this->label]);
        $messages[$name . '.max']      = trans('formBuilder.mustBeMaximum', ['label' => $this->label]); 
    ...
    public static function createNewElement($inputs)
    {
        ...
        if(trim($inputs['min']) != '')
        {
            ElementAttribute::createNewAttribute($element, 'min', trim($inputs['min']));
        }

        if(trim($inputs['max']) != '')
        {
            ElementAttribute::createNewAttribute($element, 'max', trim($inputs['max']));
        }
    ...
    public  function updateElement($inputs)
    {
        ...
        if(trim($inputs['min']) != '')
        {
            if(is_null(ElementAttribute::findAttribute($element, 'min')))
            {
                ElementAttribute::createNewAttribute($element, 'min', trim($inputs['min']));
            }
            else
            {
                ElementAttribute::updateAttribute($element, 'min', $inputs['min']);
            }
        }
        else
        {
            ElementAttribute::deleteAttribute($element, 'min');
        }

        if(trim($inputs['max']) != '')
        {
            if(is_null(ElementAttribute::findAttribute($element, 'max')))
            {
                ElementAttribute::createNewAttribute($element, 'max', trim($inputs['max']));
            }
            else
            {
                ElementAttribute::updateAttribute($element, 'max', $inputs['max']);
            }
        }
        else
        {
            ElementAttribute::deleteAttribute($element, 'max');
        }
