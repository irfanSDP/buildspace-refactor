...
class DsSetupForm extends CustomFormValidator {
    ...
    protected function postParentValidation($formData)
    {
        ...
        {
            $messageBag->add('start_date', trans('digitalStar/vendorManagement.startDateCannotBeEarlierThan', array('startDate' => $startDate->format(\Config::get('dates.submitted_at')))));
        }

        if (Carbon::parse($formData['end_date'])->gt($endDate))
        {
            $messageBag->add('end_date', trans('digitalStar/vendorManagement.startDateCannotBeLaterThan', array('endDate' => $endDate->format(\Config::get('dates.submitted_at')))));
        }
