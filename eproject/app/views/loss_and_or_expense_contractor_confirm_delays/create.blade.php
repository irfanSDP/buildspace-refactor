		...
		<article class="col-sm-12 col-md-12 col-lg-8">
			...
			<div class="jarviswidget">
				<header>
					...
					<h2>{{{ PCK\Forms\LOEContractorConfirmDelayForm::formTitle }}}</h2>
				</header>
					...
					<div class="widget-body no-padding">
						{{ Form::open(array('class' => 'smart-form')) }}
							<fieldset>
								...
								<section>
									<label class="label">Deadline to Submit the final Loss And/Or Expense Claim ({{{ $loe->project->pam2006Detail->deadline_submitting_final_claim_l_and_e }}} days from the end of the cause of delay):</label>
									<span id="new_deadline">
										@if ( Input::old('date_on_which_delay_is_over') and strtotime(Input::old('date_on_which_delay_is_over')) )
											{{{ ($calendarRepo->calculateFinalDate($loe->project, Input::old('date_on_which_delay_is_over'), $loe->project->pam2006Detail->deadline_submitting_final_claim_l_and_e)) }}}
										@else
							...
							{{ Form::hidden('deadline_days', $loe->project->pam2006Detail->deadline_submitting_final_claim_l_and_e, ['id' => 'deadline_days']) }}
						{{ Form::close() }}
					</div>
					<!-- end widget content -->
				</div>
