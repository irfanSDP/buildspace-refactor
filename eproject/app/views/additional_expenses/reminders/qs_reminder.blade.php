...
@if ( $ae->additionalExpenseClaim )
	<li>
		...
		<ul>
			...
			@if ( $lastThirdMessageType != PCK\ContractGroups\Types\Role::CLAIM_VERIFIER and $isEditor )
				<li>QS to request further particulars {{ link_to_route('aeThirdLevelMessage.create', 'here', array($ae->project_id, $ae->id)) }}.</li>
			@endif
		</ul>
	</li>
	...
	@if ( $ae->fourthLevelMessages->isEmpty() )
		<li>
			@if ( $isEditor )
				The Architect to decide to accept/reject the Contractor's application for Additional Expense. The QS to provide payment {{ link_to_route('aeFourthLevelMessage.create', 'evaluation', array($ae->project_id, $ae->id)) }}.
			@else
				The Architect to decide to accept/reject the Contractor's application for Additional Expense. The QS to provide payment evaluation.
			@endif
		</li>
	@else
		<li>
			...
			<ul>
				...
				@if ( ! $ae->additionalExpenseInterimClaim and ! $lastFourthMessageLockedStatus and ! $lastFourthMessageStatus and $lastFourthMessageType != PCK\ContractGroups\Types\Role::CLAIM_VERIFIER and $isEditor )
					<li>QS to respond {{ link_to_route('aeFourthLevelMessage.create', 'here', array($ae->project_id, $ae->id)) }}.</li>
				@endif
			</ul>
		</li>
	@endif
