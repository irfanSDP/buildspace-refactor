	...
	<div class="accordion" class="ui-accordion ui-widget ui-helper-reset" role="tablist">
		@foreach ( $ae->fourthLevelMessages as $message )
			@if ( $message->type == PCK\ContractGroups\Types\Role::INSTRUCTION_ISSUER )
				@include('additional_expenses.partials.architect_fourth_level_info_tab', array('message' => $message))
			@elseif ( $message->type == PCK\ContractGroups\Types\Role::CONTRACTOR )
				@include('additional_expenses.partials.contractor_fourth_level_info_tab', array('message' => $message))
			@endif
		@endforeach
	</div>
</div>