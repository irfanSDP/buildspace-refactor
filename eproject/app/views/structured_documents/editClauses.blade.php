...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        <li>{{ link_to_route('structured_documents.edit', $folder->name, array($project->id, $folder->id, $document->id)) }}</li>
    ...
    <article class="col-sm-12">
            ...
            <div>
                ...
                <div class="widget-body">
                            ...
                            <div class="dd scrollable" id="activeClauseList" style="min-width:100%;">
                                <ol class="dd-list root-list">
                                    @foreach($document->clauses as $clause)
                                        @include('structured_documents.clause', array('clause' => $clause, 'isTemplate' => false))
                                    @endforeach
                                </ol>
                            </div>
                        </div>
                    ...
                    <footer class="row" style="margin: 0 10px;text-align:right;">
                        <a href="{{ route('structured_documents.edit', array($project->id, $folder->id, $document->id)) }}" class="btn btn-default">{{ trans('forms.back') }}</a>
        ...
        <div data-category="templates" hidden>
            <div data-type="clause">
                @include('structured_documents.clause', array('clause' => null, 'isTemplate' => false))
            </div>
        </div>
    </article>
...
@section('js')
    ...
    <script>
        ...
        function handleDeleteClause(listItem) {
            ...
            if($(listItem).data('is_editable') != true)
            {
                ...
                $(listItem).detach();
                if(listItem.data('id')) deletedClauses.push(listItem.data('id'));
            }
        }
        ...
        $('[data-action=save]').on('click', function(){
            ...
            $.ajax({
                url: '{{ route('structured_documents.clauses.update', array($project->id, $folder->id, $document->id)) }}',
                method: 'POST',
                data: {
                    _token: '{{{ csrf_token() }}}',
