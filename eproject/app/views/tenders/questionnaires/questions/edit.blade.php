...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        @else
        <li>{{ link_to_route('projects.questionnaires.show', str_limit($company->name, 50), [$project->id, $company->id]) }}</li>
        <li>{{ link_to_route('projects.questionnaires.question.show', trans('general.questionnaire'), [$project->id, $question->id]) }}</li>
...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <h1 class="page-title txt-color-blueDark">
            <i class="fa fa-tasks"></i> @if(!isset($question)) {{{ trans('general.newQuestionnaire') }}} @else {{{ trans('forms.edit') }}} {{{ trans('general.questionnaire') }}} @endif
        </h1>
    </div>
</div>
        ...
        <div class="jarviswidget ">
            ...
            <div>
                <div class="widget-body">
                    {{ Form::open(['route' => ['projects.questionnaires.question.store', $project->id], 'class' => 'smart-form']) }}
                    <div class="row">
                        <section class="col col-xs-12 col-md-12 col-lg-12">
                            <label class="label">{{{ trans('general.question') }}} <span class="required">*</span>:</label>
                            <label class="textarea {{{ $errors->has('question') ? 'state-error' : null }}}">
                                {{ Form::textarea('question', Input::old('question', isset($question) ? $question->question : null), ['id'=>'questionnaire_question-txt', 'autofocus' => 'autofocus', 'class'=>'summernote']) }}
                            </label>
                    ...
                    <div class="row">
                        <section class="col col-xs-12 col-md-4 col-lg-4">
                            ...
                            <label class="input {{{ $errors->has('type') ? 'state-error' : null }}}">
                                <select class="select2 fill-horizontal" name="type" id="type-select">
                                    ...
                                    <option value="{{$value}}" @if($value == Input::old('type', ($question) ? $question->type : null)) selected @endif>{{{ $text }}}</option>
                                    @endforeach
                                </select>
                            </label>
                        ...
                        <section class="col col-xs-12 col-md-4 col-lg-4">
                            <?php $defaultMandatory = (isset($question) && !$question->required) ? 0 : 1; ?>
                    ...
                    <div class="row">
                        <section class="col col-xs-12 col-sm-12 col-md-12 col-lg-12">
                            <div class="custom-control custom-checkbox">
                                {{ Form::checkbox('with_attachment', 1, Input::old('with_attachment', isset($question) ? $question->with_attachment : false), ['id'=>'with_attachment-chkbox', 'class'=>'custom-control-input']) }}
                    ...
                    <?php
                    $selectedType = Input::old('type', ($question) ? $question->type : null);
                    ?>
                <div id="questionnaire-options" @if( $selectedType == PCK\ContractorQuestionnaire\Question::TYPE_MULTI_SELECT or $selectedType == PCK\ContractorQuestionnaire\Question::TYPE_SINGLE_SELECT)  @else style="display:none;" @endif>
                        <hr class="simple">
                        <div class="row">
                        ...
                        <div class="row">
                            <section class="col col-xs-12 col-md-12 col-lg-12">
                                <table class="table table-bordered table-condensed table-striped table-hover">
                                    ...
                                    <?php
                                    $questionnaireOptions = Input::old('options', (!isset($question) or empty($question->options->toArray())) ? [['option_id'=>-1, 'text'=>'', 'value'=>'']] : $question->options->toArray());
                                    ?>
                    ...
                    <footer>
                        {{ Form::hidden('id', (isset($question)) ? $question->id : -1) }}
                        ...
                        @else
                        {{ link_to_route('projects.questionnaires.question.show', trans('forms.back'), [$project->id, $question->id], ['class' => 'btn btn-default']) }}
                        @endif
...
$(document).ready(function () {
    ...
    $('#type-select').on('change',function(e){
        e.preventDefault();
        if(parseInt($(this).val()) == {{PCK\ContractorQuestionnaire\Question::TYPE_MULTI_SELECT}} || parseInt($(this).val()) == {{PCK\ContractorQuestionnaire\Question::TYPE_SINGLE_SELECT}}){
