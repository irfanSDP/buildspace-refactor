@if(\PCK\SystemModules\SystemModuleConfiguration::isEnabled(\PCK\SystemModules\SystemModuleConfiguration::MODULE_ID_DIGITAL_STAR))
    @if(\PCK\VendorManagement\VendorManagementUserPermission::hasPermission($currentUser, \PCK\VendorManagement\VendorManagementUserPermission::TYPE_DIGITAL_STAR_DASHBOARD))
        <script>
            var vendorsByDsRatingChart = new ApexCharts(document.querySelector("#vendorsByDsRatingChart"), {
                series: [],
                labels: [],
                chart: {
                    width: 500,
            ...
            renderVendorsByDsRatingChart(formInputs, 'vendorsByDsRating');

            function renderVendorsByDsRatingChart(data, identifier) {
                data.identifier = identifier;

                $.ajax({
                    url: "{{ route('digital-star.dashboard.charts') }}",
                    method: 'GET',
                    data: data,
                    success: function(response){
                        vendorsByDsRatingChart.updateOptions({
