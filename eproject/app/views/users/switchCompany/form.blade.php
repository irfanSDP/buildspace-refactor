    ...
    <div class="row">
        <div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
            <h1 class="page-title txt-color-blueDark">
                <i class="fa fa-exchange-alt"></i> {{{ trans('users.switchCompany') }}}
            </h1>
        </div>
    </div>
        ...
        <fieldset>
            <section class="col col-8">
                ...
                @if($user->isTransferable())
                    {{ Form::select('company_id', $companiesList, Input::old('company_id') ?? $user->company_id, array('class' => 'select2 fill-horizontal')) }}
                @else
                    {{{ $user->company->name }}}
                    <p>
                        <em class="invalid">{{ trans('users.cantTransferUser', array('user' => $user->name)) }}</em>
                    </p>
                @endif
            </section>
        </fieldset>
        <footer>
            <button type="submit" class="btn btn-primary" {{{ $user->isTransferable() ?: 'disabled' }}}><i class="fa fa-save"></i>&nbsp;{{ trans('forms.save') }}</button>
            <button type="button" class="btn btn-success" data-toggle="modal" data-target="#logModal"><i class="fa fa-list"></i>&nbsp;{{ trans('general.log') }}</button>
            @if(!$user->isTransferable())
                <button id="btnViewPendingTasks" class="btn btn-warning"><i class="fa fa-tasks"></i>&nbsp;{{ trans('general.pendingTasks') }}</button>
            @endif
        </footer>
    {{ Form::close() }}

    @include('templates.log_modal')
    @if(!$user->isTransferable())
...
@section('js')
    <script>
        $(document).ready(function(e) {
            ...
            $('#btnViewPendingTasks').on('click', function(e) {
                e.preventDefault();
