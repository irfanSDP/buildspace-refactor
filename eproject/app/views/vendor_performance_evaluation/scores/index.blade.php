...
<div class="row">
    <div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
        <h1 class="page-title txt-color-blueDark">
            <i class="fa fa-users"></i> {{{ trans('vendorManagement.forms') }}}
        </h1>
    </div>
</div>
            ...
            <div>
                <div class="widget-body">
                    <ol class="breadcrumb bg-transparent border border-info">
                        ...
                        @endforeach
                        <li class="breadcrumb-item">{{{ trans('vendorManagement.scores') }}}</li>
                    </ol>
    ...
    <script>
        $(document).ready(function () {
            var mainTable = new Tabulator('#main-table', {
                ...
                ajaxURL: "{{ route('vendorPerformanceEvaluation.templateForm.nodes.scores.list', [$templateForm->id, $node->id]) }}",
                layout:"fitColumns",
                columns:[
                    {title:"{{ trans('general.no') }}", formatter:"rownum", width:60, hozAlign:'center', cssClass:"text-center text-middle", headerSort:false},
                    {title:"{{ trans('vendorManagement.name') }}", field:"name", minWidth: 300, hozAlign:"left", headerSort:false},
                    {title:"{{ trans('vendorManagement.score') }}", field:"value", width: 100, hozAlign:"center", cssClass:"text-center text-middle", headerSort:false},
            ...
            $("#score-form").on('submit', function(e){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('vendorPerformanceEvaluation.templateForm.nodes.scores.storeOrUpdate', [$templateForm->id, $node->id])}}",
                    data: dataStr,
