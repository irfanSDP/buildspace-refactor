            ...
            @foreach ( $emails as $email )
                <tr id="email{{{ $email->id }}}">
                    <td 
                        ...
                        @if ($email->isDraft())
                            data-edit-url="{{ route('email_announcements.edit', [$email->id]) }}"
                        @endif 
                        data-email-url="{{ route('email_announcements.show', [$email->id]); }}">
                    ...
                    <td class="inbox-email-datetime text-center" data-is-draft="{{{ $email->isDraft() ? 'true' : 'false' }}}" 
                        ...
                        @endif 
                        data-email-url="{{ route('email_announcements.show', [$email->id]) }}">
                        {{ \Carbon\Carbon::parse($email->updated_at)->format('Y-m-d H:i:s') }}
                    </td>
                </tr>
            @endforeach
        @endif
