<li>
	@if ( $ai->thirdLevelMessages->count() > 0)
		The AI must be complied with by <strong><i>{{{ $ai->project->getProjectTimeZoneTime($ai->deadline_to_comply) }}}</i></strong>. Otherwise, the Employer may employ and pay other Person to execute any work necessary to give effect to the AI. The Architect to verify that the AI had been complied with or otherwise.
	@else
