    ...
    <div class="row">
        <div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
            <h1 class="page-title txt-color-blueDark">
                <i class="fa fa-users"></i> {{{ trans('vendorPreQualification.forms') }}}
            </h1>
        </div>
    </div>
                ...
                <div>
                    <div class="widget-body">
                        <ol class="breadcrumb bg-transparent border border-info">
                            ...
                            @endforeach
                            <li class="breadcrumb-item">{{{ trans('vendorPreQualification.scores') }}}</li>
                        </ol>
    ...
    <script>
        $(document).ready(function () {
            var mainTable = new Tabulator('#main-table', {
                ...
                ajaxURL: "{{ route('vendorPreQualification.formLibrary.form.node.scores.list', [$vendorGroup->id, $vendorWorkCategory->id, $node->id]) }}",
                layout:"fitColumns",
                columns:[
                    {title:"{{ trans('general.no') }}", formatter:"rownum", width:60, hozAlign:'center', cssClass:"text-center text-middle", headerSort:false},
                    {title:"{{ trans('vendorPreQualification.name') }}", field:"name", minWidth: 300, hozAlign:"left", headerSort:false, formatter:function(cell){
                        var cssClass = '';
                        if(cell.getData()['amendments_required']){
                            cssClass = 'text-danger';
                        }
                        else if(cell.getData()['remarks']){
                            cssClass = "text-success";
                        }

                        return '<span class="'+cssClass+'" title="'+cell.getData()['remarks']+'">'+cell.getValue()+'</span>';
                    }},
                    {title:"{{ trans('vendorPreQualification.score') }}", field:"value", width: 100, hozAlign:"center", cssClass:"text-center text-middle", headerSort:false},
            ...
            $("#score-form").on('submit', function(e){
                ...
                $.ajax({
                    type: "POST",
                    url: "{{route('vendorPreQualification.formLibrary.form.node.scores.storeOrUpdate', [$vendorGroup->id, $vendorWorkCategory->id, $node->id])}}",
                    data: dataStr,
