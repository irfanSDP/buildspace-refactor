...
@section('breadcrumb')
	<ol class="breadcrumb">
        ...
        <li>{{ trans('projectReportNotification.editTemplate') }}</li>
	</ol>
@endsection
...
@section('content')
    <div class="row">
		<div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
			<h1 class="page-title txt-color-blueDark">{{ trans('projectReportNotification.editTemplate') }}</h1>
		</div>
	</div>
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <div class="jarviswidget">
                <header>
                    <h2><i class="fa fa-list"></i> {{ trans('projectReportNotification.editTemplate') }}</h2>
                </header>
                <div>
                    <div class="widget-body no-padding">
                        {{ Form::open(array('url' => route('projectReport.notification.update', [$project->id, $mappingId, $record->id]), 'method' => 'post', 'class' => 'smart-form', 'id' => 'reminderForm')) }}
                            <fieldset>
                                <div class="row">
                                    <section class="col col-xs-12 col-md-3 col-lg-3">
                                        ...
                                        <label class="fill-horizontal">
                                            <input type="text" name="templateName" value="{{ $record->template_name }}" class="form-control" maxlength="250" required>
                                        </label>
                                        ...
                                        <label class="fill-horizontal">
                                            <select class="{{ (! empty($categoryColumns))?'select2':'form-control' }} fill-horizontal" name="categoryColumn" id="categoryColumn" required>
                                                ...
                                                @foreach ($categoryColumns as $columnId => $columnTitle)
                                                    <option value="{{ $columnId }}" {{ $columnId == $record->category_column_id ? 'selected' : '' }}>{{ $columnTitle }}</option>
                                                @endforeach
                                            </select>
                                        </label>
                                    ...
                                    <section class="col col-xs-12 col-md-3 col-lg-3">
                                        ...
                                        <label class="fill-horizontal">
                                            <select class="{{ (! empty($periodSelections))?'select2':'form-control' }} fill-horizontal" name="periodValue[]" id="periodValue" multiple>
                                                ...
                                                @else
                                                    @for($i = 1; $i <= 100; $i++)
                                                        <option value="{{ $i }}" {{ in_array($i, $periodValues) ? 'selected' : '' }}>{{ $i }}</option>
                                                    @endfor
                                                @endif
                                            </select>
                                        </label>
                                        ...
                                        <label class="fill-horizontal">
                                            <select class="{{ (! empty($periodSelections))?'select2':'form-control' }} fill-horizontal" name="periodType" id="periodType" required>
                                                ...
                                                @foreach ($periodSelections as $key => $period)
                                                    <option value="{{ $key }}" {{ $key === $periodType ? 'selected' : '' }}>{{ $period }}</option>
                                                @endforeach
                                            </select>
                                        </label>
                                ...
                                <div class="row">
                                    <section class="col col-xs-12 col-md-6 col-lg-6">
                                        ...
                                        <label class="fill-horizontal">
                                            <input type="text" name="subject" value="{{ $record->content->subject }}" class="form-control" maxlength="250" required>
                                        </label>
                                ...
                                <div class="row">
                                    <section class="col col-xs-12 col-md-6 col-lg-6">
                                        ...
                                        <label class="fill-horizontal">
                                            <textarea name="body" class="form-control" rows="5" required>{{ $record->content->body }}</textarea>
                                        </label>
