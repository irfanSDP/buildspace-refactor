...
@section('breadcrumb')
	<ol class="breadcrumb">
		...
		<li>{{ trans('projectReportChart.editPlot') }}</li>
	</ol>
@endsection
    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <div class="jarviswidget">
                <header>
                    <h2><i class="fa fa-list"></i> {{ trans('projectReportChart.editTemplate') }}</h2>
                </header>
                <div>
                    <div class="widget-body no-padding">
                        {{ Form::open(array('url' => route('projectReport.chart.plot.template.update', array($chart->id, $record->id)), 'method' => 'post', 'class' => 'smart-form')) }}
                            <fieldset>
                                <div class="row">
                                        ...
                                        <label class="fill-horizontal">
                                            <select class="{{ (! empty($selections['plot_types']))?'select2':'form-control' }} fill-horizontal" name="plotType" id="plotType" required>
                                                ...
                                                @foreach ($selections['plot_types'] as $key => $plotType)
                                                    <option value="{{ $key }}" {{ $key == $record->plot_type ? 'selected' : '' }}>{{ $plotType }}</option>
                                                @endforeach
                                            </select>
                                        </label>
                                        ...
                                        <label class="fill-horizontal">
                                            <select class="{{ (! empty($selections['category_columns']))?'select2':'form-control' }} fill-horizontal" name="categoryColumn" id="categoryColumn" required>
                                                ...
                                                @foreach ($selections['category_columns'] as $key => $categoryColumn)
                                                    <option value="{{ $key }}" {{ $key == $record->category_column_id ? 'selected' : '' }}>{{ $categoryColumn }}</option>
                                                @endforeach
                                            </select>
                                        </label>
