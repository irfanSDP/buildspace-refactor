<div>
	<h4 role="tab" aria-selected="false" id="{{{ str_replace('%id%', $message->id, PCK\Forms\EOTMessageFourthLevelArchitectForm::accordianId) }}}">
		<span class="ui-accordion-header-icon ui-icon fa fa-plus"></span>

		{{{ PCK\Forms\EOTMessageFourthLevelArchitectForm::formTitle }}}
	</h4>
	...
	<div class="padding-10 ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom" role="tabpanel" aria-expanded="false" aria-hidden="true" style="display: none;">
		...
		<p>
			...
			<span class="dateSubmitted">{{{ $eot->project->getProjectTimeZoneTime($message->created_at) }}}</span> by {{{ $message->createdBy->present()->byWhoAndRole($eot->project, $message->created_at) }}}
		</p>
		...
		<p>
			...
			@else
				{{{ $message->grant_different_days }}} day(s) of EOT granted
			@endif
		</p>
