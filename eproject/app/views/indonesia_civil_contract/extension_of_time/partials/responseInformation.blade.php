<fieldset class="border-top {{{ ($response->sequence % 2 == 0) ?:'bg-grey-e' }}}" id="response-{{{ $response->id }}}" style="padding-top: 5px;">
    ...
    <div data-type="expandable" data-id="response-{{{ $response->id }}}" data-default="hide">
        <section>
            <label class="label">{{ trans('extensionOfTime.subject') }}:</label>
            <strong>
                {{{ $response->subject }}}
            </strong>
        </section>
        @if($response->type != \PCK\IndonesiaCivilContract\ContractualClaimResponse\ContractualClaimResponse::TYPE_PLAIN)
            <section>
                <label class="label">{{ trans('extensionOfTime.decision') }}:</label>
                <span class="text-warning">
                    <strong>
                        {{{ \PCK\IndonesiaCivilContract\ExtensionOfTime\ExtensionOfTime::getResponseTypeText($response, $response->type) }}}
                    </strong>
                </span>
            </section>
        @endif
        <section>
            <label class="label">{{{ $response->type == \PCK\IndonesiaCivilContract\ContractualClaimResponse\ContractualClaimResponse::TYPE_PLAIN ? trans('extensionOfTime.response') : trans('extensionOfTime.reason') }}}:</label>
            {{{ $response->content }}}
        </section>
