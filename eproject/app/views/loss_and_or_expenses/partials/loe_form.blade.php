<fieldset>
	...
	<section>
		<label class="label">Cause(s) of (L &amp; E)<span class="required">*</span>: {{ $errors->first('selected_clauses', '<em class="invalid" style="color: red;">:message</em>') }}</label>
	...
	<section>
		<label class="label">Deadline to submit notice to claim ({{{ $project->pam2006Detail->deadline_submitting_note_of_intention_claim_l_and_e }}} days from date above):</label>
		<label class="input">
			<span id="new_deadline">
				@if ( is_object($ai) )
					{{{ $project->getProjectTimeZoneTime(PCK\LossOrAndExpenses\LossOrAndExpense::calculateDeadlineToSubmitNoticeToClaim($project, $ai->created_at, $project->pam2006Detail->deadline_submitting_note_of_intention_claim_l_and_e)) }}}
				@else
					@if ( Input::old('commencement_date_of_event') and strtotime(Input::old('commencement_date_of_event')) )
						{{{ PCK\LossOrAndExpenses\LossOrAndExpense::calculateDeadlineToSubmitNoticeToClaim($project, Input::old('commencement_date_of_event'), $project->pam2006Detail->deadline_submitting_note_of_intention_claim_l_and_e) }}}
					@elseif ( isset($loe) )
						{{{ $project->getProjectTimeZoneTime(PCK\LossOrAndExpenses\LossOrAndExpense::calculateDeadlineToSubmitNoticeToClaim($project, $loe->commencement_date_of_event, $project->pam2006Detail->deadline_submitting_note_of_intention_claim_l_and_e)) }}}
					@else
	...
	{{ Form::hidden('deadline_days', $project->pam2006Detail->deadline_submitting_note_of_intention_claim_l_and_e, ['id' => 'deadline_days']) }}
</fieldset>