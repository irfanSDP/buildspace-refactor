<div class="accordion" class="ui-accordion ui-widget ui-helper-reset" role="tablist">
	...
	<div>
		<?php $accordianId = ( $message->type == PCK\ContractGroups\Types\Role::INSTRUCTION_ISSUER ) ? PCK\Forms\LOEMessageFirstLevelArchitectForm::accordianId : PCK\Forms\LOEMessageFirstLevelContractorForm::accordianId; ?>
		...
		<h4 role="tab" aria-selected="false" tabindex="0" id="{{{ str_replace('%id%', $message->id, $accordianId) }}}">
			...
			@if ( $message->type == PCK\ContractGroups\Types\Role::INSTRUCTION_ISSUER )
				{{{ PCK\Forms\LOEMessageFirstLevelArchitectForm::formTitle }}}
			@else
				{{{ PCK\Forms\LOEMessageFirstLevelContractorForm::formTitle }}}
			@endif
		</h4>
		...
		<div class="padding-10 ui-accordion-content ui-helper-reset ui-widget-content ui-corner-bottom" role="tabpanel" aria-expanded="false" aria-hidden="true" style="display: none;">
			...
			@if ( $message->type == PCK\ContractGroups\Types\Role::INSTRUCTION_ISSUER )
				<p>
					<strong>Decision for Contractor's Notice of Intention to Claim Loss And/Or Expense:</strong><br>
					@if ( $message->decision )
						Yes
					@else
						No
