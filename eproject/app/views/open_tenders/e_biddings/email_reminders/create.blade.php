...
<div class="modal fade in scrollable-modal" id="sendNewEmailReminderModalBox" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
        {{ Form::open(['id' => 'sendNewReminderEmailForm-' . $formId, 'data-submit-url' => route('projects.e_bidding.email_reminders.create', ['projectId' => $project->id])]) }}
            ...
            <div class="modal-footer">
                {{ Form::button('<i class="fa fa-save"></i> '.trans('forms.save'), ['type' => 'submit', 'class' => 'btn btn-success', 'name' => 'draft'] ) }}
...
$(document).ready(function () {
    $('#sendNewEmailReminderModalBox').modal({show: true});
    
    $('#sendNewEmailReminderModalBox').on('shown.bs.modal', function () {
        $('#sendNewReminderEmailForm-{{$formId}} textarea').autosize();
    });

    $('#sendNewReminderEmailForm-' + '{{{ $formId }}}').submit(function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            data: dataString,
            success: function () {
                $('#sendNewEmailReminderModalBox').modal('toggle');
