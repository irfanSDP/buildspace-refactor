...
@section('breadcrumb')
	<ol class="breadcrumb">
		...
		<li>{{ trans('letterOfAward.listOfTemplates') }}</li>
	</ol>
@endsection
...
@section('content')
	...
	<div class="row">
		<div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-list-alt"></i> {{ trans('letterOfAward.listOfTemplates') }}
			</h1>
		</div>
		...
		<div class="col-xs-12 col-sm-3 col-md-3 col-lg-3">
			<a id="btnCreateNewTemplate" href="#" class="btn btn-primary btn-md pull-right header-btn" data-target="#editorModal" data-toggle="modal">
				<i class="fa fa-plus"></i> {{ trans('letterOfAward.newTemplate') }}
			</a>
		</div>
	</div>
	...
	<div class="jarviswidget ">
		<header>
			<h2> {{ trans('letterOfAward.listOfTemplates') }} </h2>
		</header>
		<div>
	...
	@include('letter_of_award.partials.template_editor_modal')
    ...
    <script>
        $(document).ready(function() {
			...
			var actionsFormatter = function(cell, formatterParams, onRendered) {
				...
				templateButton.id = 'btnViewemplateLink_' + cell.getRow().getData().id;
				...
				editNameButton.title = "{{ trans('letterOfAward.editTemplateName') }}";
				...
				deleteButton.title = "{{ trans('letterOfAward.deleteTemplate') }}";
			...
			templateListTable = new Tabulator('#template-list-table', {
                ...
                columns: columns,
                layout:"fitColumns",
                ajaxURL: "{{ route('letterOfAward.templates.all.get') }}",
                movableColumns:true,
            ...
            $(document).on('click', '#btnCreateNewTemplate', function () {
				...
				setSubmitButtonURL("{{ route('letterOfAward.template.store') }}");
            });
