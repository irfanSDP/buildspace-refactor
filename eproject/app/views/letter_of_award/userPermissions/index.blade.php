...
@endsection
<?php use PCK\LetterOfAward\LetterOfAwardUserPermission; ?>
@section('content')

	<div class="row">
		<div class="col-xs-12 col-sm-9 col-md-9 col-lg-9">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-users"></i> {{{ trans('letterOfAward.userPermissions') }}}
			</h1>
		</div>
	...
	<div class="well no-padding-bottom">
        <form class="form-horizontal">
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <select class="select2 fill-horizontal" data-type="module_filter" id="ddlLetterOfAwardRole">
	...
	@foreach ($modules as $moduleId => $moduleName)
        <div class="row" data-type="widget" data-module-id="{{{ $moduleId }}}">
                ...
                <div class="jarviswidget">
                    ...
                    <div>
                            ...
                            <div class="table-responsive">
                                <table class="table  table-hover" data-type="table" data-module-id="{{{ $moduleId }}}">
                                    ...
                                    <tr>
                                        ...
                                        @if ($moduleId == LetterOfAwardUserPermission::REVIEWER)
                                            <th>&nbsp;</th>
                                        @endif
                                    </tr>
                                    <tr>
                                        ...
                                        <th class="text-middle text-center text-nowrap">{{{ trans('users.email') }}}</th>
                                        @if ($moduleId == LetterOfAwardUserPermission::REVIEWER)
                                            <th class="text-middle text-center text-nowrap squeeze">{{{ trans('users.isEditor') }}}</th>
                                        @endif
                                    </tr>
                                    </thead>
                                </table>
                        ...
                        <div class="widget-footer">
                            <a class="btn btn-info" data-toggle="modal" data-target="#assignUsersModal" data-action="assignUsers"  data-module-id="{{{ $moduleId }}}">
                                <i class="fa fa-plus" aria-hidden="true"></i>&nbsp;&nbsp;
                                {{{ trans('letterOfAward.addUsers') }}}
                            </a>
                        </div>
                    </div>
                </div>
	...
	@include('form_partials.assign_users_modal', [
        'saveButtonLabel'	=> trans('letterOfAward.addUsers'),
    ])
...
@section('js')
	...
	<script>
		...
		var assignUsersTable = $('#assign-users-table').DataTable({
            ...
            "iDisplayLength":10,
            bServerSide:true,
            "sAjaxSource":"{{ route('letterOfAward.user.permissions.assignable', [$project->id]) }}",
		...
		$('#assignUsersModal [data-action=submit]').on('click', function(){
            ...
            $.ajax({
                url: "{{ route('letterOfAward.user.permissions.assign', array($project->id)) }}",
                method: 'POST',
                data: {
                    _token: '{{{ csrf_token() }}}',
        ...
        @foreach ($modules as $moduleId => $moduleName)
            $('[data-type=table][data-module-id={{{ $moduleId }}}]').DataTable({
                ...
                "sAjaxSource":"{{ route('letterOfAward.user.permissions.assigned', array($project->id)) }}",
                "fnServerParams": function ( aoData ) {
                ...
                "aoColumnDefs": [
                    ...
                    },
                    {
                        ...
                        "mData": function ( source, type, val ) {
                            ...
                            var displayData = source['name']
                                        ...
                                        + '<button type="button" data-action="revoke" data-url="' + source['route:revoke'] + '" data-module-id="'+moduleId+'" data-id="' + source['id'] + '" data-pending_check_url="' + source['route:revokePendingCheck'] + '" type="tooltip" title="'+title+'" class="btn btn-xs btn-danger">'
                    ...
                    },
                    {
                        ...
                        "mData": function ( source, type, val ) {
                            ...
                            return '<input type="checkbox" data-action="toggle-editor-status" data-url="'+source['route:toggleEditorStatus']+'" data-module-id="'+moduleId+'" value="" '+checked+'>';
                        },
                        "sClass": "text-middle text-center text-nowrap squeeze",
                        "visible" : {{{ $moduleId == LetterOfAwardUserPermission::REVIEWER ? 'true' : 'false' }}}
                    }
                ]
            });
        ...
        $('[data-type=table]').on('change', '[data-action=toggle-editor-status]', function(){
            var self = this;
            app_progressBar.toggle();
            $.ajax({
                url: $(this).data('url'),
        ...
        $(document).on('click', '[data-action=revoke]', function() {
            ...
            var checkPendingUrl = $(this).data('pending_check_url');
            ...
            $.ajax({
                ...
                if(response.hasPendingLetterOfAward) {
                    $.smallBox({
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            $('[data-type=table][data-module-id=' + moduleId + ']').DataTable().draw();
                        },
                        error: function (jqXHR, textStatus, errorThrown) {
                            // error
                        }
        ...
        {
            autoSelectValue = document.location.hash.substr(1);
        }
        else
        {
            autoSelectValue = '{{{ $firstKey }}}';
        }

        $('select[data-type=module_filter]').val(autoSelectValue).trigger('change');
	</script>
@endsection