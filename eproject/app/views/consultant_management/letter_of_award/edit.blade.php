                    ...
                    <div class="row">
                        <section class="col col-xs-12 col-md-12 col-lg-12">
                            <dl class="dl-horizontal no-margin">
                                <dt>{{{ trans('formOfTender.template') }}} :</dt>
                                <dd>
                                    <select class="fill-horizontal" name="template" id="template-select">
                    ...
                    <div class="row">
                        <section class="col col-xs-12 col-md-12 col-lg-12">
                            <dl class="dl-horizontal no-margin">
                                ...
                                <dd>
                                    {{{ $letterOfAward->reference_number }}}
                                </dd>
                                <dt>&nbsp;</dt>
                                <dd>&nbsp;</dd>
                            </dl>
                            ...
                            <div id="consultant-management-load-tab-content" class="tab-content padding-10">
                                <div class="tab-pane fade in active " id="consultant-management-loa-tab-letter-head">
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                            ...
                                            {{ HTML::decode(link_to_route('consultant.management.loa.letterhead.edit', '<i class="fa fa-edit"></i> '.trans('forms.edit'), [$vendorCategoryRfp->id], ['class' => 'btn btn-primary'])) }}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="tab-pane fade in" id="consultant-management-loa-tab-clauses">
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                            <div class="pull-right" style="padding-bottom:12px;">
                                            {{ HTML::decode(link_to_route('consultant.management.loa.clause.edit', '<i class="fa fa-edit"></i> '.trans('forms.edit'), [$vendorCategoryRfp->id], ['class' => 'btn btn-primary'])) }}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="tab-pane fade in" id="consultant-management-loa-tab-signatory">
                                    <div class="row">
                                        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                            <div class="pull-right" style="padding-bottom:12px;">
                                            {{ HTML::decode(link_to_route('consultant.management.loa.signatory.edit', '<i class="fa fa-edit"></i> '.trans('forms.edit'), [$vendorCategoryRfp->id], ['class' => 'btn btn-primary'])) }}
                                            </div>
                                        </div>
                                    </div>
        ...
        <div class="jarviswidget ">
            ...
            <div>
                <div class="widget-body">
                    ...
                    @if($letterOfAward->status == PCK\ConsultantManagement\LetterOfAward::STATUS_DRAFT)
                    <hr class="simple">
                    {{ Form::open(['route' => ['consultant.management.loa.verifier.store', $vendorCategoryRfp->id], 'class' => 'smart-form']) }}
                    <div class="row">
...
@include('templates.yesNoModal', [
    ...
    'message'   => trans('general.areYouSure') .' This will <span class="text-danger">overwrite</span> all your existing Letter of Appointment content.',
])
...
function formatTemplateSelect2(template){
    return (parseInt(template.id) == 0) ? template.text : '<i class="far fa-file"></i> '+template.text;
}
$(document).ready(function () {
    $("#template-select").select2({
        theme: 'bootstrap',
        width: 'resolve',
        templateResult: formatTemplateSelect2,
        templateSelection: formatTemplateSelect2,
        escapeMarkup: function(m) { return m; }
    });

    $('#template-select').on('select2:select', function(e){
        e.preventDefault();
        if(parseInt($('#template-select').val()) != 0){
            $('#yesNoModal').modal('show');
        }
    });

    $(document).on('click', '#yesNoModal [data-action=actionNo]', function(e) {
        e.preventDefault();
    ...
    $(document).on('click', '#yesNoModal [data-action=actionYes]', function(e) {
        ...
        $.ajax({
            url: '{{ route("consultant.management.loa.content.update", [$vendorCategoryRfp->id]) }}',
            method: 'POST',
            data: {
                _token: '{{{ csrf_token() }}}',
                id: parseInt($('#template-select').val())
            },
            success: function (response) {
                app_progressBar.maxOut();
                if(response.success){
                    window.location.replace('{{ route("consultant.management.loa.index", [$vendorCategoryRfp->id]) }}');
                }else{
                    app_progressBar.toggle();
