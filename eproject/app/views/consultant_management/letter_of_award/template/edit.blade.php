...
<ol class="breadcrumb">
    ...
    @if(isset($loa))
    <li>{{ link_to_route('consultant.management.loa.templates.show', $loa->short_title, [$loa->id]) }}</li>
        @if($type=='letterhead')
        ...
        @else
        <li>{{{ trans('forms.edit') }}} {{{trans('formOfTender.template')}}}</li>
        @endif
    @else
    <li>{{{ trans('forms.add') }}} {{{trans('formOfTender.template')}}}</li>
    @endif
</ol>
@endsection
...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="jarviswidget ">
            <header>
                <h2>
                    @if(isset($loa)) {{{ trans('forms.edit') }}} {{{ $loa->short_title }}} @else {{{ trans('forms.add') }}} {{{trans('formOfTender.template')}}} @endif
                    @if($type == 'letterhead')
                    :: {{{ trans('letterOfAward.letterHead') }}}
                    @elseif($type == 'signatory')
                    :: {{{ trans('letterOfAward.signatory') }}}
            ...
            <div>
                <div class="widget-body">
                    {{ Form::open(['route' => [$store], 'class' => 'smart-form']) }}
                    @if($type=='letterhead')
                    ...
                    @elseif($type=='clause')
                    @include('consultant_management.letter_of_award.partials.forms.clause', ['isTemplate'=>true])
                    @else
                    @include('consultant_management.letter_of_award.partials.forms.title')
                    @endif
                    <footer>
                        {{ Form::hidden('id', (isset($loa)) ? $loa->id : -1) }}
                        @if(isset($loa))
                        {{ link_to_route('consultant.management.loa.templates.show', trans('forms.back'), [$loa->id], ['class' => 'btn btn-default']) }}
                        @else
