...
@section('breadcrumb')
    <ol class="breadcrumb">
        ...
        <li>{{{ trans("general.attachmentSettings") }}}</li>
    </ol>
@endsection
...
<div class="row">
    ...
    @if($user->isSuperAdmin() or ($user->isGroupAdmin() && ($user->isConsultantManagementEditorByRole($consultantManagementContract, PCK\ConsultantManagement\ConsultantManagementContract::ROLE_RECOMMENDATION_OF_CONSULTANT) or $user->isConsultantManagementEditorByRole($consultantManagementContract, PCK\ConsultantManagement\ConsultantManagementContract::ROLE_LIST_OF_CONSULTANT))))
        <a href="{{ route('consultant.management.attachment.settings.create', [$consultantManagementContract->id]) }}" class="btn btn-primary btn-md pull-right header-btn">
            <i class="fa fa-plus"></i> {{{ trans('general.new') }}} {{{trans('general.attachmentSettings')}}}
        </a>
    @endif
    </div>
</div>
        ...
        <div class="jarviswidget ">
            <div>
                <div class="widget-body no-padding">
                    <div id="attachment_settings-table"></div>
                </div>
            </div>
        </div>
    </div>
...
$(document).ready(function () {
    new Tabulator('#attachment_settings-table', {
        fillHeight:true,
        placeholder: "{{ trans('general.noRecordsFound') }}",
        ajaxURL: "{{ route('consultant.management.attachment.settings.ajax.list', [$consultantManagementContract->id]) }}",
        ajaxConfig: "GET",
        paginationSize: 100,
        pagination: "remote",
        ajaxFiltering:true,
