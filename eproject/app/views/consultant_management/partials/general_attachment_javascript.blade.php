...
$(document).ready(function () {
    ...
    $(document).on('click', '[data-action="upload-item-attachments"]', function(e) {
        ...
        $('#general_attachment-upload-form').prop('action', attachmentsUpdateUrl);
    });
    ...
    $(document).on('click', '[data-action=submit-attachments]', function(){
        ...
        $('form#general_attachment-upload-form input[name="uploaded_files[]"]').each(function(index){
            uploadedFilesInput.push($(this).val());
        });

        app_progressBar.show();

        $.post($('form#general_attachment-upload-form').prop('action'),{
            _token: _csrf_token,
    ...
    $(document).on('click', '[data-action="list-item-attachments"]', function(e) {
        ...
        $('#generalAttachmentModal').data('url', $(this).data('route-get-attachments-list'));
        $('#generalAttachmentModal').modal('show');
    });

    $('#generalAttachmentModal').on('shown.bs.modal', function(e) {
        e.preventDefault();

        var url = $(this).data('url');

        var generalAttachmentTable = new Tabulator('#generalAttachmentTable', {
            height:360,
            pagination:"local",
            columns: [
