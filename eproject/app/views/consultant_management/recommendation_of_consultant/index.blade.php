...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <h1 class="page-title txt-color-blueDark">
            <i class="fa fa-file-signature"></i> {{{ trans('general.recommendationOfConsultant') }}}
        </h1>
    </div>
</div>

@if($recommendationOfConsultant && $recommendationOfConsultant->needApprovalFromUser(Confide::user()))
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="jarviswidget ">
            <header>
                <h2><i class="fa fa-user-tie"></i> {{{$recommendationOfConsultant->getStatusText()}}}</h2>
            </header>   
            <div>
                <div class="widget-body">
                    {{ Form::open(['route' => ['consultant.management.roc.verify', $vendorCategoryRfp->id], 'class' => 'smart-form']) }}
                    <div class="row">
            ...
            <div>
                <div class="widget-body no-padding">
                    <div class="smart-form">
                        ...
                        {{ $errors->first('empty_phase', '<em class="ps-4 pt-4 pb-4 invalid">:message</em>') }}
                        {{ $errors->first('loc_company', '<em class="ps-4 pt-4 pb-4 invalid">:message</em>') }}
                    </div>
...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="jarviswidget ">
            <header>
                <h2><i class="fa fa-building"></i> {{{ $vendorCategoryRfp->vendorCategory->name }}} @if($recommendationOfConsultant) <span class="badge bg-color-green inbox-badge">{{{$recommendationOfConsultant->getStatusText()}}}</span> @endif</h2>
            </header>
            <div>
                <div class="widget-body">
                    @if (!$recommendationOfConsultant || $recommendationOfConsultant->editableByUser(Confide::user()) )
                    @include('consultant_management.recommendation_of_consultant.partials.form')
                    @else
                    @include('consultant_management.recommendation_of_consultant.partials.info')
                    @endif
                </div>
            </div>
        </div>
...
@if (!$recommendationOfConsultant || $recommendationOfConsultant->editableByUser(Confide::user()) )
    @include('consultant_management.partials.select_consultants_modal', [
        'modalId' => 'recommendationOfConsultant-select_consultant-modal',
    ])
@endif
...
var consultantStatusFormatter = function(cell, formatterParams, onRendered){
    ...
    switch(data.status.toString()) {
        case "{{ PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_YES }}": 
            cellValue = "{{ PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_YES_TEXT }}";
            break;
        case "{{ PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_NO }}": 
            cellValue = "{{ PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_NO_TEXT }}";
            break;
        default:
            cellValue = "{{ PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_PENDING_TEXT }}";
    }
...
$(document).ready(function () {
    ...
    var selectedConsultantTable = new Tabulator('#selected_consultants-table', {
        ...
        ajaxURL: "{{ route('consultant.management.roc.selected.consultants.ajax.list', $vendorCategoryRfp->id) }}",
        ajaxConfig: "GET",
        paginationSize: 100,
        pagination: "remote",
        ajaxFiltering:true,
        ...
        columns:[
            @if (!$recommendationOfConsultant || $recommendationOfConsultant->editableByUser(Confide::user()) )
            {title:"{{ trans('general.remove') }}", field:"id", width:80, hozAlign:'center', cssClass:"text-center text-middle", headerSort:false, formatter:app_tabulator_utilities.variableHtmlFormatter, formatterParams: {
                innerHtml: [{
            ...
            {title:"{{ trans('general.status') }}", field:"status_txt", width:128, hozAlign:'center', cssClass:"text-center text-middle", headerSort:false, formatter:consultantStatusFormatter, editor:"select", editorParams:{
                values:{
                    "{{PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_PENDING}}":"{{PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_PENDING_TEXT}}",
                    "{{PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_YES}}":"{{PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_YES_TEXT}}",
                    "{{PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_NO}}":"{{PCK\ConsultantManagement\ConsultantManagementRecommendationOfConsultantCompany::STATUS_NO_TEXT}}",
                }}
            },
            @else
    ...
    @if (!$recommendationOfConsultant || $recommendationOfConsultant->editableByUser(Confide::user()) )
    var selectConsultantTable = new Tabulator('#selectConsultantTableContainer', {
        height:420,
        placeholder: "{{ trans('general.noRecordsFound') }}",
        ajaxURL: "{{ route('consultant.management.roc.consultants.ajax.list', $vendorCategoryRfp->id) }}",
        ajaxConfig: "GET",
        paginationSize: 100,
        pagination: "remote",
        ajaxFiltering:true,
        ...
        columns:[
            ...
            {title:"{{ trans('vendorManagement.consultant') }}", field:"name", minWidth: 300, hozAlign:"left", headerSort:false, headerFilter: true},
    ...
    $('#recommendationOfConsultant-select_consultant-modal').on('hidden.bs.modal', function (e) {
        selectConsultantTable.clearFilter(true);
    });

    $('#select_consultant_modal-btn').on('click', function(e){
        var selectedConsultants = [];
        $.each(selectConsultantTable.getSelectedData(), function(index, data){
            selectedConsultants.push(data.id);
        });

        if(selectedConsultants.length){
            app_progressBar.toggle();
            $.ajax({
                url: '{{{ route("consultant.management.roc.select.consultant.store", $vendorCategoryRfp->id) }}}',
                method: 'POST',
                data: {
                    _token: '{{{ csrf_token() }}}',
                ...
                success: function (data) {
                    if (data['success']) {
                        $('#recommendationOfConsultant-select_consultant-modal').modal('hide');

                        selectedConsultantTable.setData();
                    }
                    app_progressBar.maxOut();
                    app_progressBar.toggle();
                },
    ...
    var verifierLogTable = new Tabulator('#verifier_logs-table', {
        height:280,
        placeholder: "{{ trans('general.noRecordsFound') }}",
        ajaxURL: "{{ route('consultant.management.roc.verifier.ajax.log', $vendorCategoryRfp->id) }}",
        ajaxConfig: "GET",
        paginationSize: 100,
        pagination: "remote",
        ajaxFiltering:true,
