...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <h1 class="page-title txt-color-blueDark">
            <i class="fa fa-th-list"></i> {{{ trans('general.listOfConsultant') }}} {{{trans('documentManagementFolders.revision')}}} {{$listOfConsultant->consultantManagementRfpRevision->revision}}
        </h1>
    </div>
</div>

@if(!$listOfConsultant->editableByUser($user) && $listOfConsultant->needApprovalFromUser(Confide::user()))
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="jarviswidget ">
            <header>
                <h2><i class="fa fa-user-tie"></i> {{{$listOfConsultant->getStatusText()}}}</h2>
            </header>
            <div>
                <div class="widget-body">
                    {{ Form::open(['route' => ['consultant.management.loc.verify', $vendorCategoryRfp->id], 'class' => 'smart-form']) }}
                    <div class="row">
...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="jarviswidget ">
            <header>
                <h2><i class="fa fa-building"></i> {{{ $vendorCategoryRfp->vendorCategory->name }}} <span class="badge bg-color-green inbox-badge">{{{$listOfConsultant->getStatusText()}}}</span> </h2>
            </header>
            <div>
                <div class="widget-body">
                    @if ($listOfConsultant->editableByUser($user) )
                    @include('consultant_management.list_of_consultant.partials.form')
                    @else
                    @include('consultant_management.list_of_consultant.partials.info')
                    @endif
                </div>
            </div>
        </div>
...
@if ($listOfConsultant->editableByUser($user) )
    @include('consultant_management.partials.select_consultants_modal', [
        'modalId' => 'listOfConsultant-select_consultant-modal',
    ])
@endif
...
@include('consultant_management.list_of_consultant.partials.attachment')
...
@include('consultant_management.list_of_consultant.partials.attachment_javascript')
...
var consultantStatusFormatter = function(cell, formatterParams, onRendered){
    ...
    switch(data.status.toString()) {
        case "{{ PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_YES }}": 
            cellValue = "{{ PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_YES_TEXT }}";
            break;
        case "{{ PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_NO }}": 
            cellValue = "{{ PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_NO_TEXT }}";
            break;
        default:
            cellValue = "{{ PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_PENDING_TEXT }}";
    }
...
$(document).ready(function () {
    ...
    var selectedConsultantTable = new Tabulator('#selected_consultants-table', {
        ...
        ajaxURL: "{{ route('consultant.management.loc.selected.consultants.ajax.list', [$vendorCategoryRfp->id, $listOfConsultant->id]) }}",
        ajaxConfig: "GET",
        paginationSize: 100,
        pagination: "remote",
        ajaxFiltering:true,
        ...
        columns:[
            ...
            {title:"{{ trans('general.status') }}", field:"status_txt", width:128, hozAlign:'center', cssClass:"text-center text-middle", headerSort:false, formatter:consultantStatusFormatter, editor:"select", editorParams:{
                values:{
                    "{{PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_PENDING}}":"{{PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_PENDING_TEXT}}",
                    "{{PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_YES}}":"{{PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_YES_TEXT}}",
                    "{{PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_NO}}":"{{PCK\ConsultantManagement\ConsultantManagementListOfConsultantCompany::STATUS_NO_TEXT}}",
                }}
            },
            @else
    ...
    var selectConsultantTable = new Tabulator('#selectConsultantTableContainer', {
        height:420,
        placeholder: "{{ trans('general.noRecordsFound') }}",
        ajaxURL: "{{ route('consultant.management.loc.consultants.ajax.list', [$vendorCategoryRfp->id, $listOfConsultant->id]) }}",
        ajaxConfig: "GET",
        paginationSize: 100,
        pagination: "remote",
        ajaxFiltering:true,
    ...
    $('#listOfConsultant-select_consultant-modal').on('hidden.bs.modal', function (e) {
    ...
    $('#select_consultant_modal-btn').on('click', function(e){
        ...
        if(selectedConsultants.length){
            app_progressBar.toggle();
            $.ajax({
                url: '{{{ route("consultant.management.loc.select.consultant.store", [$vendorCategoryRfp->id, $listOfConsultant->id]) }}}',
                method: 'POST',
                data: {
                    _token: '{{{ csrf_token() }}}',
                ...
                success: function (data) {
                    if (data['success']) {
                        $('#listOfConsultant-select_consultant-modal').modal('hide');
    ...
    var verifierLogTable = new Tabulator('#verifier_logs-table', {
        height:280,
        placeholder: "{{ trans('general.noRecordsFound') }}",
        ajaxURL: "{{ route('consultant.management.loc.verifier.ajax.log', [$vendorCategoryRfp->id, $listOfConsultant->id]) }}",
        ajaxConfig: "GET",
        paginationSize: 100,
        pagination: "remote",
        ajaxFiltering:true,
