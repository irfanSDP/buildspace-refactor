...
<div class="row">
    ...
    <div class="col-xs-12 col-sm-3 col-md-3 col-lg-3">
        <a href="{{ route('vendorManagement.approval.projectTrackRecord.create', [$vendorRegistration->id]) }}" class="btn btn-primary btn-md pull-right header-btn">
...
<div class="row">
    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        <div class="jarviswidget padded">
            <footer>
                @include('vendor_management.partials.link_to_next_registration_approval_section', ['vendorRegistration' => $vendorRegistration, 'currentSection' => 'projectTrackRecord'])
                @if($canReject)
                    <form action="{{ route('vendorManagement.approval.projectTrackRecord.reject', [$vendorRegistration->id])}}" method="POST">
                ...
                <button type="button" class="btn btn-info pull-left spaced" data-action="upload-item-attachments" 
                        data-route-get-attachments-list="{{ route('vendorManagement.approval.processor.attachments.list', [$vendorRegistration->id, ObjectField::PROCESSOR_ATTACHMENTS_PROJECT_TRACK_RECORDS]) }}"
                        data-route-update-attachments="{{ route('vendorManagement.approval.processor.attachments.upload', [$vendorRegistration->id, ObjectField::PROCESSOR_ATTACHMENTS_PROJECT_TRACK_RECORDS]) }}"
                        data-route-get-attachments-count="{{ route('vendorManagement.approval.processor.attachments.count', [$vendorRegistration->id, ObjectField::PROCESSOR_ATTACHMENTS_PROJECT_TRACK_RECORDS]) }}"
                        data-field="{{ ObjectField::PROCESSOR_ATTACHMENTS_PROJECT_TRACK_RECORDS }}">
                    <?php 
                        $record = ObjectField::findRecord($vendorRegistration, ObjectField::PROCESSOR_ATTACHMENTS_PROJECT_TRACK_RECORDS);
            ...
            @if($section->amendmentsMade() || $section->amendmentsRequired())
                <form action="{{ route('vendorManagement.approval.projectTrackRecord.resolve', [$vendorRegistration->id])}}" method="POST" id="resolve-form">
                    <input type="hidden" name="_token" value="{{{ csrf_token() }}}">
                </form>
            @endif
        </div>
        ...
        $(document).ready(function () {
            ...
            new Tabulator('#completed-table', {
                ...
                columns:[
                    ...
                    {title:"{{ trans('vendorManagement.vendorSubWorkCategory') }}", field:"vendorSubWorkCategory", width: 180, hozAlign:"center", cssClass:"text-center text-middle", headerSort:false},
                    ...
                    @elseif($setting->project_detail_attachments)
                    {title:"{{ trans('general.actions') }}", width: 120, hozAlign:"center", cssClass:"text-center text-middle", headerSort:false, formatter:app_tabulator_utilities.variableHtmlFormatter, formatterParams: {
                        show: function(cell){
                            return cell.getData().hasOwnProperty('id');
                        },
            ...
            $('#actionLogsModal').on('shown.bs.modal', function(e) {
                ...
                actionLogsTable = new Tabulator('#actionLogsTable', {
                    ...
                    layout:"fitColumns",
                    ajaxURL: "{{ route('vendorManagement.approval.projectTrackRecord.action.logs.get', [$vendorRegistration->id]) }}",
                    movableColumns:true,
